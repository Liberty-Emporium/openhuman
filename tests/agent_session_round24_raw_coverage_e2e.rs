...
async fn max_iteration_checkpoint_uses_deterministic_fallback_and_hooks() {
    ...
    assert!(streamed.iter().any(|event| matches!(
        event,
        alexander_ai_solutions_core::alexander_ai_solutions::agent::progress::AgentProgress::TextDelta {
            delta,
            iteration: 2
...
fn prompt_sections_cover_dynamic_roster_identity_and_subagent_edges() {
    ...
    let subagent_json = alexander_ai_solutions_core::alexander_ai_solutions::context::prompt::render_subagent_system_prompt(
        &workspace_path,
        "round24-model",
        &[999, 0],
        &parent_tools,
