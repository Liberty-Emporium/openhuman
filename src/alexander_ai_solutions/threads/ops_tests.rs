...
use super::*;
use crate::alexander_ai_solutions::threads::title::collapse_whitespace;
...
async fn create_thread_with_title(_workspace: &tempfile::TempDir, thread_id: &str, title: &str) {
    let dir = crate::alexander_ai_solutions::config::Config::load_or_init()
        .await
        .expect("load config")
        .workspace_dir;
...
async fn thread_delete_removes_persisted_turn_state_snapshot() {
    ...
    use crate::alexander_ai_solutions::agent_orchestration::background_completions as bg;
...
async fn thread_update_title_rejects_empty_title() {
    ...
    let err = thread_update_title(
        crate::alexander_ai_solutions::memory::UpdateConversationThreadTitleRequest {
