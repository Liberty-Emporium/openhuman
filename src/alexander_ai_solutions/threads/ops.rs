...
use crate::alexander_ai_solutions::channels::providers::web as web_channel;
use crate::alexander_ai_solutions::config::Config;
use crate::alexander_ai_solutions::inference::provider::{self, ProviderRuntimeOptions};
use crate::alexander_ai_solutions::memory::{
...
use crate::alexander_ai_solutions::threads::title::{
...
pub async fn thread_delete(
    ...
    let cancelled =
        crate::alexander_ai_solutions::agent_orchestration::running_subagents::cancel_for_thread(
            &request.thread_id,
        );
    let discarded =
        crate::alexander_ai_solutions::agent_orchestration::background_completions::discard_for_thread(
            &request.thread_id,
        );
    log::debug!(
...
pub async fn threads_purge(
    ...
    use crate::alexander_ai_solutions::agent_orchestration::{
        background_completions, running_subagents,
    };
...
pub async fn token_usage(
    ...
    let dir = workspace_dir().await?;
    let summary = crate::alexander_ai_solutions::agent::harness::session::transcript::read_thread_usage_summary(
        &dir,
        &request.thread_id,
    );
    ...
    let audit_cost =
        |model: Option<&str>, input: u64, output: u64, cached: u64, fallback: f64| match model {
            Some(m) => crate::alexander_ai_solutions::agent::cost::estimate_call_cost_usd(
                m,
                &crate::alexander_ai_solutions::inference::provider::UsageInfo {
                    input_tokens: input,
    ...
    let response = match summary {
        Some(s) => {
            let context_window = s
                .model
                .as_deref()
                .and_then(crate::alexander_ai_solutions::inference::model_context::context_window_for_model)
                .unwrap_or(0);
