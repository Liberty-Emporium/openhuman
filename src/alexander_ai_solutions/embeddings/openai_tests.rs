...
async fn embed_429_indefinite_bails_after_retry_cap() {
    use crate::alexander_ai_solutions::embeddings::retry_after::MAX_429_RETRIES;
    use std::sync::{Arc, Mutex};
    let counter = Arc::new(Mutex::new(0u32));
    let counter_clone = counter.clone();
...
async fn embed_429_without_retry_after_uses_exponential_backoff() {
    use crate::alexander_ai_solutions::embeddings::retry_after::{
        backoff_ms_for_attempt, BASE_BACKOFF_MS,
    };
    use std::sync::{Arc, Mutex};
...
async fn embed_429_no_retry_after_header_falls_back_to_exponential() {
    use crate::alexander_ai_solutions::embeddings::retry_after::BASE_BACKOFF_MS;
    use std::sync::{Arc, Mutex};
