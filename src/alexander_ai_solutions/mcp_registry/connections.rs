...
//! similar) lives in [`crate::alexander_ai_solutions::mcp_client`] — this module just
...
use crate::alexander_ai_solutions::config::{Config, HttpHeader, McpAuthConfig};
use crate::alexander_ai_solutions::mcp_client::{McpHttpClient, McpRemoteTool, McpStdioClient};
...
/// [`crate::alexander_ai_solutions::mcp_client::registry::McpTransportClient`] but lives
...
impl ActiveClient {
    ...
    async fn call_tool(
        ...
        name: &str,
        arguments: Value,
    ) -> anyhow::Result<crate::alexander_ai_solutions::mcp_client::McpServerToolResult> {
        match self {
...
fn is_unauthorized_error(err: &anyhow::Error) -> bool {
    ...
    err.chain()
        .any(|cause| cause.is::<crate::alexander_ai_solutions::mcp_client::McpUnauthorizedError>())
}
...
async fn connect_inner(config: &Config, server: &InstalledServer) -> anyhow::Result<Vec<McpTool>> {
    ...
    let safe_remote_tools = crate::alexander_ai_solutions::mcp_client::apply_safety_filter(
        &server.server_id,
        remote_tools,
    );
...
mod tests {
    ...
    use crate::alexander_ai_solutions::config::McpAuthConfig;
    use crate::alexander_ai_solutions::mcp_client::McpUnauthorizedError;
    use crate::alexander_ai_solutions::mcp_registry::types::ServerStatus;
