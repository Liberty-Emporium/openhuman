...
fn handle_snapshot(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async { to_json(crate::alexander_ai_solutions::health::rpc::health_snapshot()) })
}

fn handle_system_info(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async { to_json(crate::alexander_ai_solutions::health::rpc::system_info()) })
}
