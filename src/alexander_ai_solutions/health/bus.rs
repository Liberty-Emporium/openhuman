    ...
    async fn handle(&self, event: &DomainEvent) {
        match event {
            DomainEvent::SystemStartup { component } => {
                crate::alexander_ai_solutions::health::mark_component_ok(component);
            }
            ...
            DomainEvent::HealthRestarted { component } => {
                crate::alexander_ai_solutions::health::bump_component_restart(component);
            }
            DomainEvent::ChannelConnected { channel } => {
                crate::alexander_ai_solutions::health::mark_component_ok(&format!(
                    "channel:{channel}"
                ));
            }
