...
fn start_globe_hotkey_listener(
    ...
    String,
> {
    use crate::alexander_ai_solutions::accessibility::{globe_listener_poll, globe_listener_start};
...
async fn process_recording_bg(
    ...
    match handle.stop().await {
        Ok(result) => {
            ...
            match crate::alexander_ai_solutions::voice::voice_transcribe_bytes(
                config,
                &result.wav_bytes,
                Some("wav".to_string()),
                context,
...
pub async fn start_if_enabled(app_config: &Config) {
    ...
    let server_config = VoiceServerConfig {
        ...
        activation_mode: match app_config.voice_server.activation_mode {
            crate::alexander_ai_solutions::config::VoiceActivationMode::Tap => ActivationMode::Tap,
