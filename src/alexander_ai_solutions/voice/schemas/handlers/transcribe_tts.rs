...
pub(crate) fn handle_voice_status(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::alexander_ai_solutions::voice::voice_status(&config).await?)
    })
}
...
pub(crate) fn handle_voice_transcribe(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::alexander_ai_solutions::voice::voice_transcribe(
                &config,
                &p.audio_path,
                p.context.as_deref(),
                p.skip_cleanup,
...
pub(crate) fn handle_voice_transcribe_bytes(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::alexander_ai_solutions::voice::voice_transcribe_bytes(
                &config,
                &p.audio_bytes,
                p.extension,
                p.context.as_deref(),
...
pub(crate) fn handle_voice_tts(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::alexander_ai_solutions::voice::voice_tts(
                &config,
                &p.text,
...
pub(crate) fn handle_voice_cloud_transcribe(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let opts = crate::alexander_ai_solutions::voice::cloud_transcribe::CloudTranscribeOptions {
            model: p.model,
            language: p.language,
        ...
        to_json(
            crate::alexander_ai_solutions::voice::cloud_transcribe::transcribe_cloud(
                &config,
                &p.audio_base64,
                &opts,
            )
...
pub(crate) fn handle_voice_stt_dispatch(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let model = p
            ...
            .map(str::to_string)
            .unwrap_or_else(|| {
                crate::alexander_ai_solutions::voice::DEFAULT_WHISPER_MODEL.to_string()
            });
