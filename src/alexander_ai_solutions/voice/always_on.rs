...
use crate::alexander_ai_solutions::config::VoiceServerConfig as CfgVoiceServer;
...
use crate::alexander_ai_solutions::voice::audio_capture::{
...
fn notch_status(status: &str, ttl_ms: u32) {
    let _ = crate::alexander_ai_solutions::overlay::publish_attention(
        crate::alexander_ai_solutions::overlay::OverlayAttentionEvent::new(status)
            .with_source("voice")
            .with_ttl_ms(ttl_ms),
    );
}
...
async fn transcribe_and_deliver(config: &Config, samples_16k: Vec<f32>) {
    ...
    let provider_name = crate::alexander_ai_solutions::voice::effective_stt_provider(config);
    let model = crate::alexander_ai_solutions::voice::DEFAULT_WHISPER_MODEL.to_string();
    ...
    let provider = match crate::alexander_ai_solutions::voice::create_stt_provider(
        &provider_name,
        &model,
        config,
    ) {
...
async fn deliver_command(config: &Config, cmd: String) {
    use crate::alexander_ai_solutions::voice::command_router::{route, VoiceIntent};
    ...
    if matches!(intent, VoiceIntent::Unknown) {
        ...
        crate::alexander_ai_solutions::voice::dictation_listener::publish_transcription(cmd);
        return;
    }
    log::info!(
...
async fn execute_intent(
    config: &Config,
    intent: crate::alexander_ai_solutions::voice::command_router::VoiceIntent,
) -> Result<String, String> {
    use crate::alexander_ai_solutions::voice::command_router::VoiceIntent as VI;
    match intent {
        VI::Play { query } => {
            let backend = crate::alexander_ai_solutions::accessibility::automate::RealBackend::new(
                config.clone(),
            );
            let out = crate::alexander_ai_solutions::accessibility::automate::run(
                "Music",
                &format!("play {query}"),
                &backend,
                crate::alexander_ai_solutions::accessibility::automate::AutomateOptions::default(),
            )
            .await;
            if out.success {
                Ok(out.summary)
        ...
        VI::OpenApp { app } => {
            crate::alexander_ai_solutions::tools::implementations::system::launch_platform(&app)
                .await
        }
...
fn capture_on_thread(
    ...
    use crate::alexander_ai_solutions::accessibility::{
        detect_microphone_permission, PermissionState,
    };
