...
use crate::alexander_ai_solutions::voice::server::{run_standalone, VoiceServerConfig};
...
pub(crate) fn run_standalone_subcommand(args: &[String]) -> Result<()> {
    ...
    rt.block_on(async {
        let mut config = match crate::alexander_ai_solutions::config::Config::load_or_init().await {
            Ok(cfg) => cfg,
            Err(e) => {
