...
use crate::alexander_ai_solutions::tinyplace::ops::{global_state, map_err};
use crate::alexander_ai_solutions::tinyplace::payment::{
...
async fn wallet_usdc_balance(address: &str) -> (Option<Value>, String) {
    ...
    let mint = crate::alexander_ai_solutions::wallet::solana_cluster().usdc_mint();
    let endpoints = crate::alexander_ai_solutions::wallet::tinyplace_solana_rpc_endpoints();
    ...
    for rpc_url in &endpoints {
        ...
        let safe_url = crate::alexander_ai_solutions::wallet::redact_rpc_url(rpc_url);
...
pub(crate) fn handle_tinyplace_signal_provision(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let store =
            crate::alexander_ai_solutions::tinyplace::signal_store::global_signal_store().await?;
        let client = global_state().client().await?;
...
pub(crate) fn handle_tinyplace_signal_send_message(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let store =
            crate::alexander_ai_solutions::tinyplace::signal_store::global_signal_store_arc()
                .await?;
        let client = global_state().client().await?;
        let signer = require_signer(client)?;
...
mod tests {
    ...
    async fn wallet_usdc_balance_reads_from_tinyplace_settlement_rpc() {
        let _lock = crate::alexander_ai_solutions::config::TEST_ENV_LOCK
            .lock()
