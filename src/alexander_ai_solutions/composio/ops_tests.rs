use super::*;
use crate::alexander_ai_solutions::context::prompt::IntegrationConnection;
...
fn cache_guard() -> std::sync::MutexGuard<'static, ()> {
    crate::alexander_ai_solutions::composio::connected_integrations::composio_cache_test_lock()
}
...
use crate::alexander_ai_solutions::memory_store::chunks::store as memory_tree_store;
...
fn config_with_backend(tmp: &tempfile::TempDir, base: String) -> Config {
    ...
    c.config_path = tmp.path().join("config.toml");
    c.api_url = Some(base);
    crate::alexander_ai_solutions::credentials::AuthService::from_config(&c)
        .store_provider_token(
            crate::alexander_ai_solutions::credentials::APP_SESSION_PROVIDER,
            crate::alexander_ai_solutions::credentials::DEFAULT_AUTH_PROFILE_NAME,
            "test-token",
...
async fn composio_delete_connection_clear_memory_cascades_source_tree_and_content_file() {
    ...
    use crate::alexander_ai_solutions::memory_store::trees::store as tree_store;
    use crate::alexander_ai_solutions::memory_store::trees::types::{SummaryNode, TreeKind};
    use rusqlite::params;
...
async fn composio_delete_connection_clear_memory_cascades_live_sealed_tree_and_file() {
    ...
    use crate::alexander_ai_solutions::memory_store::content::stage_chunks;
    use crate::alexander_ai_solutions::memory_store::trees::store as tree_store;
    use crate::alexander_ai_solutions::memory_store::trees::types::{Buffer, TreeKind};
    use crate::alexander_ai_solutions::memory_tree::tree::bucket_seal::{
        seal_one_level, LabelStrategy,
    };
...
async fn composio_get_user_profile_via_mock_returns_provider_profile() {
    use crate::alexander_ai_solutions::config::TEST_ENV_LOCK;
    let _cache_guard = cache_guard();
    let _env_guard = TEST_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());

    crate::alexander_ai_solutions::composio::providers::init_default_providers();
...
async fn composio_sync_gmail_via_mock_archives_raw_email_and_updates_outcome() {
    let _serial = crate::alexander_ai_solutions::memory::ops::GLOBAL_MEMORY_TEST_LOCK
        .lock()
        .await;
    use crate::alexander_ai_solutions::config::TEST_ENV_LOCK;
    use crate::alexander_ai_solutions::memory_store::content::raw::{raw_rel_path, RawKind};
    use crate::alexander_ai_solutions::memory_tree::tree::rpc::{
        list_chunks_rpc, ListChunksRequest,
    };
    let _cache_guard = cache_guard();
    let _env_guard = TEST_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    ...
    config.save().await.unwrap();
    let _ =
        crate::alexander_ai_solutions::memory::global::init(config.workspace_dir.clone()).unwrap();
...
fn direct_mode_config(tmp: &tempfile::TempDir) -> Config {
    ...
    c.workspace_dir = tmp.path().join("workspace");
    c.config_path = tmp.path().join("config.toml");
    c.composio.mode = crate::alexander_ai_solutions::config::schema::COMPOSIO_MODE_DIRECT.into();
    crate::alexander_ai_solutions::credentials::AuthService::from_config(&c)
        .store_provider_token(
            crate::alexander_ai_solutions::credentials::ops::COMPOSIO_DIRECT_PROVIDER,
            crate::alexander_ai_solutions::credentials::DEFAULT_AUTH_PROFILE_NAME,
            "ck_test_direct_key",
            std::collections::HashMap::new(),
            true,
...
async fn init_memory_client(workspace: &std::path::Path) -> tokio::sync::MutexGuard<'static, ()> {
    let guard = crate::alexander_ai_solutions::memory::ops::GLOBAL_MEMORY_TEST_LOCK
        .lock()
        .await;
    crate::alexander_ai_solutions::memory::global::init(workspace.to_path_buf())
...
async fn enrich_populates_email_from_cached_profile() {
    use crate::alexander_ai_solutions::memory_sync::composio::providers::{
