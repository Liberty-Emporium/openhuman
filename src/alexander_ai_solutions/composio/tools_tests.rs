use super::*;
use crate::alexander_ai_solutions::composio::providers::tool_scope::{CuratedTool, ToolScope};
use crate::alexander_ai_solutions::composio::providers::{
...
fn fake_config_arc() -> Arc<crate::alexander_ai_solutions::config::Config> {
...
async fn authorize_tool_execute_rejects_missing_toolkit() {
    ...
    let txt = result
        ...
        .filter_map(|c| match c {
            crate::alexander_ai_solutions::tools::traits::ToolContent::Text { text } => {
                Some(text.clone())
            }
            _ => None,
        })
...
fn tool_result_text(result: &crate::alexander_ai_solutions::tools::traits::ToolResult) -> String {
    result
        .content
        .iter()
        .filter_map(|c| match c {
...
async fn connect_tool_validates_before_gating_in_chat_context() {
    use crate::alexander_ai_solutions::approval::{ApprovalChatContext, APPROVAL_CHAT_CONTEXT};
...
async fn connection_is_active_errs_without_a_client() {
    ...
    let cfg = crate::alexander_ai_solutions::config::Config::default();
...
fn agent_tools_register_when_backend_signed_in() {
    ...
    let mut config = crate::alexander_ai_solutions::config::Config::default();
    config.config_path = tmp.path().join("config.toml");
    crate::alexander_ai_solutions::credentials::AuthService::from_config(&config)
...
async fn sandbox_read_only_blocks_write_scope_action() {
    let t = ComposioExecuteTool::new(fake_config_arc());
    let result = crate::alexander_ai_solutions::agent::harness::with_current_sandbox_mode(
...
fn render_tools_markdown_groups_by_toolkit_and_drops_schemas() {
    use crate::alexander_ai_solutions::composio::types::{
...
fn render_tools_markdown_handles_empty_response() {
    use crate::alexander_ai_solutions::composio::types::ComposioToolsResponse;
...
fn direct_mode_config() -> crate::alexander_ai_solutions::config::Config {
...
fn execute_tool_resolves_to_direct_kind_when_mode_is_direct() {
    ...
    let kind = crate::alexander_ai_solutions::composio::client::create_composio_client(&config)
        .expect("direct mode with inline api_key must resolve");
    assert_eq!(
        kind.mode(),
        crate::alexander_ai_solutions::config::schema::COMPOSIO_MODE_DIRECT,
...
fn execute_tool_resolves_to_backend_kind_when_mode_is_backend() {
    ...
    assert_eq!(
        kind.mode(),
        crate::alexander_ai_solutions::config::schema::COMPOSIO_MODE_BACKEND,
...
async fn execute_tool_per_call_factory_means_no_baked_client() {
    ...
    // per call which reads from `~/.alexanderai/config.toml` (or the
