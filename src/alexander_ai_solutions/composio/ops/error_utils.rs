...
pub(crate) fn direct_mode_without_key(config: &Config) -> OpResult<bool> {
    if config.composio.mode.trim()
        != crate::alexander_ai_solutions::config::schema::COMPOSIO_MODE_DIRECT
    {
        return Ok(false);
    }
    let has_key = crate::alexander_ai_solutions::credentials::get_composio_api_key(config)
...
///
/// The shared [`crate::alexander_ai_solutions::integrations::IntegrationClient`]
