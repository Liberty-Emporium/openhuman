...
pub async fn composio_get_mode(config: &Config) -> OpResult<RpcOutcome<serde_json::Value>> {
    ...
    let key_present = crate::alexander_ai_solutions::credentials::get_composio_api_key(config)
...
pub async fn composio_set_api_key(
    ...
    crate::alexander_ai_solutions::credentials::store_composio_api_key(config, trimmed)
        .await
        .map_err(|e| format!("[composio-direct] store_composio_api_key failed: {e}"))?;

    let mode_log = if activate_direct {
        let mut cfg_mut = crate::alexander_ai_solutions::config::rpc::load_config_with_timeout()
            .await
            .map_err(|e| format!("[composio-direct] reload config failed: {e}"))?;
        cfg_mut.composio.mode =
            crate::alexander_ai_solutions::config::schema::COMPOSIO_MODE_DIRECT.into();
        cfg_mut
            .save()
            .await
...
pub async fn composio_clear_api_key(config: &Config) -> OpResult<RpcOutcome<serde_json::Value>> {
    ...
    crate::alexander_ai_solutions::credentials::clear_composio_api_key(config)
        .await
    ...
    let mut cfg_mut = crate::alexander_ai_solutions::config::rpc::load_config_with_timeout()
        ...
        .map_err(|e| format!("[composio-direct] reload config failed: {e}"))?;
    cfg_mut.composio.mode =
        crate::alexander_ai_solutions::config::schema::COMPOSIO_MODE_BACKEND.into();
    cfg_mut
        .save()
        .await
        .map_err(|e| format!("[composio-direct] save config failed: {e}"))?;
