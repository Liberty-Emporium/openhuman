...
//! `Tool` trait plumbing in [`crate::alexander_ai_solutions::tools`].
//!
...
use crate::alexander_ai_solutions::agent::harness::current_sandbox_mode;
use crate::alexander_ai_solutions::agent::harness::current_task_recency_window;
...
pub struct ComposioListToolkitsTool {
    ...
    /// [`crate::alexander_ai_solutions::config::ComposioConfig::mode`] toggle is
...
impl Tool for ComposioConnectTool {
    ...
    async fn execute(&self, args: Value) -> anyhow::Result<ToolResult> {
        ...
        if crate::alexander_ai_solutions::approval::APPROVAL_CHAT_CONTEXT
            .try_with(|_| ())
            .is_err()
        {
            return Ok(ToolResult::error(format!(
        ...
        let gate = match crate::alexander_ai_solutions::approval::ApprovalGate::try_global() {
            Some(g) => g,
            None => {
                return Ok(ToolResult::error(
...
pub struct ComposioListToolsTool {
    ...
    /// [`crate::alexander_ai_solutions::composio::ops::composio_execute`] and avoids
...
pub struct ComposioExecuteTool {
    ...
    /// [`crate::alexander_ai_solutions::composio::ops::composio_execute`]. See
    /// issue #1710.
    config: Arc<Config>,
}
...
pub fn all_composio_agent_tools(
    ...
    if !crate::alexander_ai_solutions::agent::harness::subagent_runner::user_is_signed_in_to_composio(config) {
        tracing::debug!(
