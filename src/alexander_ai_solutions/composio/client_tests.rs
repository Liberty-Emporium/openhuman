...
async fn authorize_rejects_empty_toolkit() {
    let inner = Arc::new(
        crate::alexander_ai_solutions::integrations::IntegrationClient::new(
...
fn config_with_session_token(
    tmp: &tempfile::TempDir,
) -> crate::alexander_ai_solutions::config::Config {
...
fn store_get_clear_composio_api_key_roundtrip() {
    use crate::alexander_ai_solutions::credentials::{
        get_composio_api_key, COMPOSIO_DIRECT_PROVIDER,
    };
...
fn direct_tool_for_test() -> std::sync::Arc<crate::alexander_ai_solutions::tools::ComposioTool> {
    std::sync::Arc::new(crate::alexander_ai_solutions::tools::ComposioTool::new(
        "ck_test_direct",
        Some("default"),
        std::sync::Arc::new(crate::alexander_ai_solutions::security::SecurityPolicy::default()),
    ))
}
...
fn direct_tool_for_mock(
    base_v3: String,
) -> std::sync::Arc<crate::alexander_ai_solutions::tools::ComposioTool> {
    std::sync::Arc::new(
        crate::alexander_ai_solutions::tools::ComposioTool::new_with_v3_base(
            "ck_test_direct",
            Some("default"),
            std::sync::Arc::new(crate::alexander_ai_solutions::security::SecurityPolicy::default()),
            base_v3,
...
async fn pricing_for_config_short_circuits_in_direct_mode() {
    ...
    let client = crate::alexander_ai_solutions::integrations::IntegrationClient::new(
        "http://127.0.0.1:0".into(),
        "test".into(),
    );
    let mut config = crate::alexander_ai_solutions::config::Config::default();
    ...
    let pricing =
        crate::alexander_ai_solutions::integrations::pricing_for_config(&client, &config).await;
