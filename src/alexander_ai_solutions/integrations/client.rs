...
pub(crate) fn extract_error_detail(body: &str, max_bytes: usize) -> String {
    ...
    if let Ok(v) = serde_json::from_str::<serde_json::Value>(body) {
        if let Some(msg) = v.get("error").and_then(|e| e.as_str()) {
            ...
            if !trimmed.is_empty() {
                return crate::alexander_ai_solutions::util::truncate_at_byte_boundary(
                    trimmed, max_bytes,
                );
            }
        }
    }
    crate::alexander_ai_solutions::util::truncate_at_byte_boundary(body, max_bytes)
}
...
/// `alexander_ai_solutions::socket::ws_loop`. A *third-party* integration's auth failure
...
fn handle_session_jwt_unauthorized(method: &str, path: &str, url: &str, detail: &str) -> String {
    ...
    crate::core::event_bus::publish_global(crate::core::event_bus::DomainEvent::SessionExpired {
        ...
        reason: crate::alexander_ai_solutions::inference::provider::ops::sanitize_api_error(
            &message,
        ),
    });
...
pub struct IntegrationClient {
    ...
    budget_config: Option<Arc<crate::alexander_ai_solutions::config::Config>>,
...
impl IntegrationClient {
    ...
    pub fn new_with_budget_config(
        backend_url: String,
        auth_token: String,
        config: Arc<crate::alexander_ai_solutions::config::Config>,
    ) -> Self {
    ...
    async fn ensure_budget_available(&self, path: &str) -> anyhow::Result<()> {
        ...
        if let Some(config) = &self.budget_config {
            if crate::alexander_ai_solutions::team::managed_tool_budget_exhausted(config).await {
                anyhow::bail!(
