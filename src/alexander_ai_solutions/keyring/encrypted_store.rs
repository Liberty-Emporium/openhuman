...
impl SecretStore {
    ...
    fn load_or_create_key_from_keyring(&self) -> Result<Zeroizing<Vec<u8>>> {
        ...
        match crate::alexander_ai_solutions::keyring::migrate_from_file(
            &user_id,
            KEYCHAIN_MASTER_KEY,
            &self.key_path,
        ) {
            Ok(crate::alexander_ai_solutions::keyring::MigrationOutcome::MigratedAndDeleted) => {
                log::info!(
                    "[security] migrated legacy secret-store key into keychain from {}",
                    self.key_path.display()
                );
            }
            Ok(crate::alexander_ai_solutions::keyring::MigrationOutcome::AlreadyMigrated)
            | Ok(crate::alexander_ai_solutions::keyring::MigrationOutcome::NoSourceFile) => {}
            Err(error) => {
                log::warn!(
        ...
        let hex_key = crate::alexander_ai_solutions::keyring::get_or_create_random(
            &user_id,
            KEYCHAIN_MASTER_KEY,
            KEY_LEN,
        )
                ...
                #[cfg(windows)]
                {
                    msg.push_str(
                        "\n\nThis is often caused by incorrect file permissions on Windows. \
                         Try repairing ACLs on the .alexanderai directory:\n\
                         icacls \"%USERPROFILE%\\.openhuman\" /reset /t /c\n\
                         icacls \"%USERPROFILE%\\.alexanderai\\.secret_key\" /reset /c",
                    );
                }
                msg
            })?;
