...
use crate::alexander_ai_solutions::keyring::backend::KeyringBackend;
use crate::alexander_ai_solutions::keyring::crypto::{self, KEY_LEN};
use crate::alexander_ai_solutions::keyring::error::KeyringError;
use crate::alexander_ai_solutions::keyring::store::BackendKind;
...
pub fn init_master_key() {
    ...
    let dir = crate::alexander_ai_solutions::keyring::store::workspace_dir_for_file_backend();
    log::info!(
        "[keyring] init_master_key: resolved workspace_dir={}",
        dir.display()
    );
    crate::alexander_ai_solutions::keyring::init_workspace(&dir);

    MASTER_KEY.get_or_init(|| {
        let backend_kind = crate::alexander_ai_solutions::keyring::store::effective_backend_kind();
        ...
        match try_load_master_key() {
            ...
            Err(e) => {
                ...
                crate::alexander_ai_solutions::keyring_consent::policy::notify_master_key_unavailable(&e);
                None
            }
        }
    });
