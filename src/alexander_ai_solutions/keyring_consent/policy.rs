...
pub fn check_secret_access() -> PolicyDecision {
    if crate::alexander_ai_solutions::keyring::is_available() {
...
pub fn current_status() -> KeyringStatus {
    let available = crate::alexander_ai_solutions::keyring::is_available();
    let backend_name = crate::alexander_ai_solutions::keyring::backend_name();
...
pub fn retry_probe() -> KeyringStatus {
    ...
    crate::alexander_ai_solutions::keyring::reset_availability_cache();
    CONSENT_EVENT_PUBLISHED.store(false, Ordering::SeqCst);
    current_status()
}
