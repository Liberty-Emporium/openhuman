...
/// [`config_set_action_dir`](crate::alexander_ai_solutions::config::ops::set_action_dir)
...
pub fn reload_from(autonomy_config: &crate::alexander_ai_solutions::config::AutonomyConfig) {
    let Some(state) = STATE.get() else {
        return;
    };
    let workspace = state
...
mod tests {
    use super::*;
    use crate::alexander_ai_solutions::config::AutonomyConfig;
