...
use crate::alexander_ai_solutions::memory_queue::{self as jobs, ExtractChunkPayload, NewJob};
use crate::alexander_ai_solutions::memory_store::chunks::store::{self as chunk_store, RawRef};
use crate::alexander_ai_solutions::memory_store::chunks::types::SourceKind;
use crate::alexander_ai_solutions::memory_store::chunks::{
    chunk_markdown, ChunkerInput, ChunkerOptions,
};
...
use crate::alexander_ai_solutions::memory_tree::score::{self, ScoreResult, ScoringConfig};
            ...
            match std::panic::catch_unwind(move || markdown_body_preview(&md_for_preview)) {
                ...
                Err(_) => {
                    log::error!(
                        ...
                        crate::alexander_ai_solutions::memory::util::redact::redact(source_id)
                    );
                    None
                }
            }
...
fn markdown_body_preview(md: &str) -> String {
    ...
    if len <= BODY_PREVIEW_MAX_BYTES {
        ...
        let start = crate::alexander_ai_solutions::util::ceil_char_boundary(
            md,
            len - BODY_PREVIEW_MAX_BYTES,
        );
        debug_assert!(
...
mod tests {
    ...
    use crate::alexander_ai_solutions::memory_store::chunks::types::SourceKind;
    use crate::alexander_ai_solutions::memory_sync::canonicalize::chat::ChatMessage;
    use crate::alexander_ai_solutions::memory_tree::score::store::{count_scores, lookup_entity};
