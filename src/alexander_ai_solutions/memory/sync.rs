...
//! 3. dispatch into [`crate::alexander_ai_solutions::memory_sync`] backends
...
pub fn register_sync_stage_bridge(config: &crate::alexander_ai_solutions::config::Config) {
    ...
    async fn handle(&self, event: &DomainEvent) {
        if let DomainEvent::MemorySyncStageChanged { stage, .. } = event {
            if stage == "completed" {
                ...
                crate::alexander_ai_solutions::memory_queue::ensure_reembed_backfill(&self.config);
            }
        }
    }
}
