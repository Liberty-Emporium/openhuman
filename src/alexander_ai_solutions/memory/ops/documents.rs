...
use crate::alexander_ai_solutions::memory_store::{
    NamespaceDocumentInput, NamespaceRetrievalContext,
};
...
mod tests {
    ...
    /// duration, holding [`crate::alexander_ai_solutions::config::TEST_ENV_LOCK`] so sibling
    ...
    impl WorkspaceEnvGuard {
        fn pin(workspace: &std::path::Path) -> Self {
            let env_lock = crate::alexander_ai_solutions::config::TEST_ENV_LOCK
                .lock()
    ...
    fn ensure_memory_client() -> WorkspaceEnvGuard {
        let workspace = crate::alexander_ai_solutions::memory::ops::ensure_shared_memory_client();
