...
use crate::alexander_ai_solutions::memory_store::{MemoryClient, MemoryClientRef};
...
/// Initialise using the default `~/.alexanderai/workspace` directory.
///
...
/// would pin the singleton to `~/.alexanderai/workspace`, causing every
...
/// fallback: a fallback would pin the global to `~/.alexanderai/workspace` on
