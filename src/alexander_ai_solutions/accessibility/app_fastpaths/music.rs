...
pub async fn run(goal: &str, backend: &dyn AutomateBackend) -> AutomateOutcome {
    ...
    use crate::alexander_ai_solutions::overlay::OverlayAttentionTone;
    progress(
...
mod live {
    use super::run;
    use crate::alexander_ai_solutions::accessibility::automate::RealBackend;

    #[tokio::test]
    #[ignore = "requires macOS + Music app + Accessibility permission"]
    async fn music_fastpath_live() {
        let backend = RealBackend::new(crate::alexander_ai_solutions::config::Config::default());
