...
//!
//! The cache lives at `~/.alexanderai/skill-registry/cache.json` with a 1-hour
...
fn registry_dir() -> Option<PathBuf> {
    ...
    dirs::home_dir().map(|h| h.join(".alexanderai").join(CACHE_DIR))
}
...
mod tests {
    ...
    fn within_ttl_is_fresh_past_ttl_is_stale() {
        let _guard = crate::alexander_ai_solutions::skill_registry::TEST_ENV_LOCK
            .lock()
