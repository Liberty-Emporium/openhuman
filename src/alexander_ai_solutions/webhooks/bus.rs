...
use crate::alexander_ai_solutions::webhooks::WebhookResponseData;
    ...
    async fn handle(&self, event: &DomainEvent) {
        ...
        let (response, resolved_skill_id, response_error) = match registration {
            Some(ref reg) if reg.target_kind == "echo" => {
                ...
                let resp =
                    crate::alexander_ai_solutions::webhooks::ops::build_echo_response(request);
...
async fn run_agent_trigger(
    envelope: &crate::alexander_ai_solutions::agent::triage::TriggerEnvelope,
    ...
    match outcome {
        crate::alexander_ai_solutions::agent::triage::TriageOutcome::Decision(run) => {
            crate::alexander_ai_solutions::agent::triage::apply_decision(run.clone(), envelope)
                .await
...
mod tests {
    use super::*;
    use crate::alexander_ai_solutions::webhooks::WebhookRequest;
    use base64::Engine;
    use std::collections::HashMap;
