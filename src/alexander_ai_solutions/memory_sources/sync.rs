...
use crate::alexander_ai_solutions::memory::ingest_pipeline::ingest_document_with_scope;
...
use crate::alexander_ai_solutions::memory_sync::composio::{self, ComposioUsage, SyncReason};
...
pub async fn sync_source(source: MemorySourceEntry, config: Config) -> Result<(), String> {
    ...
    tokio::spawn(async move {
        ...
        let inner = tokio::spawn(async move {
            ...
            if let Ok(retried) =
                crate::alexander_ai_solutions::memory_queue::store::retry_all_failed(&config)
            {
                if retried > 0 {
                    tracing::info!(
                        retried = retried,
            ...
            let outcome = match source.kind {
                ...
                SourceKind::GithubRepo => {
                    ...
                    // audit for this kind.
                    crate::alexander_ai_solutions::memory_sync::sources::github::run_github_sync(
                        &source, &config,
                    )
                    .await
            ...
            match outcome {
                Ok(items) => {
                    ...
                    if let Err(e) =
                        crate::alexander_ai_solutions::memory_diff::ops::auto_snapshot_after_sync(
                            &source, &config,
                        )
                        .await
                    {
