    ...
    if let Some(batch) = plan_delivery(&session) {
        ...
        match (
            ...
            (Some(thread_id), Some(notice)) => {
                ...
                if let Err(e) = crate::alexander_ai_solutions::agent::task_dispatcher::run_system_turn_on_thread(
                    thread_id, notice,
                )
                .await
                {
...
mod tests {
    use super::*;
    use crate::alexander_ai_solutions::agent_orchestration::background_completions::record_completion;
