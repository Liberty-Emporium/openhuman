...
use crate::alexander_ai_solutions::inference::provider::thread_context::current_thread_id;
...
///    [`crate::alexander_ai_solutions::config::ContextConfig::super_context_enabled`]),
///    so the collection happens regardless of the model's decision.
///
/// Must be called from within an active agent turn (i.e. with the
/// [`crate::alexander_ai_solutions::agent::harness::fork_context::PARENT_CONTEXT`]
            ...
            SubagentRunStatus::Completed => {
                ...
                if let (Some(parent), Some(thread_id)) = (current_parent(), current_thread_id()) {
                    if let Some(objective) = AgentPrepareContextTool::parse_proposed_goal(&bundle) {
                        match crate::alexander_ai_solutions::thread_goals::store::set_if_absent(
...
mod tests {
    ...
    async fn execute_short_circuits_when_context_already_prepared() {
        let tool = AgentPrepareContextTool::new();
        let result =
            crate::alexander_ai_solutions::agent::harness::with_agent_context_prepared_sources(
