...
async fn fetch_live_connected_toolkit_slugs_once() -> Option<Vec<String>> {
    ...
    let config = crate::alexander_ai_solutions::config::Config::load_or_init()
        .await
        .ok()?;
    match crate::alexander_ai_solutions::composio::fetch_connected_integrations_status(&config).await {
        crate::alexander_ai_solutions::composio::FetchConnectedIntegrationsStatus::Authoritative(entries) => {
        ...
        crate::alexander_ai_solutions::composio::FetchConnectedIntegrationsStatus::Unavailable => None,
    }
}
