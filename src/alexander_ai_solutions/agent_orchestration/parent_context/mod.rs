...
pub(crate) async fn build_root_parent(
    ...
    if crate::alexander_ai_solutions::agent::harness::AgentDefinitionRegistry::global().is_none() {
        if let Err(err) =
            ...
            )
        {
            ...
            if crate::alexander_ai_solutions::agent::harness::AgentDefinitionRegistry::global()
                .is_none()
            {
                return Err(err)
