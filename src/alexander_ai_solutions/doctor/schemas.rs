...
fn handle_report(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::alexander_ai_solutions::doctor::rpc::doctor_report(&config).await?)
    })
}
...
fn handle_models(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::alexander_ai_solutions::doctor::rpc::doctor_models(&config, use_cache).await?,
        )
    })
}
