...
fn doctor_model_targets() -> Vec<String> {
    crate::alexander_ai_solutions::inference::provider::list_providers()
        .into_iter()
...
fn check_daemon_state(config: &Config, items: &mut Vec<DiagnosticItem>) {
    let cat = "daemon";
    let state_file = crate::alexander_ai_solutions::service::daemon::state_file_path(config);
...
fn check_memory_tree_db(config: &Config, items: &mut Vec<DiagnosticItem>) {
    ...
    // ── Probe connection ─────────────────────────────────────────────
    match crate::alexander_ai_solutions::memory_store::chunks::store::with_connection(
        config,
        |conn| {
            let n: i64 =
...
fn check_embedding_model_health(config: &Config, items: &mut Vec<DiagnosticItem>) {
    ...
    let (provider, model, _dims) =
        crate::alexander_ai_solutions::memory_store::factories::effective_embedding_settings(
            &config.memory,
    ...
    let base_url = crate::alexander_ai_solutions::inference::local::ollama_base_url();
