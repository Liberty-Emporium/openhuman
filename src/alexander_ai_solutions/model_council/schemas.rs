...
struct ModelCouncilSynthesizeParams {
    question: String,
    members: Vec<crate::alexander_ai_solutions::model_council::council::CouncilMemberResult>,
    chair_model: String,
    temperature: Option<f64>,
}
...
fn handle_run(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::alexander_ai_solutions::model_council::council::run_council(
                &config,
                &p.question,
                &p.member_models,
                &p.chair_model,
...
fn handle_answer_member(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::alexander_ai_solutions::model_council::council::answer_member(
                &config,
                &p.question,
                &p.model,
                p.temperature,
...
fn handle_synthesize(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::alexander_ai_solutions::model_council::council::synthesize_members(
                &config,
                &p.question,
                p.members,
                &p.chair_model,
