...
async fn stt(samples: &[i16], sample_rate: u32) -> Result<String, String> {
    use crate::alexander_ai_solutions::voice::cloud_transcribe::{
        transcribe_cloud, CloudTranscribeOptions,
    };

    let config = crate::alexander_ai_solutions::config::ops::load_config_with_timeout().await?;
    let wav_bytes =
        crate::alexander_ai_solutions::meet_agent::wav::pack_pcm16le_mono_wav(samples, sample_rate);
...
async fn tts(text: &str) -> Result<Vec<i16>, String> {
    use crate::alexander_ai_solutions::voice::reply_speech::{
        synthesize_reply, ReplySpeechOptions,
    };
...
async fn gather_screen_context() -> Option<String> {
    #[cfg(target_os = "macos")]
    {
        let context = crate::alexander_ai_solutions::accessibility::foreground_context();
        context.map(|ctx| {
            format!(
                "App: {} | Window: {}",
