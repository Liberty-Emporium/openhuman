...
use crate::alexander_ai_solutions::autocomplete::AutocompleteStatus;
use crate::alexander_ai_solutions::config::rpc as config_rpc;
use crate::alexander_ai_solutions::config::Config;
use crate::alexander_ai_solutions::credentials::session_support::{
    is_local_session_token, load_app_session_profile, session_state_from_profile,
    session_token_from_profile,
};
use crate::alexander_ai_solutions::credentials::{
    AuthService, APP_SESSION_PROVIDER, DEFAULT_AUTH_PROFILE_NAME,
};
use crate::alexander_ai_solutions::inference::LocalAiStatus;
use crate::alexander_ai_solutions::screen_intelligence::AccessibilityStatus;
use crate::alexander_ai_solutions::service::{ServiceState, ServiceStatus};
...
pub struct StoredAppState {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub keyring_consent: Option<crate::alexander_ai_solutions::keyring_consent::ConsentPreference>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AppStateSnapshot {
    pub auth: crate::alexander_ai_solutions::credentials::responses::AuthStateResponse,
    ...
    pub keyring_status: crate::alexander_ai_solutions::keyring_consent::KeyringStatus,
...
pub struct StoredAppStatePatch {
    ...
    #[serde(default)]
    pub keyring_consent:
        Option<Option<crate::alexander_ai_solutions::keyring_consent::ConsentPreference>>,
}
...
fn build_client() -> Result<Client, String> {
    // Platform-appropriate TLS backend — see [`crate::alexander_ai_solutions::tls`].
...
fn config_dir_for_workspace_env() -> Option<PathBuf> {
    ...
    if let Some(parent) = workspace_dir.parent() {
        let legacy_dir = parent.join(".alexanderai");
...
async fn activate_revalidated_user_dir(user_id: &str) -> Result<Config, String> {
    let root_dir = crate::alexander_ai_solutions::config::default_root_openhuman_dir()
        .map_err(|error| format!("failed to locate default root: {error}"))?;
    let previous_active = crate::alexander_ai_solutions::config::read_active_user_id(&root_dir);
    let user_dir = crate::alexander_ai_solutions::config::user_openhuman_dir(&root_dir, user_id);
    ...
    crate::alexander_ai_solutions::config::write_active_user_id(&root_dir, user_id).map_err(|error| {
    ...
    if previous_active.is_none() {
        let pre_ws =
            crate::alexander_ai_solutions::config::pre_login_user_dir(&root_dir).join("workspace");
        if let Err(error) =
            crate::alexander_ai_solutions::memory_conversations::purge_threads(pre_ws)
        {
            debug!(
...
async fn finish_revalidated_user_activation(
    ...
    if let Err(error) =
        crate::alexander_ai_solutions::memory::global::init(target_config.workspace_dir.clone())
    {
        warn!(
    ...
    if let Err(error) =
        crate::alexander_ai_solutions::subconscious::global::bootstrap_after_login().await
    {
    ...
    if let Some(source_config) = service_rebind_source {
        crate::alexander_ai_solutions::credentials::stop_login_gated_services(source_config).await;
        crate::alexander_ai_solutions::credentials::start_login_gated_services(target_config).await;
    } else {
        debug!(
    ...
    crate::alexander_ai_solutions::credentials::sentry_scope::bind(user_id);
}
...
async fn clear_deferred_session_after_backend_rejection(
    ...
    *CURRENT_USER_CACHE.lock() = None;
    ...
    match crate::alexander_ai_solutions::config::default_root_openhuman_dir() {
        Ok(root_dir) => {
            let active_user = crate::alexander_ai_solutions::config::read_active_user_id(&root_dir);
            ...
            if should_clear_active_user {
                if let Err(error) =
                    crate::alexander_ai_solutions::config::clear_active_user(&root_dir)
                {
                    warn!(
    ...
    crate::alexander_ai_solutions::credentials::stop_login_gated_services(config).await;
    crate::alexander_ai_solutions::subconscious::global::reset_engine_for_user_switch().await;
    crate::alexander_ai_solutions::credentials::sentry_scope::clear();
...
pub fn peek_cached_current_user_identity(
) -> Option<crate::alexander_ai_solutions::agent::prompts::UserIdentity> {
    let cache = CURRENT_USER_CACHE.lock();
    ...
    let identity = crate::alexander_ai_solutions::agent::prompts::UserIdentity { id, name, email };
    ...
    let (screen_intelligence, local_ai, autocomplete, service) = tokio::join!(
        async {
            ...
            let status = match tokio::time::timeout(SNAPSHOT_SUB_OP_TIMEOUT, async {
                let _ = crate::alexander_ai_solutions::screen_intelligence::global_engine()
                    .apply_config(si_config)
                    .await;
                crate::alexander_ai_solutions::screen_intelligence::global_engine()
                    .status()
                    .await
            })
            .await
        ...
        async {
            ...
            let status = match tokio::time::timeout(
                ...
                crate::alexander_ai_solutions::inference::rpc::inference_status(
                    &config_for_local_ai,
                ),
            )
            .await
            {
                ...
                Ok(Err(error)) => {
                    ...
                    crate::alexander_ai_solutions::inference::LocalAiStatus::disabled(
                        &config_for_local_ai,
                    )
                }
                Err(_) => {
        ...
        async {
            ...
            let status = match tokio::time::timeout(
                SNAPSHOT_SUB_OP_TIMEOUT,
                crate::alexander_ai_solutions::autocomplete::global_engine()
        ...
        async {
            ...
            let status = tokio::task::spawn_blocking(move || {
                crate::alexander_ai_solutions::service::status(&config_for_service)
            })
            .await
...
pub async fn snapshot() -> Result<RpcOutcome<AppStateSnapshot>, String> {
    ...
    crate::alexander_ai_solutions::keyring_consent::policy::initialize(
    ...
    let keyring_status = crate::alexander_ai_solutions::keyring_consent::policy::current_status();
...
fn degraded_runtime_snapshot(config: &Config) -> RuntimeSnapshot {
    use crate::alexander_ai_solutions::screen_intelligence::{
    ...
    RuntimeSnapshot {
        ...
        local_ai: crate::alexander_ai_solutions::inference::LocalAiStatus::disabled(config),
        autocomplete: crate::alexander_ai_solutions::autocomplete::AutocompleteStatus {
            platform_supported: cfg!(target_os = "macos"),
