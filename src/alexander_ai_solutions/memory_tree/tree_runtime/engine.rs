...
use crate::alexander_ai_solutions::memory_tree::tree_runtime::store;
...
async fn summarize_to_limit(
    ...
    let response = if response.len() > char_limit {
        ...
        let truncated = &response
            [..crate::alexander_ai_solutions::util::floor_char_boundary(&response, char_limit)];
        truncated.to_string()
    } else {
        response
    };
    ...
    for entry in std::fs::read_dir(dir)? {
        ...
        } else if ft.is_file() && name.ends_with(".md") && name != "summary.md" && name != "root.md"
        {
            ...
            if level == NodeLevel::Hour {
                ...
                let node =
                    crate::alexander_ai_solutions::memory_tree::tree_runtime::store::parse_node_markdown_pub(
