...
pub use crate::alexander_ai_solutions::runtime_python_server::{
...
use crate::alexander_ai_solutions::memory_tree::score::resolver::{canonicalise, CanonicalEntity};
...
pub async fn extract_query_entities(config: &Config, query: &str) -> Vec<CanonicalEntity> {
    ...
    if config.memory_tree.spacy_enabled {
        match crate::alexander_ai_solutions::runtime_python_server::extract_spacy(config, trimmed)
            .await
        {
            Ok(resp) => {
...
mod tests {
    ...
    fn spacy_response_maps_nouns_to_topics() {
        let resp = SpacyResponse {
            entities: vec![
                crate::alexander_ai_solutions::runtime_python_server::spacy::SpacyEntity {
                    text: "Alice".into(),
                    label: "PERSON".into(),
                    start: 0,
                    end: 5,
