...
use crate::alexander_ai_solutions::memory::chat::{ChatPrompt, ChatProvider};
...
/// [`crate::alexander_ai_solutions::memory::chat::build_chat_provider`]). Tests
...
impl EntityExtractor for LlmEntityExtractor {
    ...
    async fn extract(&self, text: &str) -> anyhow::Result<ExtractedEntities> {
        ...
        for attempt in 0..MAX_ATTEMPTS {
            match self.try_extract(text).await {
                AttemptOutcome::Done(extracted) => {
                    ...
                    crate::alexander_ai_solutions::memory_tree::health::clear_structure_degraded();
                    return Ok(extracted);
                }
                AttemptOutcome::Permanent(code) => {
                    ...
                    crate::alexander_ai_solutions::memory_tree::health::mark_structure_degraded(
                        code,
                    );
        ...
        crate::alexander_ai_solutions::memory_tree::health::mark_structure_degraded(
            crate::alexander_ai_solutions::memory_tree::health::FailureCode::ExtractionTimeout,
        );
...
enum AttemptOutcome {
    ...
    Permanent(crate::alexander_ai_solutions::memory_tree::health::FailureCode),
}
    ...
    async fn try_extract(&self, text: &str) -> AttemptOutcome {
        ...
        let raw = match self.provider.chat_for_json(&prompt).await {
            ...
            Err(e) => {
                ...
                if crate::alexander_ai_solutions::inference::provider::reliable::is_non_retryable(
                    &e,
                ) {
                    log::warn!(
...
fn permanent_failure_code(
    err: &anyhow::Error,
) -> crate::alexander_ai_solutions::memory_tree::health::FailureCode {
    use crate::alexander_ai_solutions::memory_tree::health::FailureCode;
...
fn build_system_prompt(emit_topics: bool, output_language: Option<&str>) -> String {
    ...
    let language_directive =
        crate::alexander_ai_solutions::config::output_language_directive(output_language)
