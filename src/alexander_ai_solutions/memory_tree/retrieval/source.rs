...
use crate::alexander_ai_solutions::memory_store::trees::types::{SummaryNode, Tree, TreeKind};
use crate::alexander_ai_solutions::memory_tree::retrieval::types::{
    hit_from_summary, QueryResponse, RetrievalHit,
};
use crate::alexander_ai_solutions::memory_tree::score::embed::{
    build_embedder_from_config, cosine_similarity,
};
...
pub async fn query_source(
    ...
    let source_scope = crate::alexander_ai_solutions::memory::source_scope::current_source_scope();
...
mod tests {
    ...
    async fn seed_source(cfg: &Config, scope: &str, ts: DateTime<Utc>) {
        ...
        for seq in 0..2u32 {
            let c = Chunk {
                ...
                token_count:
                    crate::alexander_ai_solutions::memory_store::trees::types::INPUT_TOKEN_BUDGET
                        * 6
                        / 10,
                seq_in_source: seq,
                created_at: ts,
    ...
    async fn profile_source_scope_restricts_all_query_to_allowlisted_trees() {
        use crate::alexander_ai_solutions::memory::source_scope::with_source_scope;
        let (_tmp, cfg) = test_config();
        let ts = Utc::now();
        seed_source(&cfg, "slack:#eng", ts).await;
        seed_source(&cfg, "gmail:alice@example.com", ts).await;
    ...
    async fn query_reranks_by_cosine_similarity() {
        ...
        let phoenix_tree = src_store::get_tree_by_scope(
            &cfg,
            crate::alexander_ai_solutions::memory_store::trees::types::TreeKind::Source,
            "slack:#phoenix",
        )
        .unwrap()
        .unwrap();
        ...
        use crate::alexander_ai_solutions::memory_tree::score::embed::cosine_similarity;
    ...
    async fn sidecar_only_embeddings_hydrate_for_rerank() {
        ...
        use crate::alexander_ai_solutions::memory_tree::score::embed::{
            cosine_similarity, EMBEDDING_DIM,
        };
