...
use crate::alexander_ai_solutions::memory::ingest_pipeline::{
    ingest_chat as do_ingest_chat, ingest_document as do_ingest_document,
    ingest_email as do_ingest_email, IngestResult,
};
use crate::alexander_ai_solutions::memory_store::chunks::store::{
    self as chunk_store, ListChunksQuery,
};
use crate::alexander_ai_solutions::memory_store::chunks::types::{Chunk, SourceKind};
use crate::alexander_ai_solutions::memory_sync::canonicalize::{
...
pub async fn backfill_status_rpc(
    ...
    let in_progress =
        crate::alexander_ai_solutions::memory_queue::backfill_in_progress() || pending_jobs > 0;
    Ok(RpcOutcome::single_log(
...
pub struct PipelineStatusResponse {
    ...
    #[serde(default)]
    pub degraded: crate::alexander_ai_solutions::memory_tree::health::DegradedState,
    ...
    #[serde(default)]
    pub first_blocking_cause:
        Option<crate::alexander_ai_solutions::memory_tree::health::PipelineFailure>,
...
pub async fn pipeline_status_rpc(
    ...
    use crate::alexander_ai_solutions::config::SchedulerGateMode;
    use crate::alexander_ai_solutions::memory_queue::store as queue_store;
    use crate::alexander_ai_solutions::memory_queue::types::JobStatus;
    ...
    let degraded = crate::alexander_ai_solutions::memory_tree::health::current_degraded_state();
    ...
    let (latest_failure, extraction_coverage) = {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let coverage = crate::alexander_ai_solutions::memory_store::chunks::store::extraction_coverage(&cfg)
                .map_err(|e| {
                    log::warn!(
...
pub async fn doctor_rpc(
    config: &Config,
) -> Result<RpcOutcome<crate::alexander_ai_solutions::memory_tree::health::DoctorReport>, String> {
    // Offload the doctor's blocking SQLite reads off the async runtime thread.
    let report = crate::alexander_ai_solutions::memory_tree::health::async_run_doctor(config).await;
...
pub async fn retry_failed_rpc(config: &Config) -> Result<RpcOutcome<RetryFailedResponse>, String> {
    ...
    let requeued = tokio::task::spawn_blocking(move || {
        crate::alexander_ai_solutions::memory_queue::store::requeue_failed(&cfg)
    })
    .await
    ...
    crate::alexander_ai_solutions::memory_queue::wake_workers();
    Ok(RpcOutcome::single_log(
...
fn latest_failed_job_failure(
    config: &Config,
) -> Result<Option<crate::alexander_ai_solutions::memory_tree::health::PipelineFailure>, String> {
    use crate::alexander_ai_solutions::memory_tree::health::{
        FailureClass, FailureCode, PipelineFailure,
    };
...
fn derive_pipeline_status(
    is_paused: bool,
    mode: crate::alexander_ai_solutions::config::SchedulerGateMode,
    is_syncing: bool,
    failed: u64,
    failed_unrecoverable: u64,
    total_chunks: u64,
    degraded: &crate::alexander_ai_solutions::memory_tree::health::DegradedState,
...
pub async fn set_enabled_rpc(
    ...
    crate::alexander_ai_solutions::scheduler_gate::gate::update_config(
        config.scheduler_gate.clone(),
    );
...
mod tests {
    use super::*;
    use crate::alexander_ai_solutions::memory_queue as jobs;
    use crate::alexander_ai_solutions::memory_queue::store::count_total;
    use crate::alexander_ai_solutions::memory_store::chunks::types::SourceKind;
    use crate::alexander_ai_solutions::memory_sync::canonicalize::document::DocumentInput;
    ...
    fn derive_pipeline_status_precedence_matches_spec() {
        use crate::alexander_ai_solutions::config::SchedulerGateMode;
        use crate::alexander_ai_solutions::memory_tree::health::{
            DegradedState, FailureCode, PipelineFailure,
        };
    ...
    async fn pipeline_status_returns_idle_for_empty_store() {
        ...
        let _g = crate::alexander_ai_solutions::memory_tree::health::test_guard();
        let (_tmp, cfg) = test_config();
    ...
    /// host's real ~/.alexanderai directory.
    #[tokio::test]
