...
use crate::alexander_ai_solutions::memory::util::redact::redact;
use crate::alexander_ai_solutions::memory_store::chunks::store::with_connection;
use crate::alexander_ai_solutions::memory_store::content::{
    atomic::stage_summary_with_layout,
    paths::{slugify_source_id, SummaryDiskLayout},
    wiki_git::{SummaryCommitBatch, SummaryCommitEntry},
    SummaryComposeInput,
};
use crate::alexander_ai_solutions::memory_store::trees::types::{
    Buffer, SummaryNode, Tree, INPUT_TOKEN_BUDGET, OUTPUT_TOKEN_BUDGET, SUMMARY_FANOUT,
};
use crate::alexander_ai_solutions::memory_tree::score::embed::build_write_embedder;
use crate::alexander_ai_solutions::memory_tree::score::extract::EntityExtractor;
use crate::alexander_ai_solutions::memory_tree::score::resolver::canonicalise;
use crate::alexander_ai_solutions::memory_tree::summarise::{
    fallback_summary, summarise, SummaryContext, SummaryInput, SummaryOutput,
};
use crate::alexander_ai_solutions::memory_tree::tree::factory::TreeFactory;
use crate::alexander_ai_solutions::memory_tree::tree::registry::new_summary_id;
use crate::alexander_ai_solutions::memory_tree::tree::store;
...
/// by `flush_stale_buffers` (see [`crate::alexander_ai_solutions::memory_tree::
...
pub(crate) async fn seal_one_level(
            ...
            Some(embedder) => {
                let v = match embedder.embed(&embed_input).await {
                    ...
                    Err(e) => {
                        ...
                        let failure =
                            crate::alexander_ai_solutions::memory_tree::health::classify_embed_error(&e);
                ...
                crate::alexander_ai_solutions::memory_tree::score::embed::pack_checked(&v)
                    .context(format!(
                ...
                crate::alexander_ai_solutions::memory_tree::health::clear_semantic_recall_degraded(
                );
                Some(v)
            }
        }
    ...
    let child_basename_overrides: Option<Vec<Option<String>>> = if node.level == 1 {
        let overrides: Vec<Option<String>> = node
            ...
            .map(|chunk_id| {
                ...
                match crate::alexander_ai_solutions::memory_store::chunks::store::get_chunk_raw_refs(
                    config, chunk_id,
                ) {
    ...
    if let Err(err) =
        crate::alexander_ai_solutions::memory_store::content::obsidian::ensure_obsidian_defaults(
            &content_root,
        )
    {
        log::warn!(
    ...
    with_connection(config, move |conn| {
        ...
        store::insert_summary_tx(
            ...
            &node,
            Some(&staged),
            &crate::alexander_ai_solutions::memory_store::chunks::store::tree_active_signature(
                config,
            ),
        )?;
        ...
        crate::alexander_ai_solutions::memory_tree::score::store::index_summary_entity_ids_tx(
            &tx,
            &node.entities,
            &node.id,
            node.score,
        ...
        if enqueue_follow_ups {
            ...
            if should_seal(&parent) {
                use crate::alexander_ai_solutions::memory_queue::store::enqueue_tx as enqueue_job_tx;
                use crate::alexander_ai_solutions::memory_queue::types::{NewJob, SealPayload};
...
fn truncate_for_embed(text: &str, max_tokens: u32) -> String {
    let approx =
        crate::alexander_ai_solutions::memory_store::chunks::types::approx_token_count(text);
...
fn commit_summary_seal(config: &Config, batch: &SummaryCommitBatch) -> Result<()> {
    crate::alexander_ai_solutions::memory_store::content::wiki_git::commit_summaries(
...
fn hydrate_leaf_inputs(config: &Config, chunk_ids: &[String]) -> Result<Vec<SummaryInput>> {
    use crate::alexander_ai_solutions::memory_store::chunks::store::get_chunk;
    use crate::alexander_ai_solutions::memory_store::content::read as content_read;
    use crate::alexander_ai_solutions::memory_tree::score::store::{
        get_score, list_entity_ids_for_node,
    };
...
fn hydrate_summary_inputs(config: &Config, summary_ids: &[String]) -> Result<Vec<SummaryInput>> {
    use crate::alexander_ai_solutions::memory_store::content::read as content_read;
    use crate::alexander_ai_solutions::memory_store::trees::store::get_summaries_batch;
...
async fn seal_explicit_children(
        ...
        match build_write_embedder(config).context("build embedder during doc-subtree seal")? {
            ...
            Some(embedder) => {
                let v = embedder.embed(&embed_input).await.map_err(|e| {
                    let failure =
                        crate::alexander_ai_solutions::memory_tree::health::classify_embed_error(
                            &e,
                        );
    ...
    let signature =
        crate::alexander_ai_solutions::memory_store::chunks::store::tree_active_signature(config);
    with_connection(config, move |conn| {
        let tx = conn.unchecked_transaction()?;
