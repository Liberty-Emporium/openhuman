...
impl EventHandler for ChannelInboundSubscriber {
    ...
    async fn handle(&self, event: &DomainEvent) {
        ...
        let request_id = match crate::alexander_ai_solutions::channels::providers::web::start_chat(
            &client_id,
            &thread_id,
            message,
            None,
...
async fn build_channel_client() -> Option<(crate::api::rest::BackendOAuthClient, String)> {
    let config = match crate::alexander_ai_solutions::config::rpc::load_config_with_timeout().await
    {
        Ok(c) => c,
        Err(e) => {
