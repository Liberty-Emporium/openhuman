...
use crate::alexander_ai_solutions::agent::harness::build_tool_instructions_filtered;
use crate::alexander_ai_solutions::agent::host_runtime;
use crate::alexander_ai_solutions::channels::context::{
...
use crate::alexander_ai_solutions::channels::dingtalk::DingTalkChannel;
use crate::alexander_ai_solutions::channels::discord::DiscordChannel;
use crate::alexander_ai_solutions::channels::email_channel::EmailChannel;
use crate::alexander_ai_solutions::channels::imessage::IMessageChannel;
use crate::alexander_ai_solutions::channels::irc;
use crate::alexander_ai_solutions::channels::irc::IrcChannel;
use crate::alexander_ai_solutions::channels::lark::LarkChannel;
use crate::alexander_ai_solutions::channels::linq::LinqChannel;
#[cfg(feature = "channel-matrix")]
use crate::alexander_ai_solutions::channels::matrix::MatrixChannel;
use crate::alexander_ai_solutions::channels::mattermost::MattermostChannel;
use crate::alexander_ai_solutions::channels::qq::QQChannel;
use crate::alexander_ai_solutions::channels::signal::SignalChannel;
use crate::alexander_ai_solutions::channels::slack::SlackChannel;
use crate::alexander_ai_solutions::channels::telegram::TelegramChannel;
use crate::alexander_ai_solutions::channels::traits;
use crate::alexander_ai_solutions::channels::whatsapp::WhatsAppChannel;
#[cfg(feature = "whatsapp-web")]
use crate::alexander_ai_solutions::channels::whatsapp_web::WhatsAppWebChannel;
use crate::alexander_ai_solutions::channels::yuanbao::YuanbaoChannel;
use crate::alexander_ai_solutions::channels::Channel;
use crate::alexander_ai_solutions::config::Config;
use crate::alexander_ai_solutions::context::channels_prompt::build_system_prompt;
use crate::alexander_ai_solutions::inference::provider::{self, Provider};
use crate::alexander_ai_solutions::memory::Memory;
use crate::alexander_ai_solutions::memory_store;
use crate::alexander_ai_solutions::security::SecurityPolicy;
use crate::alexander_ai_solutions::tools;
...
pub async fn start_channels(mut config: Config) -> Result<()> {
    ...
    crate::alexander_ai_solutions::workflows::bus::register_workflow_cleanup_subscriber();
    ...
    // alive for the lifetime of the process (same pattern as TracingSubscriber).
    {
        ...
        EMAIL_SIG_HANDLE.get_or_init(|| {
            crate::alexander_ai_solutions::learning::extract::signature::register_email_signature_subscriber()
        });
    }
    ...
    // client is initialised later in `start_channels`).
    {
        ...
        REBUILD_TRIGGER_HANDLE.get_or_init(|| {
            if let Some(client) = crate::alexander_ai_solutions::memory::global::client_if_ready() {
                use crate::alexander_ai_solutions::learning::cache::FacetCache;
                use crate::alexander_ai_solutions::learning::scheduler::register_event_trigger;
                use crate::alexander_ai_solutions::learning::StabilityDetector;
                use std::sync::Arc;
                ...
                crate::alexander_ai_solutions::learning::scheduler::spawn_rebuild_loop(
                    Arc::clone(&detector),
                    crate::alexander_ai_solutions::learning::scheduler::DEFAULT_REBUILD_INTERVAL,
                    shutdown_rx,
                );
    ...
    // PROFILE.md blocks (style, identity, tooling, vetoes, goals).
    {
        ...
        PROFILE_MD_RENDERER_HANDLE.get_or_init(|| {
            if let Some(client) = crate::alexander_ai_solutions::memory::global::client_if_ready() {
                use crate::alexander_ai_solutions::learning::cache::FacetCache;
                use crate::alexander_ai_solutions::learning::ProfileMdRenderer;
                use std::sync::Arc;
        ...
        match resolve_chat_workload(&config) {
            ChatWorkloadResolution::Cloud => {
                ...
                let m = config
                    .default_model
                    .clone()
                    .unwrap_or_else(|| crate::alexander_ai_solutions::config::DEFAULT_MODEL.into());
    ...
    crate::alexander_ai_solutions::config::ensure_agent_dirs(&mut config).await;
    ...
    let security = crate::alexander_ai_solutions::security::live_policy::install(
    ...
    let effective_timeout = crate::alexander_ai_solutions::tool_timeout::set_tool_timeout_secs(
    ...
    let audit = crate::alexander_ai_solutions::security::get_or_create_workspace_audit_logger(
        crate::alexander_ai_solutions::config::AuditConfig::default(),
        config.workspace_dir.clone(),
    )?;
    let temperature = config.default_temperature;
    let local_embedding = config.workload_local_model("embeddings");
    let embedding_api_key = crate::alexander_ai_solutions::embeddings::resolve_api_key(
        &config,
    ...
    let skills = crate::alexander_ai_solutions::workflows::load_workflow_metadata(&workspace);
    ...
    crate::alexander_ai_solutions::workflows::bus::ensure_triggered_workflow_subscriber(&workspace);
    ...
    // subagent via category_filter = "skill".
    let non_skill_tools: Vec<&Box<dyn crate::alexander_ai_solutions::tools::Tool>> = tools_registry
        .iter()
        .filter(|t| {
            t.category() != crate::alexander_ai_solutions::tools::traits::ToolCategory::Workflow
        })
        .collect();
    let non_skill_refs: Vec<&dyn crate::alexander_ai_solutions::tools::Tool> =
    ...
    let _cron_delivery_handle = bus.subscribe(Arc::new(
        crate::alexander_ai_solutions::cron::bus::CronDeliverySubscriber::new(Arc::clone(
            &channels_by_name,
        )),
    ));
    ...
    let proactive_sub =
        crate::alexander_ai_solutions::channels::proactive::ProactiveMessageSubscriber::new(
    ...
    crate::alexander_ai_solutions::channels::proactive::register_active_channel_handle(
    ...
    let _telegram_remote_handle = if channels_by_name.contains_key("telegram") {
        let handle = bus.subscribe(Arc::new(
            crate::alexander_ai_solutions::channels::providers::telegram::TelegramRemoteSubscriber::new(
                config.workspace_dir.clone(),
            ),
        ));
    ...
    let _telegram_approval_surface_handle = if channels_by_name.contains_key("telegram") {
        let handle = bus.subscribe(Arc::new(
            crate::alexander_ai_solutions::channels::providers::telegram::TelegramApprovalSurfaceSubscriber::new(
                Arc::clone(&channels_by_name),
            ),
        ));
    ...
    let _tree_summarizer_handle = bus.subscribe(Arc::new(
        crate::alexander_ai_solutions::memory_tree::tree_runtime::bus::TreeSummarizerEventSubscriber::new(),
    ));
...
fn format_access_context(security: &SecurityPolicy) -> String {
    use crate::alexander_ai_solutions::security::{AutonomyLevel, TrustedAccess};
...
fn resolve_yuanbao_app_secret(
    mut yb_cfg: crate::alexander_ai_solutions::channels::providers::yuanbao::YuanbaoConfig,
    config: &Config,
) -> crate::alexander_ai_solutions::channels::providers::yuanbao::YuanbaoConfig {
    if !yb_cfg.app_secret.is_empty() {
        return yb_cfg;
    }
    let auth = crate::alexander_ai_solutions::credentials::AuthService::from_config(config);
...
pub mod test_support {
    ...
    pub fn resolve_yuanbao_app_secret_for_test(
        yb_cfg: crate::alexander_ai_solutions::channels::providers::yuanbao::YuanbaoConfig,
        config: &Config,
    ) -> crate::alexander_ai_solutions::channels::providers::yuanbao::YuanbaoConfig {
...
mod yuanbao_secret_tests {
    use super::*;
    use crate::alexander_ai_solutions::channels::providers::yuanbao::YuanbaoConfig;
