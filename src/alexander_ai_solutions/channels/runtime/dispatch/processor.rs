...
use crate::alexander_ai_solutions::agent::bus::{
...
use crate::alexander_ai_solutions::channels::providers::telegram::TELEGRAM_APPROVAL_CLIENT_ID;
use crate::alexander_ai_solutions::channels::routes::{
...
use crate::alexander_ai_solutions::channels::SendMessage;
use crate::alexander_ai_solutions::inference::provider::{self, ChatMessage};
...
///
/// [`ApprovalChatContext`]: crate::alexander_ai_solutions::approval::ApprovalChatContext
/// [`ApprovalGate`]: crate::alexander_ai_solutions::approval::ApprovalGate
...
///
/// [`ApprovalGate::decide`]: crate::alexander_ai_solutions::approval::ApprovalGate::decide
async fn try_route_approval_reply(msg: &traits::ChannelMessage) -> bool {
    let Some(gate) = crate::alexander_ai_solutions::approval::ApprovalGate::try_global() else {
        return false;
    };
    ...
    let Some(decision) =
        crate::alexander_ai_solutions::approval::parse_approval_reply(&msg.content)
    else {
        return false;
    };
...
pub(crate) async fn process_channel_message(
    ...
    if ctx.auto_save_memory {
        ...
        let _ = ctx
            ...
            .store(
                ...
                &autosave_key,
                &msg.content,
                crate::alexander_ai_solutions::memory::MemoryCategory::Conversation,
                None,
            )
            .await;
    }
    ...
    // `crate::alexander_ai_solutions::agent::bus::register_agent_handlers`); this keeps
    ...
    let turn_origin =
        crate::alexander_ai_solutions::agent::turn_origin::AgentTurnOrigin::ExternalChannel {
            channel: msg.channel.clone(),
    ...
    let turn_request = AgentTurnRequest {
        ...
        multimodal_files:
            crate::alexander_ai_solutions::config::MultimodalFileConfig::for_untrusted_channel_input(
            ),
    ...
    let llm_result = tokio::time::timeout(Duration::from_secs(ctx.message_timeout_secs), async {
        if channel_has_approval_surface(&msg.channel) {
            ...
            crate::alexander_ai_solutions::approval::APPROVAL_CHAT_CONTEXT
    ...
    let (success, response_text) = match llm_result {
        Ok(Ok(response)) => {
            // Save user + assistant turn to per-sender history
            {
                ...
                while turns.len()
                    > crate::alexander_ai_solutions::channels::context::MAX_CHANNEL_HISTORY
                {
                    turns.remove(0);
                }
            }
        ...
        Ok(Err(e)) => {
            ...
            // arm is unchanged.
            if crate::alexander_ai_solutions::agent::error::is_max_iterations_error(&e.to_string())
            {
                log::info!(
                    target: "channels",
