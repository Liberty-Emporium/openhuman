...
use crate::alexander_ai_solutions::config::{
    Config, DiscordConfig, IMessageConfig, TelegramConfig,
};
...
pub async fn connect_channel(
    ...
    if channel_id == "imessage" && auth_mode == ChannelAuthMode::ManagedDm {
        ...
        tracing::info!(
            target: "alexander_ai_solutions::channels",
    ...
    if channel_id == "telegram" && auth_mode == ChannelAuthMode::BotToken {
        ...
        let (stream_mode, draft_update_interval_ms, silent_streaming, mention_only) =
            if let Some(existing) = persisted.channels_config.telegram.as_ref() {
                ...
                (
                    crate::alexander_ai_solutions::config::StreamMode::default(),
                    1000,
                    true,
                    false,
                )
...
pub async fn channel_status(
    ...
    let health = crate::alexander_ai_solutions::health::snapshot();
...
/// ([`crate::alexander_ai_solutions::channels::proactive::set_runtime_active_channel`]) so the
...
pub async fn set_default_channel(
    ...
    crate::alexander_ai_solutions::channels::proactive::set_runtime_active_channel(Some(
        canonical.clone(),
    ));
