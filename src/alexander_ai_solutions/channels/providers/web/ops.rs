...
use crate::alexander_ai_solutions::prompt_injection::{
...
pub async fn start_chat(
        ...
        let (file_cfg, image_cfg) =
            match crate::alexander_ai_solutions::config::rpc::load_config_with_timeout().await {
                ...
                Err(err) => {
                    ...
                    (
                        crate::alexander_ai_solutions::config::MultimodalFileConfig::default(),
                        crate::alexander_ai_solutions::config::MultimodalConfig::default(),
                    )
                }
            };
        let extracted = crate::alexander_ai_solutions::agent::multimodal::inline_file_attachments(
            &message, &file_cfg,
        )
        .await;
        let processed = crate::alexander_ai_solutions::agent::multimodal::stash_image_attachments(
            &extracted, &image_cfg,
        )
        .await;
        log::debug!(
    ...
    if let Some(gate) = crate::alexander_ai_solutions::approval::ApprovalGate::try_global() {
        if let Some(request_id) = gate.pending_for_thread(&thread_id) {
            if let Some(decision) =
                crate::alexander_ai_solutions::approval::parse_approval_reply(&message)
            {
    ...
    let parsed_mode = match queue_mode.as_deref() {
        Some("steer") => crate::alexander_ai_solutions::agent::harness::run_queue::QueueMode::Steer,
        Some("followup") => {
            crate::alexander_ai_solutions::agent::harness::run_queue::QueueMode::Followup
        }
        Some("collect") => {
            crate::alexander_ai_solutions::agent::harness::run_queue::QueueMode::Collect
        }
        Some("parallel") => {
            crate::alexander_ai_solutions::agent::harness::run_queue::QueueMode::Parallel
        }
        _ => crate::alexander_ai_solutions::agent::harness::run_queue::QueueMode::Interrupt,
    };
    ...
    if !matches!(
        parsed_mode,
        crate::alexander_ai_solutions::agent::harness::run_queue::QueueMode::Interrupt
    ) {
        let in_flight = IN_FLIGHT.lock().await;
        if let Some(existing) = in_flight.get(&map_key) {
            let queued_msg =
                crate::alexander_ai_solutions::agent::harness::run_queue::QueuedMessage {
                    text: message.clone(),
                    mode: parsed_mode,
    ...
    let turn_run_queue = crate::alexander_ai_solutions::agent::harness::run_queue::RunQueue::new();
    ...
    let handle = tokio::spawn(async move {
        let approval_ctx = crate::alexander_ai_solutions::approval::ApprovalChatContext {
            thread_id: thread_id_task.clone(),
            client_id: client_id_task.clone(),
        };
        let origin = crate::alexander_ai_solutions::agent::turn_origin::AgentTurnOrigin::WebChat {
            thread_id: thread_id_task.clone(),
            client_id: client_id_task.clone(),
        };
        ...
        let result = tokio::select! {
            ...
            res = crate::alexander_ai_solutions::agent::turn_origin::with_origin(
                origin,
                crate::alexander_ai_solutions::approval::APPROVAL_CHAT_CONTEXT.scope(
                    approval_ctx,
                    run_chat_task(
                        &client_id_task,
                        &thread_id_task,
        ...
        match result {
            Ok(chat_result) => {
                crate::alexander_ai_solutions::channels::providers::presentation::deliver_response(
                    &client_id_task,
                    &thread_id_task,
                    &request_id_task,
            ...
            Err(err) => {
                ...
                if crate::alexander_ai_solutions::agent::error::is_max_iterations_error(&detailed) {
                    log::info!(
                        target: "web_channel",
...
fn dispatch_followups(
    followups: Vec<crate::alexander_ai_solutions::agent::harness::run_queue::QueuedMessage>,
) {
...
async fn spawn_parallel_turn(
    ...
    let run_queue = crate::alexander_ai_solutions::agent::harness::run_queue::RunQueue::new();
...
pub async fn channel_web_cancel(
    ...
    let cancelled = if cancelled_request_id.is_some() {
        ...
        crate::alexander_ai_solutions::agent::task_dispatcher::cancel_session(thread_id.trim())
            .await
    };
