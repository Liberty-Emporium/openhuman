...
use crate::alexander_ai_solutions::profiles::AgentProfileStore;
use crate::alexander_ai_solutions::threads::turn_state::TurnStateStore;
...
pub(crate) async fn run_chat_task(
    ...
    run_queue: Arc<crate::alexander_ai_solutions::agent::harness::run_queue::RunQueue>,
    ...
    if was_built_fresh {
        match crate::alexander_ai_solutions::memory_conversations::get_messages(
    ...
    let result =
        match crate::alexander_ai_solutions::inference::provider::thread_context::with_thread_id(
            thread_id.to_string(),
            crate::alexander_ai_solutions::memory::source_scope::with_source_scope(
        ...
        .await
        {
            ...
            Err(err) => {
                ...
                let has_fresh_signal = if !is_budget && is_empty {
                    super::ops::has_fresh_budget_signal(thread_id, &current_fp.provider_binding)
                        .await
                } else {
                    false
                };
                match super::ops::classify_budget_correlation(is_budget, is_empty, has_fresh_signal)
                {
                    BudgetCorrelation::BudgetExhausted => {
                        ...
                        super::ops::record_budget_signal(thread_id, &current_fp.provider_binding)
                            .await;
                        log::warn!(
    ...
    if let Ok(ref task_result) = result {
        ...
        if speak_reply && !trimmed_response.is_empty() {
            let opts =
                crate::alexander_ai_solutions::voice::reply_speech::ReplySpeechOptions::default();
            match crate::alexander_ai_solutions::voice::reply_speech::synthesize_reply(
                &config,
        ...
        if metadata.source.as_deref() == Some("ptt") {
            if let Some(session_id) = metadata.session_id {
                crate::alexander_ai_solutions::voice::publish_ptt_transcript_committed(
                    thread_id.to_string(),
                    session_id,
