...
pub(crate) fn is_inference_budget_exceeded_error(message: &str) -> bool {
    ...
    crate::alexander_ai_solutions::inference::provider::is_budget_exhausted_message(message)
}
    ...
    while let Some(c) = chars.next() {
        match c {
            '"' => {
                ...
                let sanitized =
                    crate::alexander_ai_solutions::inference::provider::sanitize_api_error(trimmed);
                return Some(crate::alexander_ai_solutions::util::truncate_with_ellipsis(
                    &sanitized,
                    MAX_DETAIL_CHARS,
                ));
            }
...
/// error string. Mirrors the typed [`crate::alexander_ai_solutions::inference::
...
pub(crate) fn classify_inference_error(err: &str) -> ClassifiedError {
    ...
    } else if crate::alexander_ai_solutions::agent::error::is_max_iterations_error(err) {
        ClassifiedError {
    ...
    } else if crate::alexander_ai_solutions::inference::provider::is_provider_config_rejection_message(err) {
...
/// String-flat mirror of
/// [`crate::alexander_ai_solutions::inference::provider::reliable::is_non_retryable_rate_limit`].
///
