...
use crate::alexander_ai_solutions::config::{rpc as config_rpc, Config};
...
use crate::alexander_ai_solutions::memory_sync::workspace::watcher::state::WatcherStateStore;
use crate::alexander_ai_solutions::scheduler_gate::gate::current_policy;
...
async fn handle_remove(
    ...
    if let Some(mtime) = last_mtime {
        let source_id = format!("vault_watcher:{rel}@{mtime}");
        if let Err(e) =
            crate::alexander_ai_solutions::memory::ingest_pipeline::mark_document_deleted(config, &source_id)
                .await
        {
            tracing::warn!(
                path = %rel,
