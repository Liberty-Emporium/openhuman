...
//!   [`crate::alexander_ai_solutions::agent::triage::run_triage`] to produce a
//!   [`TriageDecision`] and then
//!   [`crate::alexander_ai_solutions::agent::triage::apply_decision`] to act on it.
...
//!   the bus.
//!
//! [trigger_triage]: crate::alexander_ai_solutions::agent_registry::agents
//!
//! ## Feature flag
//!
...
use crate::alexander_ai_solutions::agent::triage::{
    apply_decision, run_triage, TriageOutcome, TriggerEnvelope,
};
use crate::alexander_ai_solutions::composio::trigger_history;
use crate::alexander_ai_solutions::config::rpc as config_rpc;
use crate::alexander_ai_solutions::config::schema::COMPOSIO_MODE_DIRECT;
...
use crate::alexander_ai_solutions::composio::client::ComposioClient;
use crate::alexander_ai_solutions::composio::ops;
        ...
        tokio::spawn(async move {
            ...
            let (src_max_items, src_sync_depth_days) = {
                let registry_sources =
                    crate::alexander_ai_solutions::memory_sources::list_sources()
                        .await
                        .unwrap_or_default();
                registry_sources
                    .iter()
                    .find(|s| {
                        s.kind == crate::alexander_ai_solutions::memory_sources::SourceKind::Composio
                            && s.connection_id.as_deref() == Some(connection_id.as_str())
                    })
                    .map(|s| (s.max_items, s.sync_depth_days))
                    .unwrap_or_else(|| {
                        crate::alexander_ai_solutions::memory_sources::memory_sync_defaults_for_toolkit(
                            toolkit.as_str(),
                        )
                    })
            };
            ...
            if !ctx.config.onboarding_completed {
                ...
                let Some(provider) = get_provider(&toolkit) else {
                    ...
                    if let Err(e) =
                        crate::alexander_ai_solutions::memory_sources::upsert_composio_source(
                            &toolkit,
                            &connection_id,
                            &label,
                        )
            ...
            // initial sync ran.
            let label = format!("{toolkit} connection");
            if let Err(e) = crate::alexander_ai_solutions::memory_sources::upsert_composio_source(
                &toolkit,
                &connection_id,
                &label,
            )
