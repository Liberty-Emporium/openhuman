...
//! [`crate::alexander_ai_solutions::inference::paths::resolve_whisper_binary`] — kept in
...
//! [`crate::alexander_ai_solutions::inference::local::install_whisper`] which streams the
//! GGML model file (`ggml-<size>.bin`) into
//! `~/.alexanderai/bin/whisper/` via a `.part` file + atomic rename, plus
...
use crate::alexander_ai_solutions::inference::paths::resolve_whisper_binary_with_config;
...
pub async fn transcribe_whisper(
    ...
    let model_path = crate::alexander_ai_solutions::inference::paths::resolve_stt_model_path_by_id(
        &model_id, config,
    )
