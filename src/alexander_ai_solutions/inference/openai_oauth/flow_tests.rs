...
use crate::alexander_ai_solutions::credentials::profiles::{
    AuthProfile, AuthProfileKind, AuthProfilesStore, TokenSet,
};
use crate::alexander_ai_solutions::inference::openai_oauth::store::{
    import_codex_cli_auth_from_path, OPENAI_OAUTH_PROFILE_NAME, OPENAI_PROVIDER_KEY,
};
use crate::alexander_ai_solutions::inference::openai_oauth::{
    lookup_openai_bearer_token, lookup_openai_oauth_credentials,
};
use crate::alexander_ai_solutions::inference::provider::factory::lookup_key_for_slug;
...
async fn inference_import_codex_cli_surfaces_error_when_auth_missing() {
    let _env_lock = crate::alexander_ai_solutions::config::TEST_ENV_LOCK
        .lock()
    ...
    let err =
        crate::alexander_ai_solutions::inference::ops::inference_openai_oauth_import_codex_cli(
            &config,
        )
        .await
        .unwrap_err();
...
fn credits_gate_bypassed_with_oauth_only_credentials() {
    ...
    use crate::alexander_ai_solutions::inference::provider::factory::role_bypasses_managed_credits;
