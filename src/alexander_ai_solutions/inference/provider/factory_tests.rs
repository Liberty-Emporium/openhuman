...
use crate::alexander_ai_solutions::inference::provider::traits::{
    ChatMessage, ChatRequest, ProviderDelta,
};
...
fn orcarouter_legacy_type_seeds_defaults() {
    use crate::alexander_ai_solutions::config::schema::cloud_providers::migrate_legacy_fields;
...
fn managed_backend_translates_hint_burst_to_burst_tier() {
    ...
    let (_, model) = create_chat_provider_from_string("chat", "openhuman", &config)
        .expect("managed backend must build");
    assert_eq!(model, crate::alexander_ai_solutions::config::MODEL_BURST_V1);
}
...
fn managed_backend_pins_subconscious_role_to_chat_tier() {
    ...
    assert_eq!(model, crate::alexander_ai_solutions::config::MODEL_CHAT_V1);
...
async fn live_lmstudio_provider_streams_thinking_and_text() {
    let _guard = crate::alexander_ai_solutions::inference::inference_test_guard();
    let mut config = Config::default();
...
fn local_provider_string_detection() {
    use crate::alexander_ai_solutions::inference::local::profile::is_local_provider_string;
...
fn omlx_dispatch_empty_model_errors() {
    ...
    let config = crate::alexander_ai_solutions::config::Config::default();
