...
async fn configure_openrouter_workspace(tmp: &TempDir, endpoint: String, token: &str) -> Config {
    ...
    auth.store_provider_token(
        &crate::alexander_ai_solutions::inference::provider::factory::auth_key_for_slug(
            "openrouter",
        ),
        "default",
        token,
...
fn is_backend_auth_failure_only_matches_openhuman_backend_401_403() {
    ...
    let backend =
        crate::alexander_ai_solutions::inference::provider::openhuman_backend::PROVIDER_LABEL;
...
async fn chat_completions_backend_401_publishes_session_expired() {
    ...
    let err =
        crate::alexander_ai_solutions::inference::provider::traits::Provider::chat_with_system(
            &provider,
            None,
            "hi",
            "reasoning-quick-v1",
