...
impl Provider for OpenAiCompatibleProvider {
    fn capabilities(
        &self,
    ) -> crate::alexander_ai_solutions::inference::provider::traits::ProviderCapabilities {
        crate::alexander_ai_solutions::inference::provider::traits::ProviderCapabilities {
    ...
    fn prompt_cache_capabilities(
        &self,
    ) -> crate::alexander_ai_solutions::inference::provider::traits::PromptCacheCapabilities {
    ...
    async fn chat(
        ...
        if self.responses_api_primary {
            ...
            if let Some(tx) = request.stream {
                let _ = tx
                    .send(
                        crate::alexander_ai_solutions::inference::provider::ProviderDelta::TextDelta {
                            delta: text.clone(),
                        },
                    )
                    .await;
    ...
    fn stream_chat_with_system(
        ...
        tokio::spawn(async move {
            ...
            if !response.status().is_success() {
                ...
                let sanitized_error =
                    crate::alexander_ai_solutions::inference::provider::sanitize_api_error(
                        &raw_error,
                    );
                let message = format!("{}: {}", status, sanitized_error);
                if crate::alexander_ai_solutions::inference::provider::is_budget_exhausted_http_400(
                    status, &raw_error,
                ) {
                    crate::alexander_ai_solutions::inference::provider::log_budget_exhausted_http_400(
                        "stream_chat",
                        provider_name.as_str(),
                        Some(model_owned.as_str()),
                        status,
                    );
                } else if crate::alexander_ai_solutions::inference::provider::is_local_provider_no_model_loaded(
                    status, &raw_error,
                ) {
                    // Local inference server up but no model loaded — pure local
                    // user-state, demote instead of paging (TAURI-RUST-DMQ).
                    crate::alexander_ai_solutions::inference::provider::log_local_provider_no_model_loaded(
                        "stream_chat",
                        provider_name.as_str(),
                        Some(model_owned.as_str()),
                        status,
                    );
                } else if crate::alexander_ai_solutions::inference::provider::is_custom_openai_upstream_bad_request_http_400(
                    provider_name.as_str(),
                    status,
                    &raw_error,
                ) {
                    crate::alexander_ai_solutions::inference::provider::log_custom_openai_upstream_bad_request_http_400(
                        "stream_chat",
                        provider_name.as_str(),
                        Some(model_owned.as_str()),
                        status,
                    );
                } else if crate::alexander_ai_solutions::inference::provider::is_provider_access_policy_denied_http_403(
                    status,
                    &raw_error,
                ) {
                    crate::alexander_ai_solutions::inference::provider::log_provider_access_policy_denied_http_403(
                        "stream_chat",
                        provider_name.as_str(),
                        Some(model_owned.as_str()),
                        status,
                    );
                } else if crate::alexander_ai_solutions::inference::provider::is_provider_config_rejection_http(
                    status,
                    provider_name.as_str(),
                    &raw_error,
                ) {
                    crate::alexander_ai_solutions::inference::provider::log_provider_config_rejection(
                        "stream_chat",
                        provider_name.as_str(),
                        Some(model_owned.as_str()),
                        status,
                    );
                } else if crate::alexander_ai_solutions::inference::provider::is_backend_error_code_owned(
                    provider_name.as_str(),
                    &raw_error,
                ) {
                    ...
                    crate::alexander_ai_solutions::inference::provider::log_backend_error_code_owned(
                        "stream_chat",
                        provider_name.as_str(),
                        Some(model_owned.as_str()),
                        status,
                ...
                } else if crate::alexander_ai_solutions::inference::provider::is_byo_provider_auth_failure_http(
                    provider_name.as_str(),
                    status,
                    &raw_error,
                ) {
                    crate::alexander_ai_solutions::inference::provider::log_byo_provider_auth_failure(
                        "stream_chat",
                        provider_name.as_str(),
                        Some(model_owned.as_str()),
                        status,
                    );
                } else if crate::alexander_ai_solutions::inference::provider::is_provider_insufficient_credits_402(
                    status,
                    &raw_error,
                ) {
                    ...
                    crate::alexander_ai_solutions::inference::provider::log_provider_insufficient_credits_402(
                        "stream_chat",
                        provider_name.as_str(),
                        Some(model_owned.as_str()),
                        status,
                    );
                } else if crate::alexander_ai_solutions::inference::provider::should_report_provider_http_failure(
                    status,
                ) {
                    crate::core::observability::report_error(
                        message.as_str(),
    ...
    async fn effective_context_window(&self, model: &str) -> Option<u64> {
        ...
        let Some(kind) = self.local_provider_kind else {
            return crate::alexander_ai_solutions::inference::context_window_for_model(model);
        };
        ...
        crate::alexander_ai_solutions::inference::model_context::context_window_for_model_with_local_fallback(
            model,
            Some(kind),
        )
    }
...
impl OpenAiCompatibleProvider {
    ...
    async fn lm_studio_loaded_context_window(&self, model: &str) -> Option<u64> {
        use crate::alexander_ai_solutions::inference::local::lm_studio;
