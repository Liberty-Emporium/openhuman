...
pub async fn list_configured_models_from_config(
    ...
    let looked_up =
        crate::alexander_ai_solutions::inference::provider::factory::lookup_key_for_slug(
            &entry.slug,
            config,
        )
        .unwrap_or_default();
    ...
    let client = crate::alexander_ai_solutions::config::build_runtime_proxy_client_with_timeouts(
        "providers.list_models",
        30,
        10,
    );

    use crate::alexander_ai_solutions::config::schema::cloud_providers::AuthStyle;
    ...
    if !status.is_success() {
        ...
        let truncated =
            crate::alexander_ai_solutions::util::truncate_with_ellipsis(&sanitized, 300);
    ...
    let body: serde_json::Value = serde_json::from_str(&raw_body).map_err(|e| {
        ...
        let snippet = crate::alexander_ai_solutions::util::truncate_with_ellipsis(&sanitized, 300);
        format!(
...
/// [`crate::alexander_ai_solutions::config::schema::cloud_providers::is_slug_reserved`]),
...
pub fn synthesize_local_runtime_entry(
    slug: &str,
    config: &crate::alexander_ai_solutions::config::Config,
) -> Option<crate::alexander_ai_solutions::config::schema::cloud_providers::CloudProviderCreds> {
    ...
    let endpoint = match slug {
        ...
        "ollama" => {
            let base = crate::alexander_ai_solutions::inference::local::ollama_base_url_from_config(
                config,
            );
