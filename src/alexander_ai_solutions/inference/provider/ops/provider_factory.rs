...
pub fn create_backend_inference_provider(
    ...
    if let (Some(url), Some(key)) = (inference_url, api_key) {
        ...
        Ok(Box::new(
            crate::alexander_ai_solutions::inference::provider::compatible::OpenAiCompatibleProvider::new_no_responses_fallback(
                "custom_openai",
                url,
                Some(key),
                crate::alexander_ai_solutions::inference::provider::compatible::AuthStyle::Bearer,
            ),
        ))
    } else {
        ...
        Ok(Box::new(
            crate::alexander_ai_solutions::inference::provider::openhuman_backend::OpenHumanBackendProvider::new(
                backend_url,
                options,
            ),
        ))
...
pub fn create_resilient_provider(
    ...
    backend_url: Option<&str>,
    api_key: Option<&str>,
    reliability: &crate::alexander_ai_solutions::config::ReliabilityConfig,
) -> anyhow::Result<Box<dyn Provider>> {
...
pub fn create_routed_provider(
    ...
    api_key: Option<&str>,
    reliability: &crate::alexander_ai_solutions::config::ReliabilityConfig,
    model_routes: &[crate::alexander_ai_solutions::config::ModelRouteConfig],
    default_model: &str,
) -> anyhow::Result<Box<dyn Provider>> {
        ...
        .map(|r| {
            (
                ...
                router::Route {
                    ...
                    model: r.model.clone(),
                    context_window:
                        crate::alexander_ai_solutions::inference::model_context::context_window_for_model(
                            &r.model,
                        ),
                },
            )
...
pub fn create_intelligent_routing_provider(
    ...
    let provider = crate::alexander_ai_solutions::routing::new_provider(
        remote,
        &config.local_ai,
        default_model,
