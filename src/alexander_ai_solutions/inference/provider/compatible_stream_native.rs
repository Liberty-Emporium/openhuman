use crate::alexander_ai_solutions::inference::provider::traits::ChatResponse as ProviderChatResponse;
...
impl OpenAiCompatibleProvider {
    ...
    pub(super) async fn stream_native_chat(
        ...
        delta_tx: &tokio::sync::mpsc::Sender<
            crate::alexander_ai_solutions::inference::provider::ProviderDelta,
        >,
        dump_seq: u64,
                    ...
                    for choice in chunk.choices {
                        if let Some(content) = choice.delta.content.as_ref() {
                            if !content.is_empty() {
                                ...
                                let _ = delta_tx
                                    .send(crate::alexander_ai_solutions::inference::provider::ProviderDelta::TextDelta {
                                        delta: content.clone(),
                                    })
                                    .await;
                        ...
                        if let Some(reasoning) = choice.delta.reasoning_content.as_ref() {
                            if !reasoning.is_empty() {
                                ...
                                let _ = delta_tx
                                    .send(
                                        crate::alexander_ai_solutions::inference::provider::ProviderDelta::ThinkingDelta {
                            ...
                            for tc in tc_list {
                                ...
                                if !entry.emitted_start {
                                        ...
                                        (entry.id.as_ref(), entry.name.as_ref())
                                    {
                                        ...
                                        let _ = delta_tx
                                            .send(crate::alexander_ai_solutions::inference::provider::ProviderDelta::ToolCallStart {
                                                call_id: id.clone(),
                                        ...
                                        if !entry.arguments.is_empty() {
                                            ...
                                            let _ = delta_tx
                                                .send(crate::alexander_ai_solutions::inference::provider::ProviderDelta::ToolCallArgsDelta {
                                                    call_id: id.clone(),
                                                    delta: buffered,
                                                })
                                                .await;
...
mod sse_error_frame_tests {
    ...
    use crate::alexander_ai_solutions::inference::provider::openhuman_backend::PROVIDER_LABEL;
