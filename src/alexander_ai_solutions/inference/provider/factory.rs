...
use crate::alexander_ai_solutions::config::schema::cloud_providers::{
...
use crate::alexander_ai_solutions::inference::provider::claude_agent_sdk::subprocess::ClaudeAgentSdkProvider;
use crate::alexander_ai_solutions::inference::provider::compatible::{
    AuthStyle as CompatAuthStyle, OpenAiCompatibleProvider,
};
use crate::alexander_ai_solutions::inference::provider::openai_codex::{
...
use crate::alexander_ai_solutions::inference::provider::openhuman_backend::OpenHumanBackendProvider;
use crate::alexander_ai_solutions::inference::provider::traits::Provider;
use crate::alexander_ai_solutions::inference::provider::ProviderRuntimeOptions;
...
fn is_abstract_tier_model(model: &str) -> bool {
    use crate::alexander_ai_solutions::config::{
...
pub fn resolve_model_for_hint(hint_or_tier: &str, config: &Config) -> String {
    let hint_to_tier: &[(&str, &str)] = &[
        (
            "reasoning",
            crate::alexander_ai_solutions::config::MODEL_REASONING_V1,
        ),
        ("chat", crate::alexander_ai_solutions::config::MODEL_CHAT_V1),
        (
            "agentic",
            crate::alexander_ai_solutions::config::MODEL_AGENTIC_V1,
        ),
        (
            "coding",
            crate::alexander_ai_solutions::config::MODEL_CODING_V1,
        ),
        (
            "burst",
            crate::alexander_ai_solutions::config::MODEL_BURST_V1,
        ),
        (
            "vision",
            crate::alexander_ai_solutions::config::MODEL_VISION_V1,
        ),
        (
            "summarization",
            crate::alexander_ai_solutions::config::MODEL_SUMMARIZATION_V1,
        ),
        ...
        (
            "subconscious",
            crate::alexander_ai_solutions::config::MODEL_CHAT_V1,
        ),
    ];
    let tier_to_role: &[(&str, &str)] = &[
        ...
        (crate::alexander_ai_solutions::config::MODEL_CHAT_V1, "chat"),
        (
            crate::alexander_ai_solutions::config::MODEL_REASONING_QUICK_V1,
            "chat",
        ),
        (
            crate::alexander_ai_solutions::config::MODEL_AGENTIC_V1,
...
/// the constants in `crate::alexander_ai_solutions::config`; the four `hint:*` strings that
...
/// ([`crate::alexander_ai_solutions::inference::model_context::model_vision_enabled`]).
    ...
    if s.is_empty() || s == "cloud" {
        ...
        if !matches!(role, "chat" | "reasoning" | "coding") {
            if let Some(chat) = config.chat_provider.as_deref() {
                if crate::alexander_ai_solutions::inference::local::profile::is_local_provider_string(chat) {
...
fn route_has_usable_credentials(resolved: &str, config: &Config) -> bool {
    ...
    if crate::alexander_ai_solutions::inference::local::profile::is_local_provider_string(r)
        || r.starts_with(
            crate::alexander_ai_solutions::inference::provider::claude_code::PROVIDER_PREFIX,
        )
...
pub mod test_provider_override {
    use super::Provider;
    use crate::alexander_ai_solutions::inference::provider::traits::{
...
pub fn create_chat_provider_from_string(
    ...
    if let Some(model_with_temp) = p.strip_prefix(
        crate::alexander_ai_solutions::inference::provider::claude_code::PROVIDER_PREFIX,
    ) {
        ...
        let workspace =
            crate::alexander_ai_solutions::inference::provider::claude_code::workspace_dir_from_config(config);
        log::debug!(
        ...
        let provider =
            crate::alexander_ai_solutions::inference::provider::claude_code::ClaudeCodeProvider::from_env(
                model.clone(),
                workspace,
...
fn make_openhuman_backend(
    ...
    let model = match model.strip_prefix("hint:") {
        Some("reasoning") => crate::alexander_ai_solutions::config::MODEL_REASONING_V1.to_string(),
        Some("chat") => crate::alexander_ai_solutions::config::MODEL_CHAT_V1.to_string(),
        Some("agentic") => crate::alexander_ai_solutions::config::MODEL_AGENTIC_V1.to_string(),
        Some("coding") => crate::alexander_ai_solutions::config::MODEL_CODING_V1.to_string(),
        Some("burst") => crate::alexander_ai_solutions::config::MODEL_BURST_V1.to_string(),
        Some("summarization") => {
            crate::alexander_ai_solutions::config::MODEL_SUMMARIZATION_V1.to_string()
        }
        Some("vision") => crate::alexander_ai_solutions::config::MODEL_VISION_V1.to_string(),
        Some(_) => {
        ...
        None => {
            if is_known_openhuman_tier(&model) {
                ...
                crate::alexander_ai_solutions::config::MODEL_REASONING_V1.to_string()
            }
        }
    };
...
fn verify_session_active(config: &Config) -> anyhow::Result<()> {
    ...
    if crate::alexander_ai_solutions::scheduler_gate::is_signed_out() {
        anyhow::bail!(
    ...
    let state_dir = config
        ...
        .unwrap_or_else(|| {
            directories::UserDirs::new()
                .map(|d| d.home_dir().join(".alexanderai"))
                .unwrap_or_else(|| std::path::PathBuf::from(".alexanderai"))
        });
    let auth = AuthService::new(&state_dir, config.secrets.encrypt);
    let has_session = auth
        .get_provider_bearer_token(
            crate::alexander_ai_solutions::credentials::APP_SESSION_PROVIDER,
            None,
        )?
        .filter(|s| !s.trim().is_empty())
        .is_some();
...
fn cloud_entry_provider_string(
    entry: &crate::alexander_ai_solutions::config::schema::cloud_providers::CloudProviderCreds,
    config: &Config,
) -> String {
    ...
    let model = entry
        ...
        .unwrap_or(crate::alexander_ai_solutions::config::DEFAULT_MODEL);
...
fn make_ollama_provider(
    ...
    use crate::alexander_ai_solutions::inference::local::profile::LocalProviderKind;

    let base_url =
        crate::alexander_ai_solutions::inference::local::ollama_base_url_from_config(config);
...
fn make_lm_studio_provider(
    ...
    let endpoint =
        crate::alexander_ai_solutions::inference::local::lm_studio::lm_studio_base_url(config);
...
fn make_mlx_provider(
    ...
    use crate::alexander_ai_solutions::inference::local::profile::{
        LocalProviderKind, MLX_PROFILE,
    };
...
fn make_omlx_provider(
    ...
    use crate::alexander_ai_solutions::inference::local::profile::{
        LocalProviderKind, OMLX_PROFILE,
    };
...
fn make_local_openai_provider(
    ...
    use crate::alexander_ai_solutions::inference::local::profile::{
        LocalProviderKind, LOCAL_OPENAI_PROFILE,
    };
...
pub fn lookup_key_for_slug(slug: &str, config: &Config) -> anyhow::Result<String> {
    ...
    if slug == "openai" {
        match crate::alexander_ai_solutions::inference::openai_oauth::lookup_openai_bearer_token(
            config,
        ) {
