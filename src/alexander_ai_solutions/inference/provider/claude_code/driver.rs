...
fn seatbelt_profile(workspace_dir: &std::path::Path) -> String {
    ...
    // workspace subdir. `workspace_dir` is `…/.alexanderai-staging/users/<id>/…`,
...
/// Resolve the OpenHuman internal root (`~/.openhuman` / `~/.alexanderai-staging`)
...
fn openhuman_internal_root(workspace_dir: &std::path::Path) -> std::path::PathBuf {
    ...
    loop {
        if cur
            .file_name()
            .and_then(|n| n.to_str())
            .map(|n| n.starts_with(".alexanderai"))
            .unwrap_or(false)
        {
...
pub async fn run_turn(ctx: TurnContext<'_>) -> anyhow::Result<ChatResponse> {
    ...
    match crate::alexander_ai_solutions::mcp_server::ensure_local_http().await {
...
mod tests {
    ...
    fn seatbelt_profile_denies_whole_openhuman_root_not_just_subdir() {
        ...
        // `.alexanderai-staging` tree (so root-level core.token/credentials are
        // protected), not just the subdir.
        let ws = std::path::Path::new("/Users/test/.alexanderai-staging/users/abc/workspace");
        ...
        assert!(
            p.contains("/Users/test/.alexanderai-staging\""),
            "deny subpath must be the .alexanderai root: {p}"
        );
        assert!(
    ...
    fn openhuman_internal_root_walks_up_to_dotopenhuman() {
        let r = openhuman_internal_root(std::path::Path::new(
            "/Users/x/.alexanderai/users/id/workspace/memory",
        ));
        assert_eq!(r, std::path::Path::new("/Users/x/.alexanderai"));
