...
use crate::alexander_ai_solutions::inference::local as local_runtime;
use crate::alexander_ai_solutions::inference::local::ops::ReactionDecision;
use crate::alexander_ai_solutions::inference::provider as providers;
use crate::alexander_ai_solutions::inference::{device, presets, sentiment, SentimentResult};
use crate::alexander_ai_solutions::inference::{LocalAiEmbeddingResult, LocalAiStatus};
...
pub async fn inference_test_provider_model(
        ...
        || provider.trim().starts_with("ollama:")
    {
        ...
        let (chat_provider, model) =
            crate::alexander_ai_solutions::inference::provider::factory::create_local_chat_provider_from_string(
                provider, config,
            )
        ...
        let (chat_provider, model) =
                crate::alexander_ai_solutions::inference::provider::factory::create_chat_provider_from_string(
...
pub async fn inference_openai_oauth_start(config: &Config) -> Result<RpcOutcome<Value>, String> {
    ...
    let result = crate::alexander_ai_solutions::inference::openai_oauth::start_openai_oauth(config)
        .map(|start| {
            RpcOutcome::single_log(
                json!({
...
pub async fn inference_openai_oauth_complete(
    ...
    let result = crate::alexander_ai_solutions::inference::openai_oauth::complete_openai_oauth(
        config,
        callback_url,
    )
    .await
...
pub async fn inference_openai_oauth_import_codex_cli(
    ...
    let result =
        crate::alexander_ai_solutions::inference::openai_oauth::import_openai_oauth_from_codex_cli(
            config,
        )
...
pub async fn inference_openai_oauth_status(config: &Config) -> Result<RpcOutcome<Value>, String> {
    ...
    let result = crate::alexander_ai_solutions::inference::openai_oauth::openai_oauth_status(
        config,
    )
    .map(|status| {
        RpcOutcome::single_log(
...
pub async fn inference_openai_oauth_disconnect(
    ...
    let result =
        crate::alexander_ai_solutions::inference::openai_oauth::disconnect_openai_oauth(config)
