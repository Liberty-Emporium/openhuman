...
//! [`crate::alexander_ai_solutions::inference::local::voice_install_common`] status table so
...
/// [`crate::alexander_ai_solutions::voice::factory::DEFAULT_WHISPER_MODEL`].
...
pub fn status(config: &Config) -> VoiceInstallStatus {
    ...
    if matches!(snapshot.state, VoiceInstallState::Missing) {
        let configured =
            crate::alexander_ai_solutions::inference::model_ids::effective_stt_model_id(config);
...
mod tests {
    use super::*;
    use crate::alexander_ai_solutions::inference::local::voice_install_common::reset_status;
    ...
    /// Serialise tests that write into the shared `~/.alexanderai/bin/whisper/`
