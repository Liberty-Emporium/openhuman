...
use crate::alexander_ai_solutions::inference::parse::sanitize_inline_completion;
...
impl LocalAiService {
    ...
    pub(crate) async fn chat_with_history(
        &self,
        config: &Config,
        messages: Vec<crate::alexander_ai_solutions::inference::local::ollama::OllamaChatMessage>,
        max_tokens: Option<u32>,
    ) -> Result<String, String> {
    ...
    async fn chat_with_history_internal(
        ...
        let _gate_permit = if gated {
            crate::alexander_ai_solutions::scheduler_gate::wait_for_capacity().await
        } else {
            None
        };
        ...
        if provider_from_config(config) == LocalAiProvider::LmStudio {
            ...
            let lm_messages = messages
                .into_iter()
                .map(
                    |message| crate::alexander_ai_solutions::inference::local::lm_studio::LmStudioChatMessage {
                        role: message.role,
        ...
        tracing::debug!(
            ...
            model = %crate::alexander_ai_solutions::inference::model_ids::effective_chat_model_id(config),
            "[local_ai:chat] sending to ollama /api/chat"
        );

        let started = std::time::Instant::now();

        let body = crate::alexander_ai_solutions::inference::local::ollama::OllamaChatRequest {
            model: crate::alexander_ai_solutions::inference::model_ids::effective_chat_model_id(
                config,
            ),
            messages,
            stream: false,
            options: Some(
                crate::alexander_ai_solutions::inference::local::ollama::OllamaGenerateOptions {
        ...
        let payload: crate::alexander_ai_solutions::inference::local::ollama::OllamaChatResponse =
            response
                .json()
                .await
    ...
    async fn inference_with_temperature_internal(
        ...
        if provider_from_config(config) == LocalAiProvider::LmStudio {
            let messages = vec![
                crate::alexander_ai_solutions::inference::local::lm_studio::LmStudioChatMessage {
        ...
        let payload: crate::alexander_ai_solutions::inference::local::ollama::OllamaGenerateResponse = response
            .json()
            .await
