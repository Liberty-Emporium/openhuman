...
use crate::alexander_ai_solutions::inference::local::install::{
    find_system_ollama_binary, run_ollama_install_script,
};
use crate::alexander_ai_solutions::inference::local::process_util::apply_no_window;
use crate::alexander_ai_solutions::inference::paths::find_workspace_ollama_binary;

use super::super::LocalAiService;

impl LocalAiService {
    pub(in crate::alexander_ai_solutions::inference::local::service) async fn resolve_or_install_ollama_binary(
        &self,
        config: &Config,
    ...
    pub(in crate::alexander_ai_solutions::inference::local::service) async fn command_works(
        &self,
        command: &Path,
    ) -> bool {
    ...
    pub(in crate::alexander_ai_solutions::inference::local::service) async fn download_and_install_ollama(
        &self,
        config: &Config,
    ) -> Result<(), String> {
        let install_dir =
            crate::alexander_ai_solutions::inference::paths::workspace_ollama_dir(config);
        ...
        if crate::alexander_ai_solutions::inference::local::install::is_ollama_installer_running() {
            log::info!(
            ...
            while crate::alexander_ai_solutions::inference::local::install::is_ollama_installer_running() {
