...
async fn inline_complete_interactive_does_not_block_on_held_permit() {
    ...
    let _held = crate::alexander_ai_solutions::scheduler_gate::gate::try_acquire_llm_permit()
...
async fn gated_inline_complete_blocks_on_held_permit() {
    ...
    let held = crate::alexander_ai_solutions::scheduler_gate::gate::try_acquire_llm_permit()
        .expect("test must start with a free permit");
