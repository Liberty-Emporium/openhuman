...
///
/// Default callers see the shared `~/.alexanderai/` root, which avoids
...
pub(crate) fn resolve_whisper_binary() -> Option<PathBuf> {
    ...
    if let Ok(shared) = crate::alexander_ai_solutions::config::default_root_openhuman_dir() {
...
pub(crate) fn resolve_whisper_binary_with_config(config: &Config) -> Option<PathBuf> {
    if let Some(workspace) =
        crate::alexander_ai_solutions::inference::local::install_whisper::find_workspace_whisper_binary(config)
    {
...
pub(crate) fn resolve_piper_binary_with_config(config: &Config) -> Option<PathBuf> {
    if let Some(workspace) =
        crate::alexander_ai_solutions::inference::local::install_piper::find_workspace_piper_binary(
            config,
        )
    {
        return Some(workspace);
...
mod tests {
    ...
    fn resolve_tts_voice_path_appends_onnx_for_voice_ids() {
        ...
        // root (`~/.alexanderai/`) — not the temp config. If a sibling
