...
use super::*;
use crate::alexander_ai_solutions::config::UpdateRestartStrategy;
use crate::alexander_ai_solutions::update::types::{UpdateApplyResult, UpdateInfo};
...
async fn update_version_returns_cargo_pkg_version() {
    ...
    assert_eq!(v, crate::alexander_ai_solutions::update::current_version());
}
...
async fn update_version_returns_target_triple() {
    ...
    assert_eq!(
        triple,
        crate::alexander_ai_solutions::update::platform_triple()
    );
}
...
async fn update_version_asset_prefix_contains_target_triple() {
    let outcome = update_version().await;
    let triple = crate::alexander_ai_solutions::update::platform_triple();
    let prefix = outcome
        .value
        .get("asset_prefix")
        .and_then(|v| v.as_str())
...
async fn update_run_rejected_when_rpc_mutations_disabled() {
    ...
    let mut cfg = crate::alexander_ai_solutions::config::Config {
        workspace_dir: tmp.path().join("workspace"),
        action_dir: tmp.path().join("workspace"),
        config_path: tmp.path().join("config.toml"),
        ..crate::alexander_ai_solutions::config::Config::default()
    };
    cfg.update = crate::alexander_ai_solutions::config::UpdateConfig {
        rpc_mutations_enabled: false,
        ..crate::alexander_ai_solutions::config::UpdateConfig::default()
    };
