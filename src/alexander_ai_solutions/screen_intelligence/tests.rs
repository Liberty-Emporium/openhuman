...
use crate::alexander_ai_solutions::accessibility::{parse_foreground_output, AppContext};
use crate::alexander_ai_solutions::config::{Config, ScreenIntelligenceConfig};
...
fn screen_intelligence_env_lock() -> std::sync::MutexGuard<'static, ()> {
    match crate::alexander_ai_solutions::config::TEST_ENV_LOCK.lock() {
        Ok(guard) => guard,
...
async fn persist_vision_summary_uses_pii_safe_document_key() {
    ...
    assert!(
        crate::alexander_ai_solutions::memory_store::safety::pii::has_likely_pii(&raw_key),
    ...
    assert!(
        !crate::alexander_ai_solutions::memory_store::safety::pii::has_likely_pii(&persisted.key),
