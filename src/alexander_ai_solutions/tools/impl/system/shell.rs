use crate::alexander_ai_solutions::agent::host_runtime::RuntimeAdapter;
use crate::alexander_ai_solutions::javascript::NodeBootstrap;
use crate::alexander_ai_solutions::runtime_python::PythonBootstrap;
use crate::alexander_ai_solutions::security::{
    AuditLogger, CommandExecutionLog, GateDecision, SecurityPolicy,
};
use crate::alexander_ai_solutions::tools::traits::{
    PermissionLevel, Tool, ToolResult, ToolTimeout,
};
...
impl ShellTool {
    ...
    /// [`crate::alexander_ai_solutions::agent::harness::with_action_dir_override`] (an
    ...
    fn effective_action_dir(&self) -> std::path::PathBuf {
        crate::alexander_ai_solutions::agent::harness::current_action_dir_override()
    ...
    /// `1..=3600`. See
    /// [`crate::alexander_ai_solutions::tool_timeout::explicit_call_timeout_duration`].
    fn explicit_timeout(&self, requested: Option<u64>) -> Option<Duration> {
        crate::alexander_ai_solutions::tool_timeout::explicit_call_timeout_duration(
            requested,
            crate::alexander_ai_solutions::tool_timeout::MAX_TIMEOUT_SECS,
        )
    }
}
...
impl ShellTool {
    ...
    pub(crate) async fn run_with_security(
        ...
        if matches!(
            crate::alexander_ai_solutions::agent::harness::current_sandbox_mode(),
            Some(crate::alexander_ai_solutions::agent::harness::definition::SandboxMode::Sandboxed)
        ) {
        ...
        let scratch_dir = crate::alexander_ai_solutions::security::openhuman_scratch_dir();
        ...
        let tool_result = match result {
            Ok(Ok(output)) => {
                ...
                if stdout.len() > MAX_OUTPUT_BYTES {
                    stdout.truncate(crate::alexander_ai_solutions::util::floor_char_boundary(
                        &stdout,
                        MAX_OUTPUT_BYTES,
                    ));
                ...
                if stderr.len() > MAX_OUTPUT_BYTES {
                    stderr.truncate(crate::alexander_ai_solutions::util::floor_char_boundary(
                        &stderr,
                        MAX_OUTPUT_BYTES,
                    ));
    ...
    async fn run_sandboxed(
        ...
        use crate::alexander_ai_solutions::sandbox;

        let config = crate::alexander_ai_solutions::config::RuntimeConfig::default();
        let policy = sandbox::resolve_sandbox_policy(
            crate::alexander_ai_solutions::agent::harness::definition::SandboxMode::Sandboxed,
        ...
        let effective = explicit_timeout.unwrap_or_else(|| {
            Duration::from_secs(
                crate::alexander_ai_solutions::tool_timeout::SANDBOX_UNBOUNDED_CAP_SECS,
            )
        });
        tracing::debug!(
...
mod tests {
    ...
    use crate::alexander_ai_solutions::security::{AutonomyLevel, CommandClass, SecurityPolicy};
    ...
    fn audit_with_tempdir() -> (Arc<AuditLogger>, tempfile::TempDir) {
        use crate::alexander_ai_solutions::config::AuditConfig;
    ...
    async fn shell_emits_audit_line_on_success() {
        use crate::alexander_ai_solutions::security::AuditEvent;
    ...
    async fn shell_does_not_resolve_or_install_node_on_its_own() {
        let node = Arc::new(NodeBootstrap::new(
            crate::alexander_ai_solutions::config::schema::NodeConfig {
                enabled: true,
    ...
    async fn shell_sandboxed_mode_routes_through_sandbox_backend() {
        use crate::alexander_ai_solutions::agent::harness::definition::SandboxMode;
        use crate::alexander_ai_solutions::agent::harness::with_current_sandbox_mode;
