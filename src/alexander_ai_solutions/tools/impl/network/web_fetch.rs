...
use crate::alexander_ai_solutions::config::HttpRequestConfig;
...
impl Tool for WebFetchTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let (snippet, truncated) = if body.len() > max_bytes {
            let cut = crate::alexander_ai_solutions::util::floor_char_boundary(&body, max_bytes);
            (&body[..cut], true)
        } else {
            (body.as_str(), false)
        };
...
mod tests {
    ...
    fn zero_and_none_limits_fall_back_to_defaults() {
        ...
        let defaults = crate::alexander_ai_solutions::config::HttpRequestConfig::default();
