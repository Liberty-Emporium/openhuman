...
use crate::alexander_ai_solutions::config::{PolymarketClobCredentials, PolymarketConfig};
use crate::alexander_ai_solutions::security::policy::ToolOperation;
use crate::alexander_ai_solutions::security::SecurityPolicy;
use crate::alexander_ai_solutions::tools::traits::{Tool, ToolCategory, ToolResult};
use crate::alexander_ai_solutions::wallet::{
    secret_material, status as wallet_status, WalletChain,
};
...
impl PolymarketTool {
    pub fn new(config: &PolymarketConfig, security: Arc<SecurityPolicy>) -> Self {
        ...
        let builder = crate::alexander_ai_solutions::config::apply_runtime_proxy_to_builder(
            builder,
            "tool.polymarket",
        );
    ...
    async fn resolve_signer_and_user(&self, user: Option<String>) -> Result<(LocalWallet, String)> {
        ...
        let mnemonic = crate::alexander_ai_solutions::encryption::rpc::decrypt_secret(
            &config,
...
fn summarize_error_body(body: &str) -> String {
    ...
    if compact.is_empty() {
        ...
        crate::alexander_ai_solutions::util::truncate_with_ellipsis(&compact, MAX_ERROR_BODY_CHARS)
    }
}
