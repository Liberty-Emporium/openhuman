use super::*;
use crate::alexander_ai_solutions::config::{BrowserConfig, Config, MemoryConfig};
use crate::alexander_ai_solutions::credentials::{
    AuthService, APP_SESSION_PROVIDER, DEFAULT_AUTH_PROFILE_NAME,
};
use crate::alexander_ai_solutions::security::AuditLogger;
use crate::alexander_ai_solutions::workflows::types::ToolContent;
...
fn test_memory(tmp: &TempDir) -> Arc<dyn Memory> {
    ...
    Arc::from(
        crate::alexander_ai_solutions::memory_store::create_memory(&mem_cfg, tmp.path()).unwrap(),
    )
}
...
fn integration_test_config(tmp: &TempDir, backend_url: &str) -> Config {
    ...
    cfg.search.engine = crate::alexander_ai_solutions::config::SEARCH_ENGINE_PARALLEL.into();
...
fn integration_tools_for_config(tmp: &TempDir, cfg: &Config) -> Vec<Box<dyn Tool>> {
    ...
    let http = crate::alexander_ai_solutions::config::HttpRequestConfig::default();
    all_tools(
        Arc::new(cfg.clone()),
        &security,
...
fn all_tools_registers_generic_mcp_bridge_tools_when_servers_exist() {
    ...
    cfg.mcp_client
        .servers
        .push(crate::alexander_ai_solutions::config::McpServerConfig {
            name: "docs".into(),
            ...
            timeout_secs: 30,
            auth: crate::alexander_ai_solutions::config::McpAuthConfig::None,
        });
...
fn all_tools_registers_brave_engine_lsp_and_tool_stats_when_enabled() {
    ...
    let http = crate::alexander_ai_solutions::config::HttpRequestConfig::default();
    let mut cfg = test_config(&tmp);
    cfg.search.engine = crate::alexander_ai_solutions::config::SEARCH_ENGINE_BRAVE.into();
    cfg.search.brave.api_key = Some("test-brave-key".into());
    cfg.learning.enabled = true;
    cfg.learning.tool_tracking_enabled = true;

    let _env_guard = crate::alexander_ai_solutions::config::TEST_ENV_LOCK
        .lock()
        .unwrap_or_else(|e| e.into_inner());
    unsafe {
        std::env::set_var(
            crate::alexander_ai_solutions::tools::implementations::LSP_ENABLED_ENV,
            "1",
        );
    }
...
fn all_tools_registers_querit_engine_when_enabled() {
    ...
    let http = crate::alexander_ai_solutions::config::HttpRequestConfig::default();
    let mut cfg = test_config(&tmp);
    cfg.search.engine = crate::alexander_ai_solutions::config::SEARCH_ENGINE_QUERIT.into();
...
fn all_tools_omits_search_surface_when_search_is_disabled() {
    ...
    let mut cfg = test_config(&tmp);
    cfg.api_url = Some("https://backend.example.test".to_string());
    cfg.search.engine = crate::alexander_ai_solutions::config::SEARCH_ENGINE_DISABLED.into();
    cfg.search.brave.api_key = Some("test-brave-key".into());
    cfg.search.querit.api_key = Some("test-querit-key".into());
    cfg.integrations.tinyfish.enabled = true;
    store_test_session_token(&cfg);
...
async fn readonly_acting_tools_carry_policy_blocked_marker() {
    use crate::alexander_ai_solutions::security::{AutonomyLevel, POLICY_BLOCKED_MARKER};
