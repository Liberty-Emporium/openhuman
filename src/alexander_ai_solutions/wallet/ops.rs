...
fn keychain_load_mnemonic(config: &Config) -> Option<String> {
    let policy = crate::alexander_ai_solutions::keyring_consent::policy::check_secret_access();
    if policy != crate::alexander_ai_solutions::keyring_consent::PolicyDecision::Proceed
        || !crate::alexander_ai_solutions::keyring::is_available()
    {
    ...
    match crate::alexander_ai_solutions::keyring::get(&user_id, KEYCHAIN_MNEMONIC_KEY) {
        Ok(Some(val)) => {
...
fn keychain_save_mnemonic(config: &Config, encrypted_mnemonic: &str) -> bool {
    ...
    let user_id = wallet_user_id(config);
    match crate::alexander_ai_solutions::keyring::set(
        &user_id,
...
fn keychain_has_mnemonic(config: &Config) -> bool {
    ...
    let user_id = wallet_user_id(config);
    matches!(
        crate::alexander_ai_solutions::keyring::get(&user_id, KEYCHAIN_MNEMONIC_KEY),
        Ok(Some(_))
    )
}
...
pub async fn reveal_recovery_phrase() -> Result<RpcOutcome<RevealRecoveryPhraseResult>, String> {
    ...
    let phrase =
        crate::alexander_ai_solutions::credentials::ops::decrypt_secret(&config, &ciphertext)
            .await
            .map_err(|e| {
...
mod tests {
    ...
    async fn reveal_recovery_phrase_returns_error_when_no_wallet() {
        ...
        let _wallet_lock = crate::alexander_ai_solutions::wallet::test_support::TEST_LOCK.lock();
        let _workspace_guard =
            crate::alexander_ai_solutions::wallet::test_support::set_workspace_env_for_test(&temp);
    ...
    async fn reveal_recovery_phrase_returns_phrase_for_existing_wallet() {
        ...
        let _wallet_lock = crate::alexander_ai_solutions::wallet::test_support::TEST_LOCK.lock();
        let _workspace_guard =
            crate::alexander_ai_solutions::wallet::test_support::setup_wallet_in(&temp)
                .await
                .expect("setup wallet");
        ...
        assert_eq!(
            result.value.phrase,
            crate::alexander_ai_solutions::wallet::test_support::TEST_MNEMONIC
        );
