...
use crate::alexander_ai_solutions::memory_queue::types::{Job, JobKind, JobStatus, NewJob};
...
pub(crate) fn enqueue_conn(conn: &Connection, job: &NewJob) -> Result<Option<String>> {
    ...
    if inserted == 0 {
        log::debug!(
            "[memory::jobs] enqueue suppressed by dedupe kind={} key={:?}",
            job.kind.as_str(),
            job.dedupe_key
        );
        return Ok(None);
    ...
    log::debug!(
        "[memory::jobs] enqueued id={} kind={} avail_at_ms={} dedupe={:?}",
        id,
        job.kind.as_str(),
        available_at,
        job.dedupe_key
...
pub fn claim_next(config: &Config, lock_duration_ms: i64) -> Result<Option<Job>> {
    with_connection(config, |conn| {
        ...
        if let Some(j) = &row {
            log::debug!(
                "[memory::jobs] claimed id={} kind={} attempt={}/{}",
                j.id,
                j.kind.as_str(),
                j.attempts,
                j.max_attempts
...
pub fn mark_done(config: &Config, job: &Job) -> Result<()> {
    ...
    with_connection(config, |conn| {
        ...
        if n == 0 {
            ...
            log::warn!(
                "[memory::jobs] mark_done id={job_id} was a no-op \
...
pub fn mark_failed_typed(
    ...
    job: &Job,
    error: &str,
    failure: Option<&crate::alexander_ai_solutions::memory_tree::health::PipelineFailure>,
) -> Result<()> {
    let job_id = &job.id;
    ...
    with_connection(config, |conn| {
        ...
        if attempts >= max_attempts || unrecoverable {
            ...
            if n == 0 {
                log::warn!(
                    "[memory::jobs] mark_failed(terminal) id={job_id} was a no-op \
            ...
            log::info!(
                "[memory::jobs] retry id={job_id} attempt={attempts}/{max_attempts} \
            ...
            if n == 0 {
                log::warn!(
                    "[memory::jobs] mark_failed(retry) id={job_id} was a no-op \
                     (stale lease: attempts={attempts} started_at_ms={claim_started_at:?})"
                );
            }
        }
...
pub fn mark_deferred(config: &Config, job: &Job, until_ms: i64, reason: &str) -> Result<()> {
    ...
    with_connection(config, |conn| {
        ...
        if n == 0 {
            log::warn!(
                "[memory::jobs] mark_deferred id={job_id} was a no-op \
                 (stale lease: attempts={claim_attempts} started_at_ms={claim_started_at:?})"
            );
        }
        Ok(())
...
mod tests {
    ...
    fn mark_failed_typed_unrecoverable_terminates_immediately() {
        use crate::alexander_ai_solutions::memory_tree::health::{FailureCode, PipelineFailure};
        let (_tmp, cfg) = test_config();
