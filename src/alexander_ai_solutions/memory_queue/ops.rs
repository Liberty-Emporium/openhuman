...
//! `crate::alexander_ai_solutions::memory_queue::<fn>`.
...
pub fn ensure_reembed_backfill(config: &crate::alexander_ai_solutions::config::Config) {
    let sig =
        crate::alexander_ai_solutions::memory_store::chunks::store::tree_active_signature(config);
    let result = crate::alexander_ai_solutions::memory_store::chunks::store::with_connection(
        config,
        |conn| {
            Ok(crate::alexander_ai_solutions::memory_store::chunks::store::has_uncovered_reembed_work(conn, &sig)?)
        },
    );
    match result {
        Ok(true) => {
