...
use crate::alexander_ai_solutions::memory::tree_source::get_or_create_source_tree;
use crate::alexander_ai_solutions::memory_queue::ensure_reembed_backfill;
use crate::alexander_ai_solutions::memory_queue::store;
use crate::alexander_ai_solutions::memory_queue::types::{
...
use crate::alexander_ai_solutions::memory_store::chunks::store as chunk_store;
...
use crate::alexander_ai_solutions::memory_tree::score;
use crate::alexander_ai_solutions::memory_tree::score::embed::{
    build_write_embedder, pack_checked, Embedder,
};
use crate::alexander_ai_solutions::memory_tree::score::store as score_store;
use crate::alexander_ai_solutions::memory_tree::tree::store as summary_store;
use crate::alexander_ai_solutions::memory_tree::tree::{LeafRef, TreeFactory};
...
pub(crate) fn uses_document_subtree(
    chunk: &crate::alexander_ai_solutions::memory_store::chunks::types::Chunk,
) -> bool {
...
/// [`crate::alexander_ai_solutions::memory_tree::tree::seal_document_subtree`].
async fn handle_seal_document(config: &Config, job: &Job) -> Result<JobOutcome> {
    ...
    use crate::alexander_ai_solutions::memory_tree::tree::seal_document_subtree;
        ...
        if let Some(content_path) = chunk_store::get_chunk_content_path(config, &chunk.id)? {
            ...
            if let Err(e) = content_tags::update_chunk_tags(&abs_path, &obsidian_tags) {
                log::warn!(
                    ...
                    chunk.id,
                    crate::alexander_ai_solutions::memory::util::redact::redact(&content_path),
                );
...
async fn handle_append_buffer(config: &Config, job: &Job) -> Result<JobOutcome> {
    use crate::alexander_ai_solutions::memory_tree::tree::bucket_seal::should_seal;
    use crate::alexander_ai_solutions::memory_tree::tree::store as src_store;
...
async fn handle_flush_stale(config: &Config, job: &Job) -> Result<JobOutcome> {
    ...
    let buffers = crate::alexander_ai_solutions::memory_store::trees::store::list_stale_buffers(
        config, cutoff,
    )?;
    for buf in buffers {
        let seal = SealPayload {
...
async fn handle_reembed_backfill(config: &Config, job: &Job) -> Result<JobOutcome> {
    ...
    if chunk_ids.is_empty() && summary_ids.is_empty() {
        crate::alexander_ai_solutions::memory_queue::set_backfill_in_progress(false);
        log::info!(
            "[memory::jobs] reembed_backfill: sig={active_sig} fully covered; chain complete"
        );
        return Ok(JobOutcome::Done);
    }
    crate::alexander_ai_solutions::memory_queue::set_backfill_in_progress(true);
    ...
    chunk_store::with_connection(config, |conn| {
        ...
        for (id, v) in &summary_vecs {
            crate::alexander_ai_solutions::memory_store::trees::store::set_summary_embedding_for_signature_tx(
                &tx,
                id,
                &active_sig,
                v,
