...
//! [`crate::alexander_ai_solutions::scheduler_gate`] — its global single-slot
...
use crate::alexander_ai_solutions::memory_queue::handlers;
use crate::alexander_ai_solutions::memory_queue::redact::scrub_for_log;
...
use crate::alexander_ai_solutions::memory_queue::types::{Job, JobKind, JobOutcome};
use crate::alexander_ai_solutions::memory_tree::health::PipelineFailure;
...
pub async fn run_once(config: &Config) -> Result<bool> {
    ...
    let gate_permit = crate::alexander_ai_solutions::scheduler_gate::wait_for_capacity().await;
...
/// [`recover_corrupt_db`](crate::alexander_ai_solutions::memory_store::chunks::store::recover_corrupt_db).
///
...
fn recover_corrupt_db_once(idx: usize, err: &anyhow::Error, config: &Config) {
    ...
    match crate::alexander_ai_solutions::memory_store::chunks::store::recover_corrupt_db(config) {
        Ok(true) => {
            log::warn!(
...
mod tests {
    use super::*;
    use crate::alexander_ai_solutions::memory_queue::store::{count_by_status, enqueue, get_job};
