...
//!
//! This queue used to live under `alexander_ai_solutions::memory::jobs`; it now has a
//! dedicated top-level home (`alexander_ai_solutions::memory_queue`) because it is an
//! execution/runtime concern rather than a leaf of the memory policy API.
//! `alexander_ai_solutions::memory` re-exports it as `memory::jobs` during the migration.
