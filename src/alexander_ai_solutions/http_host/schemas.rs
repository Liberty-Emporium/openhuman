...
fn handle_start(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        crate::alexander_ai_solutions::http_host::rpc::start(params)
            .await?
...
fn handle_stop(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        crate::alexander_ai_solutions::http_host::rpc::stop(params)
            .await?
            .into_cli_compatible_json()
    })
}
...
fn handle_get(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let params: HostedDirLookupParams = parse(params)?;
        crate::alexander_ai_solutions::http_host::rpc::get(params)
            .await?
            .into_cli_compatible_json()
    })
}
...
fn handle_list(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        crate::alexander_ai_solutions::http_host::rpc::list()
            .await?
            .into_cli_compatible_json()
    })
}
