use super::*;
use crate::alexander_ai_solutions::config::schema::{StreamMode, TelegramConfig};
...
fn user_openhuman_dir_builds_correct_path() {
    let root = PathBuf::from("/home/test/.alexanderai");
    let dir = user_openhuman_dir(&root, "user-123");
    assert_eq!(dir, PathBuf::from("/home/test/.alexanderai/users/user-123"));
}
...
fn pre_login_user_dir_is_under_users_tree() {
    ...
    assert_eq!(
        dir,
        PathBuf::from("/home/test/.alexanderai/users").join(PRE_LOGIN_USER_ID)
    );
}
...
fn searxng_timeout_seconds_alias_deserializes() {
    let cfg: crate::alexander_ai_solutions::config::SearxngConfig =
...
fn resolve_config_dir_for_workspace_returns_parent_and_workspace() {
    let ws = PathBuf::from("/home/test/.alexanderai/workspace");
    let (config_dir, workspace_dir) = resolve_config_dir_for_workspace(&ws);
    // Config dir is the parent of workspace.
    assert!(
        config_dir.ends_with(".alexanderai")
            || config_dir == PathBuf::from("/home/test/.alexanderai")
    );
...
fn env_overlay_learning_source_values_and_invalid_ignored() {
    ...
    assert_eq!(
        ...
        crate::alexander_ai_solutions::config::ReflectionSource::Local
    );
...
fn env_overlay_dictation_activation_mode_only_toggle_or_push() {
    ...
    assert_eq!(
        ...
        crate::alexander_ai_solutions::config::DictationActivationMode::Toggle
    );
    ...
    assert_eq!(
        cfg.dictation.activation_mode,
        crate::alexander_ai_solutions::config::DictationActivationMode::Push
    );
...
fn env_overlay_context_tool_result_budget_env_suppresses_legacy_migration() {
    ...
    // the default.
    let default_budget = crate::alexander_ai_solutions::context::DEFAULT_TOOL_RESULT_BUDGET_BYTES;
    let mut cfg = Config::default();
...
fn env_overlay_auto_update_restart_strategy_accepts_supported_values() {
    ...
    assert_eq!(
        ...
        crate::alexander_ai_solutions::config::UpdateRestartStrategy::Supervisor
    );
    ...
    assert_eq!(
        cfg.update.restart_strategy,
        crate::alexander_ai_solutions::config::UpdateRestartStrategy::SelfReplace
    );
}
...
fn apply_env_overrides_commits_side_effects_to_runtime_proxy() {
    use crate::alexander_ai_solutions::config::schema::proxy::{
...
async fn load_or_init_falls_back_to_defaults_when_backup_also_corrupted() {
    ...
    assert_eq!(
        config.default_model.as_deref(),
        Some(crate::alexander_ai_solutions::config::schema::DEFAULT_MODEL),
...
fn migrate_cloud_provider_slugs_routes_cloud_to_legacy_custom_when_primary_is_openhuman() {
    ...
    cfg.cloud_providers = vec![
        crate::alexander_ai_solutions::config::schema::CloudProviderCreds {
            id: "p_oh".into(),
            slug: "openhuman".into(),
            label: "OpenHuman".into(),
            endpoint: "https://api.openhuman.ai/v1".into(),
            auth_style: crate::alexander_ai_solutions::config::schema::AuthStyle::OpenhumanJwt,
            ..Default::default()
        },
        crate::alexander_ai_solutions::config::schema::CloudProviderCreds {
            id: "p_custom".into(),
            ...
            auth_style: crate::alexander_ai_solutions::config::schema::AuthStyle::Bearer,
