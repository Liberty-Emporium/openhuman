...
impl Config {
    ...
    pub(super) fn apply_env_overrides_from(
        ...
        crate::alexander_ai_solutions::embeddings::rate_limit::set_embedding_rate_limit(
    ...
    fn apply_runtime_env<E: super::env::EnvLookup + ?Sized>(&mut self, env: &E) {
        if let Some(tier_str) = env.get("OPENHUMAN_LOCAL_AI_TIER") {
            ...
            if !tier_str.is_empty() {
                if let Some(tier) =
                    crate::alexander_ai_solutions::inference::presets::ModelTier::from_str_opt(
                        &tier_str,
                    )
                {
                    if tier == crate::alexander_ai_solutions::inference::presets::ModelTier::Custom
                    {
                        tracing::warn!(
                            tier = %tier_str,
    ...
    fn apply_memory_tree_env<E: super::env::EnvLookup + ?Sized>(&mut self, env: &E) {
        ...
        if let Some(raw) = env.get("OPENHUMAN_MEMORY_TREE_LLM_BACKEND") {
            let trimmed = raw.trim();
            if !trimmed.is_empty() {
                match crate::alexander_ai_solutions::config::LlmBackend::parse(trimmed) {
                    Ok(b) => {
                        log::debug!(
    ...
    fn apply_context_env<E: super::env::EnvLookup + ?Sized>(&mut self, env: &E) {
        ...
        let context_default =
            crate::alexander_ai_solutions::context::DEFAULT_TOOL_RESULT_BUDGET_BYTES;
