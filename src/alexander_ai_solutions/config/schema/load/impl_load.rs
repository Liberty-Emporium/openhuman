...
impl Config {
    ...
    pub(crate) async fn load_or_init_with_env_lookup(
        ...
        if config_path.exists() {
            ...
            let contents = crate::alexander_ai_solutions::util::retry_with_backoff_async(
                "read config file",
                5,
                20,
                || async {
            ...
            crate::alexander_ai_solutions::migrations::run_pending(&mut config).await;
            ...
            let mut config = Config {
                ...
                workspace_dir,
                action_dir: default_action_dir(),
                schema_version: crate::alexander_ai_solutions::migrations::CURRENT_SCHEMA_VERSION,
                ..Default::default()
            };
            config.save().await?;
