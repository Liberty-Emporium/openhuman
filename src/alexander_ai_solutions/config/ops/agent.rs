...
pub struct AutonomySettingsPatch {
    ...
    pub trusted_roots: Option<Vec<crate::alexander_ai_solutions::security::TrustedRoot>>,
...
pub async fn apply_autonomy_settings(
    ...
    use crate::alexander_ai_solutions::security::AutonomyLevel;
    ...
    crate::alexander_ai_solutions::security::live_policy::reload_from(&config.autonomy);
...
/// [`crate::alexander_ai_solutions::tool_timeout`] runtime so subsequent tool calls honour
...
pub async fn apply_agent_settings(
    ...
    use crate::alexander_ai_solutions::tool_timeout::{MAX_TIMEOUT_SECS, MIN_TIMEOUT_SECS};
    ...
    let effective = crate::alexander_ai_solutions::tool_timeout::set_tool_timeout_secs(
...
pub async fn get_agent_settings() -> Result<RpcOutcome<serde_json::Value>, String> {
    let config = load_config_with_timeout().await?;
    crate::alexander_ai_solutions::tool_timeout::set_tool_timeout_secs(
        config.agent.agent_timeout_secs,
    );
    let value = serde_json::json!({
        "agent_timeout_secs": config.agent.agent_timeout_secs,
        "effective_timeout_secs": crate::alexander_ai_solutions::tool_timeout::tool_execution_timeout_secs(),
        "env_override": crate::alexander_ai_solutions::tool_timeout::env_override_active(),
        "min_timeout_secs": crate::alexander_ai_solutions::tool_timeout::MIN_TIMEOUT_SECS,
        "max_timeout_secs": crate::alexander_ai_solutions::tool_timeout::MAX_TIMEOUT_SECS,
    });
...
pub async fn ensure_agent_dirs(config: &mut Config) {
    use crate::alexander_ai_solutions::security::{TrustedAccess, TrustedRoot};

    let projects_dir = crate::alexander_ai_solutions::config::default_projects_dir();
...
fn action_dir_source(config: &Config) -> &'static str {
    if crate::alexander_ai_solutions::config::action_dir_env_override().is_some() {
        "env"
...
pub async fn apply_agent_paths_settings(
    ...
    if let Some(raw) = update.action_dir {
        ...
        config.action_dir =
            crate::alexander_ai_solutions::config::resolve_action_dir(&config.action_dir_override);

        config.save().await.map_err(|e| e.to_string())?;

        crate::alexander_ai_solutions::security::live_policy::set_action_dir(
...
pub async fn apply_activity_level_settings(
    ...
    use crate::alexander_ai_solutions::config::schema::activity_level::AgentActivityLevel;
    ...
    crate::alexander_ai_solutions::scheduler_gate::gate::update_config(gate_cfg);
...
fn memory_sync_settings_value(stored: Option<u64>) -> serde_json::Value {
    ...
    let selected_secs =
        stored.unwrap_or(crate::alexander_ai_solutions::config::DEFAULT_MEMORY_SYNC_INTERVAL_SECS);
    json!({
        "sync_interval_secs": stored,
        "selected_secs": selected_secs,
        "is_manual": is_manual,
        "is_default": is_default,
        "default_secs": crate::alexander_ai_solutions::config::DEFAULT_MEMORY_SYNC_INTERVAL_SECS,
        "presets": crate::alexander_ai_solutions::config::MEMORY_SYNC_INTERVAL_PRESETS_SECS,
    })
}
