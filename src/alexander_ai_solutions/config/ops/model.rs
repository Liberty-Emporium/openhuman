...
pub struct ModelSettingsPatch {
    ...
    pub model_routes: Option<Vec<crate::alexander_ai_solutions::config::ModelRouteConfig>>,
    ...
    /// `auth-profiles.json` via [`crate::alexander_ai_solutions::credentials::AuthService`]).
    /// Pass `Some(vec![])` to clear all third-party cloud providers.
    pub cloud_providers: Option<
        Vec<crate::alexander_ai_solutions::config::schema::cloud_providers::CloudProviderCreds>,
    >,
    ...
    pub model_registry:
        Option<Vec<crate::alexander_ai_solutions::config::schema::ModelRegistryEntry>>,
...
pub struct MemorySettingsPatch {
    ...
    /// [`crate::alexander_ai_solutions::config::schema::agent::MemoryContextWindow`]).
...
pub async fn apply_model_settings(
    ...
    if let Some(model) = update.default_model {
        ...
        if let Some(ref m) = config.default_model {
            if !crate::alexander_ai_solutions::inference::provider::factory::is_known_openhuman_tier(
                m,
            ) {
                log::warn!(
    ...
    if let Some(providers) = update.cloud_providers {
        use crate::alexander_ai_solutions::config::schema::cloud_providers::is_slug_reserved;
    ...
    crate::alexander_ai_solutions::memory_queue::ensure_reembed_backfill(config);
...
pub async fn apply_memory_settings(
    ...
    if let Some(model) = update.embedding_model {
        ...
        if let Some(reason) =
            crate::alexander_ai_solutions::embeddings::non_embedding_model_reason(&model)
        {
    ...
    if let Some(window_label) = update.memory_window.as_deref() {
        if let Some(window) =
            crate::alexander_ai_solutions::config::schema::MemoryContextWindow::from_str_opt(
                window_label,
            )
        {
...
pub async fn apply_local_ai_settings(
    ...
    if let Some(provider) = update.provider {
        config.local_ai.provider =
            crate::alexander_ai_solutions::inference::local::provider::normalize_provider(
                &provider,
            );
    }
    if let Some(base_url) = update.base_url {
        config.local_ai.base_url = match base_url {
            ...
            Some(base_url)
                if crate::alexander_ai_solutions::inference::local::provider::normalize_provider(
                    &config.local_ai.provider,
                ) != "omlx"
                    && crate::alexander_ai_solutions::inference::local::provider::provider_from_config(config)
                        == crate::alexander_ai_solutions::inference::local::provider::LocalAiProvider::Ollama =>
            {
                Some(crate::alexander_ai_solutions::inference::local::validate_ollama_url(
                    &base_url,
                )?)
            }
