...
fn seed_and_enrich_model_registry(config: &mut Config) {
    use crate::alexander_ai_solutions::cost::catalog;
...
/// Returns the default workspace directory fallback (~/.alexanderai/workspace).
...
/// Returns the default OpenHuman configuration directory (~/.alexanderai).
...
pub(crate) fn env_scoped_fallback_root_dir() -> PathBuf {
    ...
    PathBuf::from(format!(".alexanderai{suffix}"))
}
...
///
/// Removes the current user's data directory (`~/.alexanderai/users/<id>`) plus
...
pub fn client_config_json(config: &Config) -> serde_json::Value {
    ...
    serde_json::json!({
        ...
        "credits_bypass": {
            "chat": crate::alexander_ai_solutions::inference::provider::factory::role_bypasses_managed_credits(
                "chat", config,
            ),
            "reasoning":
                crate::alexander_ai_solutions::inference::provider::factory::role_bypasses_managed_credits(
                    "reasoning", config,
                ),
        },
...
pub fn agent_server_status() -> RpcOutcome<serde_json::Value> {
    let running =
        crate::alexander_ai_solutions::service::mock::mock_agent_running().unwrap_or(true);
...
pub async fn get_dashboard_settings() -> Result<RpcOutcome<serde_json::Value>, String> {
    ...
    tracing::debug!(
        target: "alexander_ai_solutions_core::config",
    ...
    fn backfills_existing_entries_but_preserves_user_values_and_count() {
        let mut config = Config {
            model_registry: vec![
                ...
                crate::alexander_ai_solutions::config::schema::ModelRegistryEntry {
...
mod loader_io_chain_tests {
    ...
    async fn load_config_with_timeout_rejects_directory_config() {
        ...
        let _g = crate::alexander_ai_solutions::config::TEST_ENV_LOCK
            .lock()
