...
async fn reset_local_data_removes_active_user_and_markers_only() {
    ...
    // real layout (`~/.alexanderai/users/<id>`).
    ...
    let user_marker =
        crate::alexander_ai_solutions::config::active_user_marker_path(&default_openhuman_dir);
...
use crate::alexander_ai_solutions::config::TEST_ENV_LOCK as ENV_LOCK;
...
fn default_openhuman_dir_ends_in_dot_openhuman() {
    ...
    assert!(p.ends_with(".alexanderai"));
}
...
fn reset_local_data_remove_error_explains_windows_file_locks() {
    ...
    let msg =
        reset_local_data_remove_error(std::path::Path::new("C:\\Users\\me\\.alexanderai"), &err);
...
async fn apply_memory_sync_settings_reset_to_default() {
    ...
    assert_eq!(
        ...
        crate::alexander_ai_solutions::config::DEFAULT_MEMORY_SYNC_INTERVAL_SECS
    );
}
...
async fn apply_search_settings_accepts_disabled_engine() {
    ...
    assert_eq!(
        ...
        crate::alexander_ai_solutions::config::SearchEngine::Disabled
    );
}
...
async fn apply_model_settings_replaces_model_routes_when_some_and_keeps_when_none() {
    ...
    use crate::alexander_ai_solutions::config::ModelRouteConfig;
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
...
async fn apply_model_settings_replaces_model_registry_when_some_and_keeps_when_none() {
    ...
    use crate::alexander_ai_solutions::config::schema::ModelRegistryEntry;
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
...
async fn apply_memory_settings_updates_all_provided_fields() {
    ...
    assert_eq!(
        ...
        Some(crate::alexander_ai_solutions::config::schema::MemoryContextWindow::Extended)
    );
}
...
async fn apply_memory_settings_ignores_unknown_memory_window_label() {
    ...
    let mut cfg = tmp_config(&tmp);
    cfg.agent.memory_window =
        Some(crate::alexander_ai_solutions::config::schema::MemoryContextWindow::Balanced);
...
async fn apply_memory_settings_round_trips_all_window_labels() {
    use crate::alexander_ai_solutions::config::schema::MemoryContextWindow;
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
...
async fn apply_meet_settings_updates_all_meeting_assistant_fields() {
    use crate::alexander_ai_solutions::config::{AutoJoinPolicy, AutoSummarizePolicy};
    let tmp = tempdir().unwrap();
    let mut cfg = tmp_config(&tmp);
    // Defaults (issue #3511).
...
async fn apply_agent_settings_updates_timeout_and_persists_snapshot() {
    ...
    assert_eq!(
        crate::alexander_ai_solutions::tool_timeout::tool_execution_timeout_secs(),
        300
    );
}
...
async fn apply_agent_paths_empty_input_clears_override() {
    ...
    assert_eq!(
        cfg.action_dir,
        crate::alexander_ai_solutions::config::default_projects_dir()
    );
    assert_eq!(
        outcome.value["action_dir_source"],
...
async fn ensure_agent_dirs_creates_missing_action_dir_and_trusted_root() {
    use crate::alexander_ai_solutions::security::TrustedAccess;
...
fn ensure_usable_cwd_creates_missing_dir() {
    ...
    assert!(!dir.exists());
    crate::alexander_ai_solutions::config::ensure_usable_cwd(&dir).expect("missing dir is created");
    assert!(dir.is_dir());
}
...
fn ensure_usable_cwd_rejects_a_file_with_descriptive_error() {
    ...
    let err = crate::alexander_ai_solutions::config::ensure_usable_cwd(&file)
...
fn ensure_usable_cwd_errors_when_uncreatable() {
    ...
    let err = crate::alexander_ai_solutions::config::ensure_usable_cwd(&target)
