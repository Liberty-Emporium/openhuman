...
fn deserialize_params_parses_autonomy_update_with_trusted_roots() {
    ...
    assert_eq!(
        roots[0].access,
        crate::alexander_ai_solutions::security::TrustedAccess::ReadWrite
    );
    // `access` defaults to Read when omitted.
    assert_eq!(
        roots[1].access,
        crate::alexander_ai_solutions::security::TrustedAccess::Read
    );
}
...
async fn handle_get_autonomy_settings_returns_current_value() {
    ...
    let _ = crate::alexander_ai_solutions::config::ops::load_and_apply_autonomy_settings(
        crate::alexander_ai_solutions::config::ops::AutonomySettingsPatch {
...
async fn handle_get_agent_paths_returns_action_workspace_and_projects() {
    ...
    // `~/OpenHuman/projects` / `~/.alexanderai/workspace` strings that drift
