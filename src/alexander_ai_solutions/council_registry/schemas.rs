...
use crate::alexander_ai_solutions::council_registry::types::CouncilDefinition;
...
fn handle_list(_: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(crate::alexander_ai_solutions::council_registry::list_councils(&config)?)
    })
}
...
fn handle_get(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::alexander_ai_solutions::council_registry::get_council(&config, &p.id)?)
    })
}
...
fn handle_upsert(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(
            crate::alexander_ai_solutions::council_registry::upsert_council(&config, p.council)?,
        )
    })
}
...
fn handle_delete(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let p = deserialize_params::<CouncilIdParams>(params)?;
        let config = config_rpc::load_config_with_timeout().await?;
        to_json(crate::alexander_ai_solutions::council_registry::delete_council(&config, &p.id)?)
    })
}
