use super::*;
use crate::alexander_ai_solutions::agent::hooks::{ToolCallRecord, TurnContext};
use crate::alexander_ai_solutions::memory::chat::ChatPrompt;
use crate::alexander_ai_solutions::memory_store::{events as ev, fts5, segments as seg};
...
use crate::alexander_ai_solutions::memory_store::chunks::store::{
    count_chunks, list_chunks, ListChunksQuery,
};
    ...
    let closed = all_segs
        ...
        .find(|s| {
            s.session_id == session
                && s.status
                    != crate::alexander_ai_solutions::memory_store::segments::SegmentStatus::Open
        })
...
#[async_trait::async_trait]
impl crate::alexander_ai_solutions::memory_tree::score::embed::Embedder for PanicOnEmbedEmbedder {
    fn name(&self) -> &'static str {
        "panic-embedder-v1"
    }
