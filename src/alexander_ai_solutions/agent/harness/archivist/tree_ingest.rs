...
use crate::alexander_ai_solutions::memory_store::fts5;
...
impl ArchivistHook {
    ...
    pub(super) async fn pipe_segment_to_tree(
        &self,
        config: &Config,
        segment: &crate::alexander_ai_solutions::memory_store::segments::ConversationSegment,
        session_id: &str,
        ...
        let messages: Vec<ChatMessage> = entries
            ...
            .filter_map(|e| {
                ...
                let (text, _image_refs) =
                    crate::alexander_ai_solutions::agent::multimodal::parse_image_markers(
                        &raw_text,
                    );
                if text.is_empty() {
                    return None;
