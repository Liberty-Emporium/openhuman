...
use crate::alexander_ai_solutions::memory::chat::ChatProvider;
use crate::alexander_ai_solutions::memory_store::events::{self, EventRecord, EventType};
use crate::alexander_ai_solutions::memory_store::fts5::EpisodicEntry;
use crate::alexander_ai_solutions::memory_store::profile::{self, FacetType};
...
use crate::alexander_ai_solutions::memory_tree::score::embed::{
    build_embedder_from_config, Embedder,
};
...
impl ArchivistHook {
    ...
    pub fn with_config(mut self, config: Config) -> Self {
        ...
        let chat_provider: Option<Arc<dyn ChatProvider>> =
            match crate::alexander_ai_solutions::memory::chat::build_chat_provider(&config) {
                Ok(p) => {
    ...
    pub(super) async fn on_segment_closed(
        ...
        if let Some(ref cfg) = self.config {
            if cfg.learning.goals_enrichment_enabled && !summary.trim().is_empty() {
                ...
                crate::alexander_ai_solutions::memory_goals::spawn_enrich_goals(
                    cfg.clone(),
