...
impl ArchivistHook {
    ...
    pub(super) fn read_session_entries(
        ...
        if let Some(cfg) = self.config.as_ref() {
            match crate::alexander_ai_solutions::memory_archivist::store::session_entries(
                cfg, session_id,
            ) {
                Ok(turns) => {
                    return turns
    ...
    pub(super) async fn summarize_entries(
        ...
        let corpus_inputs: Vec<SummaryInput> = entries
            ...
            .map(|e| {
                use crate::alexander_ai_solutions::memory_store::chunks::types::approx_token_count;
        ...
        if self.chat_provider.is_some() {
            if let Some(ref config) = self.config {
                ...
                let summary_result = if let Some(provider) = self.chat_provider.as_ref() {
                    crate::alexander_ai_solutions::memory::chat::test_override::with_provider(
                        Arc::clone(provider),
    ...
    pub async fn rolling_segment_recap(&self, session_id: &str) -> Option<String> {
        ...
        let open_segment =
            match crate::alexander_ai_solutions::memory_store::segments::open_segment_for_session(
                conn, session_id,
            ) {
                Ok(Some(seg)) => seg,
                Ok(None) => {
