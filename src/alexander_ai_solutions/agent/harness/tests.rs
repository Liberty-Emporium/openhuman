...
impl Provider for VisionProvider {
    ...
    async fn chat(
        ...
        self.calls.fetch_add(1, Ordering::SeqCst);
        let marker_count =
            crate::alexander_ai_solutions::agent::multimodal::count_image_markers(request.messages);
        if marker_count == 0 {
...
async fn run_tool_call_loop_rejects_oversized_image_payload() {
    ...
    let tools_registry: Vec<Box<dyn Tool>> = Vec::new();
    let multimodal = crate::alexander_ai_solutions::config::MultimodalConfig {
        max_images: 4,
        max_image_size_mb: 1,
...
fn build_tool_instructions_includes_all_tools() {
    ...
    let security = Arc::new(SecurityPolicy::from_config(
        &crate::alexander_ai_solutions::config::AutonomyConfig::default(),
