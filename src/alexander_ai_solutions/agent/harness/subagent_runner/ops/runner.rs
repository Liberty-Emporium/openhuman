...
use crate::alexander_ai_solutions::agent::harness::fork_context::{
    current_parent, ParentExecutionContext,
};
use crate::alexander_ai_solutions::agent::harness::subagent_runner::extract_tool::ExtractFromResultTool;
use crate::alexander_ai_solutions::agent::harness::subagent_runner::handoff::ResultHandoffCache;
use crate::alexander_ai_solutions::agent::harness::subagent_runner::tool_prep::{
    filter_tool_indices, is_subagent_spawn_tool, load_prompt_source, top_k_for_toolkit,
};
use crate::alexander_ai_solutions::agent::harness::subagent_runner::types::{
    SubagentMode, SubagentRunError, SubagentRunOptions, SubagentRunOutcome,
};
use crate::alexander_ai_solutions::agent::harness::{
    current_spawn_depth, with_current_sandbox_mode, with_spawn_depth, MAX_SPAWN_DEPTH,
};
use crate::alexander_ai_solutions::context::prompt::{
    render_subagent_system_prompt, PromptContext, PromptTool, SubagentRenderOptions,
};
use crate::alexander_ai_solutions::file_state::with_file_state_agent_id;
use crate::alexander_ai_solutions::inference::provider::AGENT_TURN_MAX_OUTPUT_TOKENS;
use crate::alexander_ai_solutions::tools::{Tool, ToolCategory, ToolSpec};
            ...
            with_file_state_agent_id(task_id.clone(), async {
                with_current_sandbox_mode(definition.sandbox_mode, async {
                    ...
                    match worktree_action_dir {
                        Some(wt_dir) => {
                            crate::alexander_ai_solutions::agent::harness::with_action_dir_override(
                                wt_dir,
                                Box::pin(run),
                            )
                            .await
...
async fn run_typed_mode(
    ...
    let config_loaded = crate::alexander_ai_solutions::config::Config::load_or_init().await;
    ...
    let live_integrations: Vec<
        crate::alexander_ai_solutions::context::prompt::ConnectedIntegration,
    > = {
        let probe_config = crate::alexander_ai_solutions::config::Config::load_or_init()
            .await
            .ok();
        ...
        if !signed_in {
            ...
            match crate::alexander_ai_solutions::config::Config::load_or_init().await {
                Ok(config) => {
                    use crate::alexander_ai_solutions::composio::FetchConnectedIntegrationsStatus;
                    match crate::alexander_ai_solutions::composio::fetch_connected_integrations_status(&config)
                        .await
                    {
    ...
    if is_integrations_agent_with_toolkit {
        if let Some(tk) = toolkit_filter {
            let arc_config = match crate::alexander_ai_solutions::config::Config::load_or_init()
                .await
            {
            ...
            use crate::alexander_ai_solutions::composio::client::{
                ...
                .find(|ci| ci.connected && ci.toolkit.eq_ignore_ascii_case(tk))
            {
                let fresh_actions = match &client_kind {
                    Some(ComposioClientKind::Backend(client)) => {
                        match crate::alexander_ai_solutions::composio::fetch_toolkit_actions(
                            client, tk, None,
                        )
                        .await
                        {
                ...
                let selected: Vec<
                    &crate::alexander_ai_solutions::context::prompt::ConnectedIntegrationTool,
                ...
                for action in selected {
                    dynamic_tools.push(Box::new(
                        crate::alexander_ai_solutions::composio::ComposioActionTool::new(
                            arc_config.clone(),
                            action.name.clone(),
    ...
    let handoff_cache: Option<Arc<ResultHandoffCache>> = if is_integrations_agent_with_toolkit {
        ...
        let summarization_tier =
            crate::alexander_ai_solutions::inference::provider::factory::summarization_tier_model()
                .to_string();
        let (extract_provider, extract_model): (
            ...
            Ok(cfg) => {
                let route = crate::alexander_ai_solutions::inference::provider::provider_for_role(
                    "summarization",
                    &cfg,
                );
                let r = route.trim();
                ...
                if route_is_managed && !parent.provider.is_local_provider() {
                    ...
                    match crate::alexander_ai_solutions::inference::provider::create_chat_provider(
                        "summarization",
                        &cfg,
                    ) {
                        Ok((p, m)) => (Arc::from(p), m),
    ...
    let filtered_specs =
        crate::alexander_ai_solutions::agent::harness::session::dedup_visible_tool_specs(
            filtered_specs,
        );
    ...
    let dispatcher_instructions = {
        ...
        use crate::alexander_ai_solutions::agent::pformat::PFormatRegistry;
        use crate::alexander_ai_solutions::context::prompt::ToolCallFormat;
    ...
    let prompt_ctx = PromptContext {
        ...
        learned: crate::alexander_ai_solutions::context::prompt::LearnedContextData::default(),
        visible_tool_names: &visible_tool_names,
        tool_call_format: parent.tool_call_format,
        connected_integrations: &narrowed_integrations,
        connected_identities_md:
            crate::alexander_ai_solutions::agent::prompts::render_connected_identities(),
        include_profile: !definition.omit_profile,
        include_memory_md: !definition.omit_memory_md,
        curated_snapshot: None,
        user_identity: crate::alexander_ai_solutions::app_state::peek_cached_current_user_identity(
        ),
    ...
    let now_str = crate::alexander_ai_solutions::agent::prompts::current_datetime_line();
    ...
    let mut history: Vec<crate::alexander_ai_solutions::inference::provider::ChatMessage> =
        if let Some(ref initial) = options.initial_history {
            ...
            vec![
                crate::alexander_ai_solutions::inference::provider::ChatMessage::system(
                    system_prompt,
                ),
                crate::alexander_ai_solutions::inference::provider::ChatMessage::user(user_message),
            ]
        };
    ...
    let model_vision = crate::alexander_ai_solutions::config::Config::load_or_init()
        .await
        .ok()
        .map(|cfg| {
    ...
    let status = if early_exit_tool.as_deref() == Some("ask_user_clarification") {
        ...
        let checkpoint_dir = options.checkpoint_dir.clone().unwrap_or_else(|| {
            parent
                .workspace_dir
                .join(".alexanderai/subagent_checkpoints")
        });
        if let Err(e) = std::fs::create_dir_all(&checkpoint_dir) {
            ...
            let checkpoint_data =
                crate::alexander_ai_solutions::agent::harness::subagent_runner::types::SubagentCheckpointData {
        ...
        crate::alexander_ai_solutions::agent::harness::subagent_runner::types::SubagentRunStatus::AwaitingUser {
            question,
            options: options_vec,
        }
    } else {
        crate::alexander_ai_solutions::agent::harness::subagent_runner::types::SubagentRunStatus::Completed
    };
    ...
    let usage =
        crate::alexander_ai_solutions::agent::harness::subagent_runner::types::SubagentUsage {
    ...
    crate::alexander_ai_solutions::agent::harness::turn_subagent_usage::record_subagent_usage(
        task_id,
        &definition.id,
        usage,
    );
