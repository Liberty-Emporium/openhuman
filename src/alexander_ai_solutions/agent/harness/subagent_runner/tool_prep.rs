...
//! [`crate::alexander_ai_solutions::agent::debug`] can mirror the live runner
...
use crate::alexander_ai_solutions::context::prompt::PromptContext;
...
/// [`crate::alexander_ai_solutions::agent::dispatcher::XmlToolDispatcher::prompt_instructions`]
...
///   ([`crate::alexander_ai_solutions::tools::orchestrator_tools::collect_orchestrator_tools`]
...
/// [`crate::alexander_ai_solutions::agent::debug`] shares the exact same
...
pub(crate) fn filter_tool_indices(
    ...
    parent_tools
        ...
        .filter(|(_, tool)| {
            ...
            if crate::alexander_ai_solutions::tokenjuice::is_recovery_tool(name) {
...
mod recovery_visibility_tests {
    use super::*;
    use crate::alexander_ai_solutions::tokenjuice::LEGACY_RETRIEVE_TOOL_NAME as RECOVERY_TOOL_NAME;
    use crate::alexander_ai_solutions::tools::{CurrentTimeTool, RetrieveToolOutputTool};

    fn tools() -> Vec<Box<dyn crate::alexander_ai_solutions::tools::Tool>> {
        vec![
    ...
    fn names(
        idx: &[usize],
        tools: &[Box<dyn crate::alexander_ai_solutions::tools::Tool>],
    ) -> Vec<String> {
...
///
/// Exposed `pub(crate)` so the debug dump path in
/// [`crate::alexander_ai_solutions::agent::debug`] loads prompts through the
    ...
    match source {
        ...
        PromptSource::File { path } => {
            ...
            if workspace_path.is_file() {
                if let Ok(resolved) =
                    crate::alexander_ai_solutions::security::validate_path_within_root(
                        &workspace_path,
                        &prompt_root,
                    )
                {
