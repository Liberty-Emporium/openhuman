use super::*;
use crate::alexander_ai_solutions::agent::harness::definition::{ModelSpec, ToolScope};

#[test]
fn lazy_resolver_tolerates_near_miss_slugs() {
    use crate::alexander_ai_solutions::context::prompt::ConnectedIntegrationTool;
    ...
    let resolver = LazyToolkitResolver {
        config: std::sync::Arc::new(crate::alexander_ai_solutions::config::Config::default()),
...
fn make_def_named_tools(names: &[&str]) -> AgentDefinition {
    AgentDefinition {
        ...
        timeout_secs: None,
        sandbox_mode: crate::alexander_ai_solutions::agent::harness::definition::SandboxMode::None,
        background: false,
        trigger_memory_agent: Default::default(),
        tokenjuice_compression:
            crate::alexander_ai_solutions::tokenjuice::AgentTokenjuiceCompression::Auto,
        subagents: vec![],
        delegate_name: None,
        agent_tier: crate::alexander_ai_solutions::agent::harness::definition::AgentTier::Worker,
        source:
            crate::alexander_ai_solutions::agent::harness::definition::DefinitionSource::Builtin,
    }
}
...
use crate::alexander_ai_solutions::tools::{PermissionLevel, ToolResult};
...
use crate::alexander_ai_solutions::agent::harness::fork_context::with_parent_context;
use crate::alexander_ai_solutions::agent::harness::run_queue::{
    QueueMode, QueuedMessage, RunQueue,
};
...
struct CapturedRequest {
    messages: Vec<crate::alexander_ai_solutions::inference::provider::ChatMessage>,
    tool_count: usize,
    model: String,
}
...
fn make_parent(provider: Arc<dyn Provider>, tools: Vec<Box<dyn Tool>>) -> ParentExecutionContext {
    let tool_specs: Vec<crate::alexander_ai_solutions::tools::ToolSpec> =
    ...
    ParentExecutionContext {
        ...
        memory: noop_memory(),
        agent_config: crate::alexander_ai_solutions::config::AgentConfig::default(),
        workflows: Arc::new(vec![]),
        memory_context: Arc::new(None),
        session_id: "test-session".into(),
        channel: "test".into(),
        connected_integrations: vec![],
        tool_call_format: crate::alexander_ai_solutions::context::prompt::ToolCallFormat::PFormat,
...
fn noop_memory() -> Arc<dyn crate::alexander_ai_solutions::memory::Memory> {
    struct NoopMemory;
    #[async_trait]
    impl crate::alexander_ai_solutions::memory::Memory for NoopMemory {
        async fn store(
            &self,
            _namespace: &str,
            _key: &str,
            _content: &str,
            _category: crate::alexander_ai_solutions::memory::MemoryCategory,
        ...
        async fn recall(
            ...
            _query: &str,
            _limit: usize,
            _opts: crate::alexander_ai_solutions::memory::RecallOpts<'_>,
        ) -> anyhow::Result<Vec<crate::alexander_ai_solutions::memory::MemoryEntry>> {
            Ok(vec![])
        }
        async fn get(
            &self,
            _namespace: &str,
            _key: &str,
        ) -> anyhow::Result<Option<crate::alexander_ai_solutions::memory::MemoryEntry>> {
            Ok(None)
        }
        async fn list(
            &self,
            _namespace: Option<&str>,
            _category: Option<&crate::alexander_ai_solutions::memory::MemoryCategory>,
            _session_id: Option<&str>,
        ) -> anyhow::Result<Vec<crate::alexander_ai_solutions::memory::MemoryEntry>> {
            Ok(vec![])
        }
        ...
        async fn namespace_summaries(
            &self,
        ) -> anyhow::Result<Vec<crate::alexander_ai_solutions::memory::NamespaceSummary>> {
            Ok(vec![])
        }
...
fn resolve_subagent_provider_config_model_wins_over_definition_model() {
    use crate::alexander_ai_solutions::config::{Config, TeamModelConfig};
...
fn direct_mode_user_with_stored_key_passes_signed_in_check() {
    ...
    let mut config = crate::alexander_ai_solutions::config::Config::default();
    ...
    config.composio.mode =
        crate::alexander_ai_solutions::config::schema::COMPOSIO_MODE_DIRECT.to_string();
...
fn tier_gate_skips_when_parent_unresolved() {
    use crate::alexander_ai_solutions::agent::harness::definition::AgentTier;
