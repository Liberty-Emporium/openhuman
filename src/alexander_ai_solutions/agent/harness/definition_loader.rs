...
//! with a fallback to `~/.alexanderai/agents/*.toml` for user-global
...
/// `~/.alexanderai/agents/` fallback. Returns an empty Vec when neither
/// directory exists.
...
/// injected by [`crate::alexander_ai_solutions::agent_registry::agents::load_builtins`], so a
