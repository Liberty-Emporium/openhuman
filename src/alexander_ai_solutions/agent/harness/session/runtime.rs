...
use crate::alexander_ai_solutions::agent::dispatcher::ParsedToolCall;
use crate::alexander_ai_solutions::agent::error::AgentError;
use crate::alexander_ai_solutions::agent_tool_policy::ToolPolicyEngine;
use crate::alexander_ai_solutions::inference::provider::{
    self, ConversationMessage, Provider, ToolCall,
};
...
impl Agent {
    ...
    pub fn workflows(&self) -> &[crate::alexander_ai_solutions::workflows::Workflow] {
        &self.workflows
    }
    ...
    pub fn set_connected_integrations(
        &mut self,
        integrations: Vec<crate::alexander_ai_solutions::context::prompt::ConnectedIntegration>,
    ) {
    ...
    pub fn agent_config(&self) -> &crate::alexander_ai_solutions::config::AgentConfig {
        &self.config
    }
    ...
    pub fn set_on_progress(
        ...
        tx: Option<
            tokio::sync::mpsc::Sender<
                crate::alexander_ai_solutions::agent::progress::AgentProgress,
            >,
        >,
    ) {
        self.on_progress = tx;
    ...
    pub fn set_run_queue(
        &mut self,
        rq: Option<
            std::sync::Arc<crate::alexander_ai_solutions::agent::harness::run_queue::RunQueue>,
        >,
    ) {
        self.run_queue = rq;
    }
    ...
    pub fn seed_resume_from_messages(
        ...
        let learned = crate::alexander_ai_solutions::agent::prompts::LearnedContextData::default();
        let system_prompt = self.build_system_prompt(learned)?;

        let mut cached: Vec<crate::alexander_ai_solutions::inference::provider::ChatMessage> =
            Vec::with_capacity(prior.len() + 1);
        cached.push(
            crate::alexander_ai_solutions::inference::provider::ChatMessage::system(system_prompt),
        );
        for (role, content) in prior {
            let chat = match role.as_str() {
                "user" => {
                    crate::alexander_ai_solutions::inference::provider::ChatMessage::user(content)
                }
                "agent" | "assistant" => {
                    crate::alexander_ai_solutions::inference::provider::ChatMessage::assistant(
                        content,
                    )
                }
                ...
                _ => crate::alexander_ai_solutions::inference::provider::ChatMessage::user(content),
            };
            cached.push(chat);
        }
    ...
    pub fn take_last_turn_citations(
        &mut self,
    ) -> Vec<crate::alexander_ai_solutions::agent::memory_loader::MemoryCitation> {
    ...
    pub(super) fn persisted_tool_calls_for_history(
        response: &crate::alexander_ai_solutions::inference::provider::ChatResponse,
    ...
    pub async fn run_interactive(&mut self) -> Result<()> {
        ...
        let cli = crate::alexander_ai_solutions::channels::CliChannel::new();

        let listen_handle = tokio::spawn(async move {
            let _ = crate::alexander_ai_solutions::channels::Channel::listen(&cli, tx).await;
        });
