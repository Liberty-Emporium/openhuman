...
use crate::alexander_ai_solutions::agent::dispatcher::{
    NativeToolDispatcher, PFormatToolDispatcher, XmlToolDispatcher,
};
use crate::alexander_ai_solutions::agent::harness::definition::{
    AgentDefinitionRegistry, PromptSource, ToolScope,
};
use crate::alexander_ai_solutions::agent::harness::session::types::Agent;
use crate::alexander_ai_solutions::agent::host_runtime;
use crate::alexander_ai_solutions::agent::memory_loader::DefaultMemoryLoader;
use crate::alexander_ai_solutions::config::Config;
use crate::alexander_ai_solutions::context::prompt::SystemPromptBuilder;
...
use crate::alexander_ai_solutions::memory_tools::ToolMemoryCaptureHook;
use crate::alexander_ai_solutions::security::SecurityPolicy;
use crate::alexander_ai_solutions::tools::{self, Tool};
use anyhow::Result;
use std::sync::Arc;
...
impl Agent {
    ...
    pub fn from_config_for_agent(config: &Config, agent_id: &str) -> Result<Self> {
        ...
        let target_def: Option<
            crate::alexander_ai_solutions::agent::harness::definition::AgentDefinition,
    ...
    pub fn from_config_for_agent_with_reflection_chunks(
        config: &Config,
        agent_id: &str,
        reflection_chunks: Vec<crate::alexander_ai_solutions::subconscious::SourceChunk>,
    ) -> Result<Self> {
    ...
    pub fn from_config_for_agent_with_profile(
        config: &Config,
        agent_id: &str,
        reflection_chunks: Option<Vec<crate::alexander_ai_solutions::subconscious::SourceChunk>>,
        profile_prompt_suffix: Option<String>,
        profile: Option<&crate::alexander_ai_solutions::profiles::AgentProfile>,
    ) -> Result<Self> {
        let target_def: Option<
            crate::alexander_ai_solutions::agent::harness::definition::AgentDefinition,
        > = match AgentDefinitionRegistry::global() {
    ...
    fn build_session_agent_inner(
        ...
        let enabled_tools: Vec<String> = {
            use crate::alexander_ai_solutions::app_state::load_stored_app_state;
        ...
        if !enabled_tools.is_empty() {
            crate::alexander_ai_solutions::tools::filter_tools_by_user_preference(
                &mut tools,
                &enabled_tools,
            );
        }
        ...
        let (provider, mut model_name): (Box<dyn Provider>, String) =
            crate::alexander_ai_solutions::inference::provider::create_chat_provider(
                provider_role,
                config,
            )?;
        log::info!(
        ...
        let model_vision =
            crate::alexander_ai_solutions::inference::model_context::model_supports_vision(
                &model_name,
                config,
            );
            ...
            Some(def) => match &def.system_prompt {
                ...
                PromptSource::File { path } => {
                    ...
                    let body_text = if workspace_path.is_file() {
                        match crate::alexander_ai_solutions::security::validate_path_within_root(
                            &workspace_path,
                            &prompt_root,
                        ) {
                            Ok(resolved) => {
        ...
        if config.learning.enabled {
            ...
            prompt_builder = prompt_builder
                .insert_section_before(
                    "user_memory",
                    Box::new(
                        crate::alexander_ai_solutions::context::prompt::UserReflectionsSection,
                    ),
                )
                .add_section(Box::new(
                    crate::alexander_ai_solutions::learning::LearnedContextSection::new(
                        memory.clone(),
                    ),
                ))
                .add_section(Box::new(
                    crate::alexander_ai_solutions::learning::UserProfileSection::new(
                        memory.clone(),
                    ),
                ));
        ...
        if config.learning.explicit_preferences_enabled && !config.learning.enabled {
            prompt_builder = prompt_builder.add_section(Box::new(
                crate::alexander_ai_solutions::learning::UserProfileSection::new(memory.clone()),
            ));
            log::info!(
            ...
            .filter(|s| !s.is_empty())
        {
            ...
            prompt_builder = prompt_builder.add_section(Box::new(
                crate::alexander_ai_solutions::profiles::AgentProfilePromptSection::new(suffix),
            ));
        }

        // Build post-turn hooks when learning is enabled
        let mut post_turn_hooks: Vec<
            Arc<dyn crate::alexander_ai_solutions::agent::hooks::PostTurnHook>,
        > = Vec::new();
        if config.learning.enabled {
            if config.learning.reflection_enabled {
                ...
                let reflection_provider: Option<
                    Arc<dyn crate::alexander_ai_solutions::inference::provider::Provider>,
                > = if config.learning.reflection_source
                    == crate::alexander_ai_solutions::config::ReflectionSource::Cloud
                {
                ...
                post_turn_hooks.push(Arc::new(
                    crate::alexander_ai_solutions::learning::ReflectionHook::new(
            ...
            if config.learning.user_profile_enabled {
                post_turn_hooks.push(Arc::new(
                    crate::alexander_ai_solutions::learning::UserProfileHook::new(
                        config.learning.clone(),
                        memory.clone(),
                    ),
                ));
            ...
            if config.learning.tool_tracking_enabled {
                post_turn_hooks.push(Arc::new(
                    crate::alexander_ai_solutions::learning::ToolTrackerHook::new(
                        config.learning.clone(),
                        memory.clone(),
                    ),
                ));
            ...
            if config.learning.tool_memory_capture_enabled {
                post_turn_hooks.push(Arc::new(
                    crate::alexander_ai_solutions::agent_experience::AgentExperienceCaptureHook::new(
                        memory.clone(),
                        true,
                    ),
                ));
        ...
        let archivist_hook_arc: Option<
            Arc<crate::alexander_ai_solutions::agent::harness::archivist::ArchivistHook>,
        > = if config.learning.episodic_capture_enabled {
            match memory.sqlite_conn() {
                Some(conn) => {
                    let hook = Arc::new(
                        crate::alexander_ai_solutions::agent::harness::archivist::ArchivistHook::new(conn, true)
                            .with_config(config.clone()),
                    );
                    post_turn_hooks.push(Arc::clone(&hook)
                        as Arc<dyn crate::alexander_ai_solutions::agent::hooks::PostTurnHook>);
                    log::info!(
        ...
        let prewarmed_integrations =
            crate::alexander_ai_solutions::composio::cached_active_integrations(config);
        ...
        let prewarmed_integrations = match (
            ...
            (Some(list), Some(allow)) => {
                let filtered = crate::alexander_ai_solutions::profiles::filter_integrations(
                    &list,
                    Some(allow),
                );
                tracing::debug!(
        ...
        let (delegation_tools, filter_from_scope): (
            ...
            target_def,
            crate::alexander_ai_solutions::agent::harness::definition::AgentDefinitionRegistry::global(),
        ) {
            (Some(def), Some(reg)) => {
        ...
        if config.learning.enabled {
            ...
            if has_retrieval {
                prompt_builder = prompt_builder.add_section(Box::new(
                    crate::alexander_ai_solutions::learning::MemoryAccessSection,
                ));
        ...
        let pformat_registry =
            crate::alexander_ai_solutions::agent::pformat::build_registry(&tools);
        let tool_dispatcher: Box<
            dyn crate::alexander_ai_solutions::agent::dispatcher::ToolDispatcher,
        ...
        let effective_tokenjuice_compression = target_def
            ...
            .unwrap_or(crate::alexander_ai_solutions::tokenjuice::AgentTokenjuiceCompression::Full);
        ...
        let payload_summarizer: Option<
            std::sync::Arc<
                dyn crate::alexander_ai_solutions::agent::harness::payload_summarizer::PayloadSummarizer,
            >,
        > = if agent_id == "orchestrator" && config.context.summarizer_payload_threshold_tokens > 0
        {
            match crate::alexander_ai_solutions::agent::harness::definition::AgentDefinitionRegistry::global() {
                Some(reg) => match reg.get("summarizer") {
                    Some(summarizer_def) => {
                        ...
                        Some(std::sync::Arc::new(
                            crate::alexander_ai_solutions::agent::harness::payload_summarizer::SubagentPayloadSummarizer::new(
        ...
        let mut builder = Agent::builder()
            ...
            .workflows(
                crate::alexander_ai_solutions::workflows::load_workflow_metadata(
                    &config.workspace_dir,
                ),
            )
        ...
        agent.last_seen_integrations_hash =
            crate::alexander_ai_solutions::composio::connected_set_hash(
