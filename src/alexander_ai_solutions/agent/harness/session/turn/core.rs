...
use crate::alexander_ai_solutions::agent::harness::fork_context::ParentExecutionContext;
use crate::alexander_ai_solutions::agent::hooks::{self, TurnContext};
use crate::alexander_ai_solutions::agent::memory_loader::collect_recall_citations;
use crate::alexander_ai_solutions::agent::progress::AgentProgress;
use crate::alexander_ai_solutions::agent_experience::{
...
use crate::alexander_ai_solutions::memory::MemoryCategory;
use crate::alexander_ai_solutions::util::truncate_with_ellipsis;
    ...
    pub async fn turn(&mut self, user_message: &str) -> Result<String> {
        ...
        if self.history.is_empty() {
            ...
            self.announced_mcp_servers =
                crate::alexander_ai_solutions::mcp_registry::connections::connected_overview()
                    .await
                    .into_iter()
            ...
            // [`crate::alexander_ai_solutions::composio::INTEGRATIONS_CACHE`], kept
            ...
            // [`crate::alexander_ai_solutions::composio::cached_active_integrations`]
        ...
        {
            let situational =
                crate::alexander_ai_solutions::memory::preferences::recall_situational_preferences(
                    &self.memory,
                    user_message,
                )
                .await;
        ...
        let active_goal = {
            let loaded =
                crate::alexander_ai_solutions::thread_goals::runtime::load_for_current_thread(
                    &self.workspace_dir,
                )
                .await;
            ...
            match loaded {
                Some(goal)
                    if matches!(
                        goal.status,
                        crate::alexander_ai_solutions::thread_goals::ThreadGoalStatus::Paused
                    ) =>
                {
                    crate::alexander_ai_solutions::thread_goals::runtime::resume_for_current_thread(
                        &self.workspace_dir,
                    )
                    .await
                    .unwrap_or(Some(goal))
        ...
        if let Some(ref goal) = active_goal {
            if let Some(block) =
                crate::alexander_ai_solutions::thread_goals::runtime::active_goal_context_block(
                    goal,
                )
            {
                log::info!(
        ...
        let enriched = if should_run_super_context(
            ...
            let scout = harness::with_parent_context(parent_context.clone(), {
                ...
                async move {
                    crate::alexander_ai_solutions::agent_orchestration::tools::run_context_scout(
                        &user_message,
                        None,
                    )
                    .await
        ...
        let enriched = format!(
            "{}\n\n{enriched}",
            crate::alexander_ai_solutions::agent::prompts::current_datetime_line()
        );
        ...
        let turn_body = async {
            ...
            let artifact_store = Some(
                crate::alexander_ai_solutions::agent::harness::tool_result_artifacts::ToolResultArtifactStore::new(
            ...
            let turn_image_placeholders =
                crate::alexander_ai_solutions::agent::multimodal::extract_image_placeholders_in_text(
                    user_message,
                );
            ...
            self.last_turn_usage_totals = Some(
                crate::alexander_ai_solutions::agent::harness::turn_subagent_usage::LastTurnUsage {
            ...
            crate::alexander_ai_solutions::thread_goals::runtime::account_turn_against_goal(
                &self.workspace_dir,
                cumulative_input,
                cumulative_output,
        ...
        let mut turn_stop_hooks =
            crate::alexander_ai_solutions::agent::stop_hooks::current_stop_hooks();
        if let Some(ref goal) = active_goal {
            if let Some(hook) =
                crate::alexander_ai_solutions::thread_goals::runtime::GoalBudgetStopHook::for_goal(
                    &goal_workspace_dir,
                    goal,
                )
            {
        ...
        let result = if turn_stop_hooks.is_empty() {
            ...
            harness::with_parent_context(
                ...
                harness::with_agent_context_prepared_sources(
                    agent_context_prepared_sources.clone(),
                    crate::alexander_ai_solutions::agent::stop_hooks::with_stop_hooks(
                        turn_stop_hooks,
                        turn_body,
                    ),
                ),
...
mod super_context_gate_tests {
    ...
    use crate::alexander_ai_solutions::agent::harness::AgentContextPreparedSource;
