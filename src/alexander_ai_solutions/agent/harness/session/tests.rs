...
use crate::alexander_ai_solutions::agent::dispatcher::{NativeToolDispatcher, XmlToolDispatcher};
use crate::alexander_ai_solutions::inference::provider::{
    ChatRequest, ConversationMessage, Provider,
};
...
struct MockProvider {
    responses: Mutex<Vec<crate::alexander_ai_solutions::inference::provider::ChatResponse>>,
}
...
impl Provider for MockProvider {
    ...
    async fn chat(
        ...
        _model: &str,
        _temperature: f64,
    ) -> Result<crate::alexander_ai_solutions::inference::provider::ChatResponse> {
        let mut guard = self.responses.lock();
        if guard.is_empty() {
            return Ok(
                crate::alexander_ai_solutions::inference::provider::ChatResponse {
                    text: Some("done".into()),
                    tool_calls: vec![],
                    usage: None,
...
impl Tool for MockTool {
    ...
    ) -> Result<crate::alexander_ai_solutions::tools::ToolResult> {
        Ok(crate::alexander_ai_solutions::tools::ToolResult::success(
            "tool-out",
        ))
    }
}
...
fn build_minimal_agent_with_definition_name(definition_name: Option<&str>) -> Agent {
    ...
    let memory_cfg = crate::alexander_ai_solutions::config::MemoryConfig {
        backend: "none".into(),
        ..crate::alexander_ai_solutions::config::MemoryConfig::default()
    };
    let mem: Arc<dyn Memory> = Arc::from(
        crate::alexander_ai_solutions::memory_store::create_memory(&memory_cfg, &workspace_path)
            .unwrap(),
    );
...
fn set_connected_integrations_marks_session_initialized_and_updates_hash() {
    ...
    assert_eq!(
        ...
        crate::alexander_ai_solutions::composio::connected_set_hash(agent.connected_integrations())
    );
}

#[test]
fn refresh_delegation_tools_updates_schema_even_when_tool_arc_is_shared() {
    use crate::alexander_ai_solutions::agent::harness::AgentDefinitionRegistry;
...
fn refresh_workflows_picks_up_skill_installed_on_disk() {
    use crate::alexander_ai_solutions::workflows::ops_types::{SKILL_MD, TRUST_MARKER};
    ...
    std::fs::create_dir_all(wsp.join(".alexanderai")).unwrap();
    std::fs::write(wsp.join(".alexanderai").join(TRUST_MARKER), "").unwrap();
    let skill_dir = wsp
        .join(".alexanderai")
        .join("skills")
    ...
    let mem: Arc<dyn Memory> = Arc::from(
        crate::alexander_ai_solutions::memory_store::create_memory(&memory_cfg, &wsp).unwrap(),
    );
    let provider = Box::new(MockProvider {
        responses: Mutex::new(vec![]),
    });
    ...
    let provider = Box::new(MockProvider {
        responses: Mutex::new(vec![
            crate::alexander_ai_solutions::inference::provider::ChatResponse {
                ...
                tool_calls: vec![
                    crate::alexander_ai_solutions::inference::provider::ToolCall {
                        id: "tc1".into(),
                        name: "echo".into(),
                        arguments: "{}".into(),
                        extra_content: None,
...
async fn turn_dispatches_spawn_subagent_through_full_path() {
    use crate::alexander_ai_solutions::agent::harness::AgentDefinitionRegistry;
    use crate::alexander_ai_solutions::tools::SpawnSubagentTool;
...
fn seed_resume_from_messages_is_noop_on_warm_agent() {
    ...
    agent.cached_transcript_messages = Some(vec![
        crate::alexander_ai_solutions::inference::provider::ChatMessage::system("warm prefix"),
        crate::alexander_ai_solutions::inference::provider::ChatMessage::user("hi"),
    ]);
    agent
...
fn bound_cached_transcript_messages_without_system_prefix_keeps_tail() {
    ...
    let messages = vec![
        crate::alexander_ai_solutions::inference::provider::ChatMessage::user("u1"),
        crate::alexander_ai_solutions::inference::provider::ChatMessage::assistant("a1"),
        crate::alexander_ai_solutions::inference::provider::ChatMessage::user("u2"),
        crate::alexander_ai_solutions::inference::provider::ChatMessage::assistant("a2"),
        crate::alexander_ai_solutions::inference::provider::ChatMessage::user("u3"),
    ];
...
fn bound_cached_transcript_messages_snaps_past_leading_orphan_tool() {
    use crate::alexander_ai_solutions::inference::provider::ChatMessage;
