use super::*;
use crate::alexander_ai_solutions::agent::dispatcher::XmlToolDispatcher;
use crate::alexander_ai_solutions::agent::hooks::{PostTurnHook, TurnContext};
use crate::alexander_ai_solutions::agent::memory_loader::MemoryLoader;
use crate::alexander_ai_solutions::agent::tool_policy::{
...
use crate::alexander_ai_solutions::tools::ToolResult;
use crate::alexander_ai_solutions::tools::{PermissionLevel, Tool};
...
fn make_agent(visible_tool_names: Option<HashSet<String>>) -> Agent {
    ...
    let mut builder = Agent::builder()
        ...
        .config(crate::alexander_ai_solutions::config::AgentConfig {
            max_history_messages: 3,
            ..crate::alexander_ai_solutions::config::AgentConfig::default()
        });
...
fn make_agent_with_builder(
    ...
    config: crate::alexander_ai_solutions::config::AgentConfig,
    context_config: crate::alexander_ai_solutions::config::ContextConfig,
) -> Agent {
    let workspace = tempfile::TempDir::new().expect("temp workspace");
    let workspace_path = workspace.path().to_path_buf();
    std::mem::forget(workspace);
...
fn trim_history_snaps_past_orphaned_tool_results() {
    use crate::alexander_ai_solutions::inference::provider::{ToolCall, ToolResultMessage};
...
fn build_parent_context_and_sanitize_helpers_cover_snapshot_paths() {
    ...
    agent.workflows = vec![crate::alexander_ai_solutions::workflows::Workflow {
        name: "demo".into(),
        ..Default::default()
    }];
...
fn collect_tree_root_summaries_maps_namespace_body_and_timestamp() {
    ...
    use crate::alexander_ai_solutions::memory_tree::tree_runtime::store::write_node;
    use crate::alexander_ai_solutions::memory_tree::tree_runtime::types::{
...
async fn execute_tool_call_denies_tool_above_channel_permission() {
    ...
    let provider: Arc<dyn Provider> = Arc::new(DummyProvider);
    let mut config = crate::alexander_ai_solutions::config::AgentConfig::default();
    config
        .channel_permissions
...
async fn turn_runs_full_tool_cycle_with_context_and_hooks() {
    ...
    let mut agent = make_agent_with_builder(
        ...
        hooks,
        crate::alexander_ai_solutions::config::AgentConfig {
...
async fn turn_triggers_configured_memory_agent_before_parent_prompt() {
    crate::alexander_ai_solutions::agent::harness::definition::AgentDefinitionRegistry::init_global_builtins()
    ...
    let mut agent = Agent::builder()
        ...
        .trigger_memory_agent(
            crate::alexander_ai_solutions::agent::harness::definition::TriggerMemoryAgent::Always,
        )
        .build()
        .unwrap();
    assert_eq!(
...
async fn execute_tool_call_applies_inline_result_budget() {
    ...
    let agent = make_agent_with_builder(
        ...
        vec![],
        crate::alexander_ai_solutions::config::AgentConfig::default(),
        crate::alexander_ai_solutions::config::ContextConfig {
...
fn make_real_memory(workspace: &std::path::Path) -> Arc<dyn Memory> {
    use crate::alexander_ai_solutions::embeddings::NoopEmbedding;
    use crate::alexander_ai_solutions::memory_store::UnifiedMemory;
...
async fn fetch_learned_context_returns_empty_when_both_flags_off() {
    ...
    mem.store(
        ...
        crate::alexander_ai_solutions::memory::MemoryCategory::Core,
        None,
    )
    .await
    .unwrap();
...
async fn fetch_learned_context_returns_general_prefs_when_explicit_flag_on_learning_off() {
    ...
    mem.store(
        crate::alexander_ai_solutions::memory::preferences::USER_PREF_GENERAL_NAMESPACE,
        "package_manager",
