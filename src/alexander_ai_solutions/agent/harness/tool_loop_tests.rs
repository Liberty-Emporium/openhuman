...
use crate::alexander_ai_solutions::inference::provider::{ChatRequest, ChatResponse};
use crate::alexander_ai_solutions::tools::{ToolResult, ToolScope};
...
async fn run_tool_call_loop_executes_real_resolve_time_and_threads_back_correct_epoch() {
    ...
    let tools: Vec<Box<dyn Tool>> = vec![Box::new(
        crate::alexander_ai_solutions::tools::ResolveTimeTool::new(),
    )];
...
async fn run_tool_call_loop_aborts_when_stop_hook_returns_stop() {
    use crate::alexander_ai_solutions::agent::stop_hooks::{
...
async fn run_tool_call_loop_applies_per_tool_max_result_size_cap() {
    ...
    impl Tool for CappedHugeTool {
        ...
        fn permission_level(&self) -> crate::alexander_ai_solutions::tools::PermissionLevel {
            crate::alexander_ai_solutions::tools::PermissionLevel::ReadOnly
        }
    ...
    assert!(
        ...
        crate::alexander_ai_solutions::util::utf8_safe_prefix_at_byte_boundary(
...
fn hard_reject_kind_detects_markers() {
    use crate::alexander_ai_solutions::security::{POLICY_BLOCKED_MARKER, POLICY_DENIED_MARKER};
...
fn hard_reject_blocked_halts_on_first_repeat_not_third() {
    use crate::alexander_ai_solutions::security::POLICY_BLOCKED_MARKER;
    let mut g = RepeatFailureGuard::new();
    let blocked =
...
fn hard_reject_denied_halts_on_first_repeat() {
    use crate::alexander_ai_solutions::security::POLICY_DENIED_MARKER;
    let mut g = RepeatFailureGuard::new();
...
async fn auto_approved_external_effect_tool_runs_through_loop_without_parking() {
    ...
    let _env = crate::alexander_ai_solutions::config::TEST_ENV_LOCK
        .lock()
    ...
    let policy = crate::alexander_ai_solutions::security::SecurityPolicy {
        auto_approve: vec![tool_name.into()],
        ..crate::alexander_ai_solutions::security::SecurityPolicy::default()
    };
    ...
    let cfg = crate::alexander_ai_solutions::config::Config {
        workspace_dir: std::env::temp_dir(),
        ..crate::alexander_ai_solutions::config::Config::default()
    };
    crate::alexander_ai_solutions::approval::ApprovalGate::init_global(
        cfg,
        "session-loop-gate-e2e",
    );
    ...
    let result = crate::alexander_ai_solutions::approval::APPROVAL_CHAT_CONTEXT
        .scope(
            crate::alexander_ai_solutions::approval::ApprovalChatContext {
