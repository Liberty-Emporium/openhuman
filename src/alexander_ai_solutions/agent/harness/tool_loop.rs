...
use crate::alexander_ai_solutions::inference::provider::{
    ChatMessage, Provider, AGENT_TURN_MAX_OUTPUT_TOKENS,
};
use crate::alexander_ai_solutions::tools::policy::{DefaultToolPolicy, ToolPolicy};
...
/// ([`crate::alexander_ai_solutions::security::POLICY_BLOCKED_MARKER`] /
/// [`POLICY_DENIED_MARKER`](crate::alexander_ai_solutions::security::POLICY_DENIED_MARKER)).
...
pub(crate) fn hard_reject_kind(result: &str) -> Option<HardReject> {
    if result.contains(crate::alexander_ai_solutions::security::POLICY_BLOCKED_MARKER) {
        Some(HardReject::Blocked)
    } else if result.contains(crate::alexander_ai_solutions::security::POLICY_DENIED_MARKER) {
...
pub(crate) enum TerminalInferenceFailure {
    ...
    /// Detected via
    /// [`is_budget_exhausted_message`](crate::alexander_ai_solutions::inference::provider::is_budget_exhausted_message).
    BudgetExhausted,
    ...
    /// [`is_provider_config_rejection_message`](crate::alexander_ai_solutions::inference::provider::is_provider_config_rejection_message).
    ProviderConfig,
}
...
///     [`crate::alexander_ai_solutions::agent_orchestration::tools::dispatch::format_subagent_failure`]).
///
