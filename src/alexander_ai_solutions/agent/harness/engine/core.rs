...
use crate::alexander_ai_solutions::agent::cost::TurnCost;
use crate::alexander_ai_solutions::agent::multimodal;
use crate::alexander_ai_solutions::agent::stop_hooks::{
    current_stop_hooks, StopDecision, TurnState,
};
use crate::alexander_ai_solutions::context::guard::{ContextCheckResult, ContextGuard};
use crate::alexander_ai_solutions::context::{summarize_chat_history, EngineAutocompact};
...
pub(crate) async fn run_turn_engine(
    ...
    temperature: f64,
    silent: bool,
    multimodal_config: &crate::alexander_ai_solutions::config::MultimodalConfig,
    multimodal_file_config: &crate::alexander_ai_solutions::config::MultimodalFileConfig,
    ...
    for iteration in 0..max_iterations {
        ...
        match context_guard.check() {
            ...
            ContextCheckResult::CompactionNeeded => {
                tracing::warn!(
                    ...
                    crate::alexander_ai_solutions::context::guard::COMPACTION_TRIGGER_THRESHOLD
                        * 100.0
                );
        ...
        let has_vision = turn_accepts_images(
            crate::alexander_ai_solutions::agent::harness::model_vision_context::current_model_vision(),
        ...
        if let Some(loaded) = loaded_context_window {
            if let Some(prefix_err) =
                crate::alexander_ai_solutions::agent::harness::token_budget::unevictable_prefix_overflow(
        ...
        let (
            ...
            Err(e) => {
                ...
                let transient =
                    crate::alexander_ai_solutions::inference::provider::reliable::is_rate_limited(&e)
                        || crate::alexander_ai_solutions::inference::provider::reliable::is_upstream_unhealthy(
                            &e,
                        );
                if transient {
                    tracing::warn!(
        ...
        if tool_calls.is_empty() {
            ...
            if final_out.trim().is_empty() && !observer.allow_empty_final() {
                ...
                return Err(
                    crate::alexander_ai_solutions::agent::error::AgentError::EmptyProviderResponse {
                        iteration: iteration + 1,
                    }
                    .into(),
                );
