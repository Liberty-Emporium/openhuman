...
use crate::alexander_ai_solutions::agent::harness::parse::ParsedToolCall;
use crate::alexander_ai_solutions::tools::policy::{PolicyDecision, ToolPolicy};
use crate::alexander_ai_solutions::tools::traits::ToolScope;
...
fn resolve_tool_deadline(
    policy: crate::alexander_ai_solutions::tools::traits::ToolTimeout,
) -> (Option<std::time::Duration>, u64) {
    use crate::alexander_ai_solutions::tool_timeout::{MAX_TIMEOUT_SECS, MIN_TIMEOUT_SECS};
    use crate::alexander_ai_solutions::tools::traits::ToolTimeout;
    match policy {
        ToolTimeout::Inherit => {
            let s = crate::alexander_ai_solutions::tool_timeout::tool_execution_timeout_secs();
...
pub(crate) async fn run_one_tool(
    ...
    tokenjuice_compression: crate::alexander_ai_solutions::tokenjuice::AgentTokenjuiceCompression,
) -> ToolRunResult {
    ...
    let mut approval_gate_for_audit: Option<
        std::sync::Arc<crate::alexander_ai_solutions::approval::ApprovalGate>,
    > = None;
    if tool.external_effect_with_args(&call.arguments) {
        if let Some(gate) = crate::alexander_ai_solutions::approval::ApprovalGate::try_global() {
            let summary = crate::alexander_ai_solutions::approval::summarize_action(
                &call.name,
                &call.arguments,
            );
            let redacted = crate::alexander_ai_solutions::approval::redact_args(&call.arguments);
            let (outcome, request_id) =
                gate.intercept_audited(&call.name, &summary, redacted).await;
            match outcome {
                crate::alexander_ai_solutions::approval::GateOutcome::Allow => {
                    approval_request_id = request_id;
                    if approval_request_id.is_some() {
                        approval_gate_for_audit = Some(gate);
                    }
                }
                crate::alexander_ai_solutions::approval::GateOutcome::Deny { reason } => {
                    tracing::warn!(
                        iteration,
                        tool = call.name.as_str(),
                        reason = %reason,
    ...
    match policy {
        crate::alexander_ai_solutions::tools::traits::ToolTimeout::Secs(req) => tracing::debug!(
            iteration,
            tool = call.name.as_str(),
        ...
        crate::alexander_ai_solutions::tools::traits::ToolTimeout::Unbounded => tracing::debug!(
            iteration,
            tool = call.name.as_str(),
            "[agent_loop] tool runs unbounded (no harness timeout) — no explicit timeout_secs"
        ),
        crate::alexander_ai_solutions::tools::traits::ToolTimeout::Inherit => {}
    }
        ...
        Ok(Ok(r)) => {
            ...
            if success {
                ...
                let (compacted, tj_stats) =
                    crate::alexander_ai_solutions::tokenjuice::compact_tool_output_with_policy(
                        &call.name,
                        Some(&call.arguments),
                        &scrubbed,
                        Some(0),
    ...
    if let (Some(gate), Some(req_id)) = (
        ...
        let exec_outcome = if success {
            crate::alexander_ai_solutions::approval::ExecutionOutcome::Success
        } else {
            crate::alexander_ai_solutions::approval::ExecutionOutcome::Failure
        };
...
mod tests {
    ...
    fn explicit_secs_policy_clamps_out_of_range() {
        ...
        assert_eq!(
            secs_min,
            crate::alexander_ai_solutions::tool_timeout::MIN_TIMEOUT_SECS
        );
    }
}
