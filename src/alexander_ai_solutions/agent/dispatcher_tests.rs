use super::*;
use crate::alexander_ai_solutions::agent::pformat::PFormatToolParams;
...
fn tool_results(id: &str) -> ConversationMessage {
    use crate::alexander_ai_solutions::inference::provider::ToolResultMessage;
...
fn user_chat(text: &str) -> ConversationMessage {
    ConversationMessage::Chat(
        crate::alexander_ai_solutions::inference::provider::ChatMessage::user(text),
    )
}

fn assistant_chat(text: &str) -> ConversationMessage {
    ConversationMessage::Chat(
        crate::alexander_ai_solutions::inference::provider::ChatMessage::assistant(text),
    )
}
...
fn assistant_tool_calls_multi(ids: &[&str]) -> ConversationMessage {
    ConversationMessage::AssistantToolCalls {
        ...
        tool_calls: ids
            .iter()
            .map(
                |id| crate::alexander_ai_solutions::inference::provider::ToolCall {
                    id: (*id).into(),
                    name: "shell".into(),
                    arguments: "{}".into(),
                    extra_content: None,
