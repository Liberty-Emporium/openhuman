...
fn outcome_to_result(
    ...
    outcome: Option<crate::alexander_ai_solutions::workflows::run_log::RunOutcome>,
) -> ToolResult {
    match outcome {
...
impl Tool for AwaitWorkflowTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let log_path = match crate::alexander_ai_solutions::workflows::run_log::find_run_log_path(
            &workspace, &run_id,
        ) {
            Some(p) => p,
            None => {
