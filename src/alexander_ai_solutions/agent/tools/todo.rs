...
//! one) via [`crate::alexander_ai_solutions::todos::ops`]; without a thread context the
//! tool falls back to a process-global scratch list. Returns a markdown
//! rendering so transcripts read cleanly.

use crate::alexander_ai_solutions::agent::task_board::{
    TaskApprovalMode, TaskBoardCard, TaskCardStatus,
};
use crate::alexander_ai_solutions::inference::provider::thread_context;
...
fn current_location() -> BoardLocation {
    let Some(parent) =
        crate::alexander_ai_solutions::agent::harness::fork_context::current_parent()
    else {
...
mod tests {
    use super::*;
    use crate::alexander_ai_solutions::todos::global_scratch_store;
    ...
    fn scratch_lock() -> std::sync::MutexGuard<'static, ()> {
        crate::alexander_ai_solutions::todos::ops::scratch_test_lock()
    }
