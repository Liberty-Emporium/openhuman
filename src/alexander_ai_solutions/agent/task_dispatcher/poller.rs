...
use crate::alexander_ai_solutions::todos::ops::{self, BoardLocation, USER_TASKS_THREAD_ID};
use crate::alexander_ai_solutions::todos::runs::{self, RunLimits};
...
pub(crate) async fn poll_once() -> Result<(), String> {
    ...
    let Some(_permit) = crate::alexander_ai_solutions::scheduler_gate::wait_for_capacity().await
    else {
    ...
    if config.task_sources.enabled {
        boards.push((
            BoardLocation::Thread {
                workspace_dir: config.workspace_dir.clone(),
                thread_id: crate::alexander_ai_solutions::task_sources::TASK_SOURCES_THREAD_ID
                    .to_string(),
            },
            false,
        ));
...
///   [`crate::alexander_ai_solutions::agent::tools::todo`]) stamps `Required`, and that
