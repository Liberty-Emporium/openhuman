...
use crate::alexander_ai_solutions::agent::harness::definition::{
    AgentDefinitionRegistry, PromptSource,
};
use crate::alexander_ai_solutions::agent::harness::session::Agent;
use crate::alexander_ai_solutions::agent::harness::subagent_runner::with_autonomous_iter_cap;
use crate::alexander_ai_solutions::agent::task_board::TaskCardStatus;
use crate::alexander_ai_solutions::agent::task_session;
use crate::alexander_ai_solutions::config::Config;
use crate::alexander_ai_solutions::profiles::PersonalityContext;
use crate::alexander_ai_solutions::todos::ops::{self, BoardLocation, CardPatch};
use crate::alexander_ai_solutions::todos::runs::{self, RunOutcome};
...
pub(super) fn resolve_executor(workspace_dir: &Path, assigned: Option<&str>) -> ResolvedExecutor {
    ...
    if let Ok(state) = crate::alexander_ai_solutions::profiles::load_profiles(workspace_dir) {
    ...
    if let Some(skill) =
        crate::alexander_ai_solutions::workflows::registry::get_workflow(workspace_dir, handle)
    {
...
pub(super) async fn run_autonomous(
    ...
    if let Some(thread_id) = session_thread_id.as_deref() {
        ...
        crate::alexander_ai_solutions::channels::providers::web::spawn_progress_bridge(
            progress_rx,
            "system".to_string(),
            thread_id.to_string(),
            run_id.to_string(),
            crate::alexander_ai_solutions::threads::turn_state::TurnStateStore::new(
                workspace_dir.clone(),
            ),
    ...
    let run = crate::alexander_ai_solutions::memory::source_scope::with_source_scope(
        memory_scope,
        crate::alexander_ai_solutions::agent::turn_origin::with_origin(
            crate::alexander_ai_solutions::agent::turn_origin::AgentTurnOrigin::Cli,
    ...
    if let Some(thread_id) = session_thread_id.as_deref() {
        match &result {
            ...
            Err(err) => {
                crate::alexander_ai_solutions::channels::providers::web::publish_web_channel_event(
