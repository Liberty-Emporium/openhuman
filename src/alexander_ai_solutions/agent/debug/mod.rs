...
use crate::alexander_ai_solutions::composio::ComposioActionTool;
...
async fn render_integrations_agent(config: &Config, toolkit: &str) -> Result<DumpedPrompt> {
    ...
    let base_tools: Vec<Box<dyn Tool>> = match &definition_snapshot.tools {
        crate::alexander_ai_solutions::agent::harness::definition::ToolScope::Named(names) => {
        ...
        crate::alexander_ai_solutions::agent::harness::definition::ToolScope::Wildcard => agent
            .tools()
            .iter()
            .map(|t| clone_tool_as_prompt_proxy(t.as_ref()))
            .collect(),
    ...
    if !rendered_tools.is_empty() {
        ...
        text.push_str("\n\n");
        text.push_str(
            &crate::alexander_ai_solutions::agent::harness::subagent_runner::build_text_mode_tool_instructions(
                &tool_specs,
            ),
        );
    }
...
impl Tool for PromptProxyTool {
    ...
    fn permission_level(&self) -> crate::alexander_ai_solutions::tools::PermissionLevel {
        crate::alexander_ai_solutions::tools::PermissionLevel::None
    }
    async fn execute(
        &self,
        _args: serde_json::Value,
    ) -> anyhow::Result<crate::alexander_ai_solutions::tools::ToolResult> {
        Err(anyhow!(
