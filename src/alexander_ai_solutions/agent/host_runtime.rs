...
use crate::alexander_ai_solutions::config::RuntimeConfig;
...
impl RuntimeAdapter for NativeRuntime {
    ...
    fn build_shell_command(
        ...
        // (#3353, Fix 2)
        crate::alexander_ai_solutions::config::ensure_usable_cwd(workspace_dir)?;
...
fn maybe_hide_window(cmd: &mut tokio::process::Command, hide: bool) {
    ...
    crate::alexander_ai_solutions::inference::local::process_util::apply_no_window(cmd);
    #[cfg(windows)]
    tracing::debug!(
...
pub struct DockerRuntime {
    config: crate::alexander_ai_solutions::config::DockerRuntimeConfig,
}

impl DockerRuntime {
    fn new(config: crate::alexander_ai_solutions::config::DockerRuntimeConfig) -> Self {
        Self { config }
    }
}
...
mod tests {
    use super::*;
    use crate::alexander_ai_solutions::config::{DockerRuntimeConfig, RuntimeConfig};
