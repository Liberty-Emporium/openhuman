...
fn handle_chat(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::alexander_ai_solutions::inference::local::rpc::agent_chat(
                &mut config,
                &p.message,
                p.model_override,
                p.temperature,
...
fn handle_chat_simple(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        to_json(
            crate::alexander_ai_solutions::inference::local::rpc::agent_chat_simple(
                &config,
                &p.message,
                p.model_override,
                p.temperature,
...
fn handle_list_definitions(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async {
        let defs =
            crate::alexander_ai_solutions::agent::library::list_definition_metadata().await?;
...
fn handle_get_definition(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let registry =
            crate::alexander_ai_solutions::agent::harness::AgentDefinitionRegistry::global()
    ...
    Box::pin(async move {
        ...
        let envelope = match p.source.as_str() {
            "composio" => {
                ...
                crate::alexander_ai_solutions::agent::triage::TriggerEnvelope::from_composio(
            ...
            "webhook" => {
                ...
                crate::alexander_ai_solutions::agent::triage::TriggerEnvelope::from_webhook(
            ...
            "cron" => {
                ...
                let output = p
                    ...
                    .unwrap_or(job_name);
                crate::alexander_ai_solutions::agent::triage::TriggerEnvelope::from_cron(
            ...
            "external" => {
                ...
                crate::alexander_ai_solutions::agent::triage::TriggerEnvelope::from_external(
        ...
        let outcome = crate::alexander_ai_solutions::agent::triage::run_triage(&envelope)
            .await
            .map_err(|e| format!("triage evaluation failed: {e}"))?;

        let dry_run = p.dry_run.unwrap_or(false);
        match outcome {
            crate::alexander_ai_solutions::agent::triage::TriageOutcome::Decision(run) => {
                if !dry_run {
                    crate::alexander_ai_solutions::agent::triage::apply_decision(
                        run.clone(),
                        &envelope,
                    )
                    .await
            ...
            crate::alexander_ai_solutions::agent::triage::TriageOutcome::Deferred {
                defer_until_ms,
                reason,
            } => {
