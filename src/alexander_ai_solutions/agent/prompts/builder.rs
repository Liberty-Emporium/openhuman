...
impl SystemPromptBuilder {
    ...
    /// the `omit_*` flags from the [`crate::alexander_ai_solutions::agent::harness::definition::AgentDefinition`]:
    ...
    /// [`crate::alexander_ai_solutions::memory_tools::ToolMemoryStore::rules_for_prompt`]
    ...
    pub fn with_tool_memory_rules(
        mut self,
        rules: Vec<crate::alexander_ai_solutions::memory_tools::ToolMemoryRule>,
    ) -> Self {
        if rules.is_empty() {
            return self;
        }
        ...
        let section: Box<dyn PromptSection> = Box::new(
            crate::alexander_ai_solutions::memory_tools::ToolMemoryRulesSection::new(rules),
        );
        let tools_idx = self
            .sections
            .iter()
