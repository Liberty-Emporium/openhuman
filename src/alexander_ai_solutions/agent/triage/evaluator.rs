...
use crate::alexander_ai_solutions::agent::harness::definition::{AgentDefinition, PromptSource};
...
use crate::alexander_ai_solutions::inference::provider::reliable::{
...
use crate::alexander_ai_solutions::scheduler_gate::LlmPermit;
...
pub async fn run_triage(envelope: &TriggerEnvelope) -> anyhow::Result<TriageOutcome> {
    ...
    let outcome = run_triage_with_arms_inner(cloud, local, envelope, || {
        crate::alexander_ai_solutions::scheduler_gate::wait_for_capacity()
    })
    .await;
...
fn truncate_payload(payload: &serde_json::Value, max_bytes: usize) -> String {
    ...
    let end = crate::alexander_ai_solutions::util::floor_char_boundary(&pretty, max_bytes);
