...
//!   [`crate::alexander_ai_solutions::agent::progress::AgentProgress::TurnCostUpdated`];
...
///    ([`crate::alexander_ai_solutions::cost::catalog`]) — accurate per-model rates for
...
pub fn lookup_pricing(model: &str) -> ModelPricing {
    ...
    if let Some(price) = crate::alexander_ai_solutions::cost::catalog::lookup(model) {
        return ModelPricing {
