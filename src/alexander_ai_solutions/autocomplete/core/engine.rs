...
impl AutocompleteEngine {
    ...
    pub async fn start(
        ...
        PRECOMPILE_ONCE.call_once(|| {
            crate::alexander_ai_solutions::accessibility::precompile_helper_background();
        });
                ...
                if last_refresh.elapsed() >= Duration::from_millis(current_debounce_ms) {
                    ...
                    match refresh_result {
                        Ok(Ok(Err(err))) => {
                            ...
                            if is_perm_denied {
                                crate::alexander_ai_solutions::accessibility::mark_system_events_denied();
                            }
    ...
    pub async fn accept(
        ...
        // Dual-write: KV (UI list) + local docs (semantic search).
        {
            ...
            tokio::spawn(async move {
                crate::alexander_ai_solutions::autocomplete::history::save_accepted_completion(
                    &ctx,
                    &sug,
                    app.as_deref(),
                )
                .await;
                crate::alexander_ai_solutions::autocomplete::history::save_completion_to_local_docs(
                    &ctx,
                    &sug,
                    app.as_deref(),
                )
    ...
    async fn refresh(&self, context_override: Option<String>) -> Result<(), String> {
        ...
        let (relevant_examples, recent_examples) = if is_in_app {
            ...
            let relevant_future =
                crate::alexander_ai_solutions::autocomplete::history::query_relevant_examples(
                    &context, 4,
                );
            let recent_future =
                crate::alexander_ai_solutions::autocomplete::history::load_recent_examples(4);
...
pub async fn start_if_enabled(app_config: &Config) {
    ...
    crate::alexander_ai_solutions::accessibility::clear_automation_denial();
