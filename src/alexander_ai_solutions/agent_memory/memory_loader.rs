...
use crate::alexander_ai_solutions::util::provenance_tag;
use async_trait::async_trait;
use serde::{Deserialize, Serialize};

use crate::alexander_ai_solutions::agent::harness::memory_context::{
    CROSS_CHAT_LIMIT, CROSS_CHAT_SNIPPET_CHARS, WORKING_MEMORY_KEY_PREFIX, WORKING_MEMORY_LIMIT,
};
use crate::alexander_ai_solutions::learning::transcript_ingest::CONVERSATION_MEMORY_NAMESPACE;
use crate::alexander_ai_solutions::memory_conversations::ConversationStore;
...
fn memory_entry_date_label(timestamp: &str) -> Option<String> {
    chrono::DateTime::parse_from_rfc3339(timestamp)
        .ok()
        .map(|dt| {
            crate::alexander_ai_solutions::agent::prompts::memory_date_label(
    ...
    let citations = entries
        ...
        .map(|entry| {
            let snippet = if entry.content.chars().count() > 280 {
                crate::alexander_ai_solutions::util::truncate_with_ellipsis(&entry.content, 280)
            } else {
                entry.content
            };
            MemoryCitation {
    ...
    async fn load_context(
        ...
        if self.include_agent_conversations {
            ...
            let prior_entries = memory
                .recall(
                    ...
                    crate::alexander_ai_solutions::memory::RecallOpts {
        ...
        if self.include_agent_conversations {
            ...
            let cross_hits: Vec<(String, String)> = if let Some(workspace_dir) = &self.workspace_dir
            {
                ...
                let cross_session_opts = crate::alexander_ai_solutions::memory::RecallOpts {
            ...
            for (sid, content) in cross_hits {
                let snippet = if content.chars().count() > CROSS_CHAT_SNIPPET_CHARS {
                    crate::alexander_ai_solutions::util::truncate_with_ellipsis(
                        &content,
...
mod tests {
    ...
    impl Memory for MockMemory {
        ...
        async fn recall(
            ...
            _query: &str,
            _limit: usize,
            opts: crate::alexander_ai_solutions::memory::RecallOpts<'_>,
    ...
    async fn loader_surfaces_jsonl_primary_path_with_workspace_dir() {
        use crate::alexander_ai_solutions::memory_conversations::{
