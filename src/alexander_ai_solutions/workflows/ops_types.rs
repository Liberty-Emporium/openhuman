...
pub enum WorkflowScope {
    /// Workflow shipped with the user's global config (`~/.alexanderai/skills/...`).
    User,
    /// Workflow shipped with the current workspace (`<ws>/.alexanderai/skills/...`).
...
pub struct Workflow {
    ...
    pub name: String,
    /// On-disk slug — the directory name under `~/.alexanderai/skills/` (user
