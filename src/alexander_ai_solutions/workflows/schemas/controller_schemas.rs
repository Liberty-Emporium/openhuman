    ...
    match function {
        ...
        "workflows_recent_runs" => ControllerSchema {
            ...
            outputs: vec![FieldSchema {
                name: "runs",
                ty: TypeSchema::Json,
                comment: "Array of `{ run_id, workflow_id, started, status, duration_ms, finished, log_path }` — see crate::alexander_ai_solutions::workflows::run_log::ScannedRun.",
                required: true,
            }],
        },
        ...
        "workflows_uninstall" => ControllerSchema {
            namespace: "workflows",
            function: "uninstall",
            description: "Remove an installed user-scope SKILL.md skill from `~/.alexanderai/skills/<name>/`. Only user-scope installs are supported; project-scope and legacy skills are read-only. Rejects path separators and traversal; canonicalises before delete.",
            inputs: vec![FieldSchema {
                name: "name",
                ty: TypeSchema::String,
                comment: "Exact on-disk slug of the installed skill — matches WorkflowSummary.id (the directory under ~/.alexanderai/skills/), which may differ from the frontmatter display name in Workflow.name.",
                required: true,
            }],
            outputs: vec![
                FieldSchema {
