...
/// the preflight described in [`crate::alexander_ai_solutions::workflows::schemas`]'s
...
pub struct WorkflowDefinition {
    ...
    /// boots — see
    /// [`crate::alexander_ai_solutions::skill_runtime::spawn_workflow_run_background`].
    #[serde(default)]
...
/// (`~/.alexanderai/skills`), project (`<ws>/.alexanderai/skills`, trusted), and
...
/// (which lands in `.alexanderai/skills`) is runnable, not just listable.
...
pub fn load_workflows(workspace_dir: &Path) -> Vec<WorkflowDefinition> {
    ...
    if let Ok(builtins) = crate::alexander_ai_solutions::agent_registry::agents::load_builtins() {
