...
fn load_skills_parses_skill_md_frontmatter() {
    ...
    write(&ws.join(".alexanderai").join("trust"), "");
    let skill_dir = ws.join(".alexanderai").join("skills").join("hello-world");
    write(
...
fn deprecated_top_level_fields_load_with_migration_warning() {
    ...
    let ws = dir.path();
    write(&ws.join(".alexanderai").join("trust"), "");
    let skill_dir = ws.join(".alexanderai").join("skills").join("legacy-fm");
    write(
        &skill_dir.join("SKILL.md"),
...
fn project_skills_skipped_when_not_trusted() {
    ...
    let ws = dir.path();
    // No trust marker.
    let skill_dir = ws.join(".alexanderai").join("skills").join("unsafe");
    write(
        &skill_dir.join("SKILL.md"),
...
fn frontmatter_missing_name_warns_and_falls_back() {
    ...
    let ws = dir.path();
    write(&ws.join(".alexanderai").join("trust"), "");
    let skill_dir = ws.join(".alexanderai").join("skills").join("mystery");
    write(
        &skill_dir.join("SKILL.md"),
...
fn frontmatter_missing_description_uses_first_body_line() {
    ...
    let ws = dir.path();
    write(&ws.join(".alexanderai").join("trust"), "");
    let skill_dir = ws.join(".alexanderai").join("skills").join("s");
    write(
        &skill_dir.join("SKILL.md"),
        "---\nname: s\n---\n\n# Heading\n\nActual first line.\n",
    );
...
fn directory_name_mismatch_warns_but_loads() {
    ...
    let ws = dir.path();
    write(&ws.join(".alexanderai").join("trust"), "");
    let skill_dir = ws.join(".alexanderai").join("skills").join("dir-name");
    write(
        &skill_dir.join("SKILL.md"),
...
fn project_scope_shadows_user_scope_on_collision() {
    ...
    write(&ws_dir.path().join(".alexanderai").join("trust"), "");
...
fn symlinked_skill_dirs_are_skipped() {
    ...
    // Symlink <ws>/.alexanderai/skills/evil -> external/evil
    let skills_root = ws.join(".alexanderai").join("skills");
...
fn hidden_dirs_are_skipped() {
    ...
    let ws = dir.path();
    write(&ws.join(".alexanderai").join("trust"), "");
    let hidden = ws.join(".alexanderai").join("skills").join(".hidden");
    write(
...
fn edit_updates_workflow_that_still_lives_under_legacy_skills_root() {
    ...
    // at `~/.alexanderai/skills/<slug>/SKILL.md`. Editing it (overwrite=true)
    // must resolve that legacy location and update it in place — NOT fail with
    // "cannot update workflow '<slug>': it does not exist at
    // ~/.alexanderai/workflows/<slug>" (which only checked the new root).
    let home = tempfile::tempdir().unwrap();
    let ws = tempfile::tempdir().unwrap();
...
fn create_skill_project_scope_writes_under_workspace_when_trusted() {
    let home = tempfile::tempdir().unwrap();
    let ws = tempfile::tempdir().unwrap();
    write(&ws.path().join(".alexanderai").join(TRUST_MARKER), "");
...
/// The current user-scope layout is `~/.alexanderai/workflows/<id>/SKILL.md`
...
fn discover_reads_user_scope_workflows_dir() {
    ...
    assert_eq!(
        found.len(),
        1,
        "workflow under .alexanderai/workflows/ must load"
    );
...
fn uninstall_skill_rejects_path_traversal_names() {
    let home = tempfile::tempdir().unwrap();
    std::fs::create_dir_all(home.path().join(".alexanderai").join("skills")).unwrap();
...
/// `~/.alexanderai/workflows/<id>/WORKFLOW.md` (the regression that left delete
...
fn uninstall_skill_rejects_symlink_escape() {
    let home = tempfile::tempdir().unwrap();
    let skills_root = home.path().join(".alexanderai").join("skills");
    std::fs::create_dir_all(&skills_root).unwrap();
...
/// A symlinked skills *root* (`~/.alexanderai/skills -> elsewhere`) must
...
fn uninstall_skill_rejects_symlinked_skills_root() {
    ...
    std::fs::create_dir_all(home.path().join(".alexanderai")).unwrap();
    std::os::unix::fs::symlink(
        &real_skills,
        home.path().join(".alexanderai").join("skills"),
    )
    .unwrap();
    let err = uninstall_workflow(
        UninstallWorkflowParams {
...
fn skill_create_input_def_deserializes_full_row_from_json() {
    let row: crate::alexander_ai_solutions::workflows::ops_create::WorkflowCreateInputDef =
...
fn skill_create_input_def_round_trips_through_registry_skill_input() {
    ...
    let rows = vec![
        crate::alexander_ai_solutions::workflows::ops_create::WorkflowCreateInputDef {
            name: "repo".into(),
    ...
    let parsed: crate::alexander_ai_solutions::workflows::registry::WorkflowDefinition =
...
fn symlinked_manifest_file_is_rejected() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let ws = dir.path();
    write(&ws.join(".alexanderai").join(TRUST_MARKER), "");
    let skill_dir = ws.join(".alexanderai").join("skills").join("sneaky");
    std::fs::create_dir_all(&skill_dir).unwrap();
...
fn discover_automations_excludes_user_skill_root_but_keeps_workflows() {
    let home = tempfile::tempdir().unwrap();
    // A capability skill installed under ~/.alexanderai/skills/.
    write(
        &home
            .path()
            .join(".alexanderai")
    ...
    // A real automation authored under ~/.alexanderai/workflows/.
    write(
        &home
            .path()
            .join(".alexanderai")
