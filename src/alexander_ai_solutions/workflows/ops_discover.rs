...
/// `<workspace>/.alexanderai/skills/` instead, but this directory is kept for
...
/// so user-scope skills (`~/.alexanderai/skills/`, `~/.agents/skills/`) are
...
///   `~/.alexanderai/skills/` and `~/.agents/skills/`.
...
///
/// Looks for `<workspace>/.alexanderai/trust`. The marker file's contents are
...
///
/// `Workflow` roots (`.alexanderai/workflows/`) hold task *automations* authored
/// via "New workflow". `Skill` roots (`.alexanderai/skills/`, `.agents/skills/`,
...
/// new automations created via "New workflow" land in `~/.alexanderai/workflows/`.
...
fn user_roots(home: &Path) -> Vec<(PathBuf, RootKind)> {
    ...
    vec![
        (home.join(".alexanderai").join("skills"), RootKind::Skill),
        (home.join(".agents").join("skills"), RootKind::Skill),
        (
            home.join(".alexanderai").join("workflows"),
            RootKind::Workflow,
        ),
    ]
}
...
fn project_roots(workspace: &Path) -> Vec<(PathBuf, RootKind)> {
    vec![
        (
            workspace.join(".alexanderai").join("skills"),
            RootKind::Skill,
        ),
        (workspace.join(".agents").join("skills"), RootKind::Skill),
        (
            workspace.join(".alexanderai").join("workflows"),
            RootKind::Workflow,
        ),
    ]
}
...
/// `.alexanderai/trust` marker) and locating the matching entry; this keeps the
...
mod include_skills_tests {
    ...
    fn automations_excludes_skill_roots_but_full_discover_includes_them() {
        ...
        // A registry-style install lands under `~/.alexanderai/skills/`.
        seed_bundle(
            &home_path.join(".alexanderai").join("skills"),
            "installed-skill",
            "SKILL.md",
        );
        // A "New workflow" automation lands under `~/.alexanderai/workflows/`.
        seed_bundle(
            &home_path.join(".alexanderai").join("workflows"),
            "my-automation",
            "WORKFLOW.md",
        );
