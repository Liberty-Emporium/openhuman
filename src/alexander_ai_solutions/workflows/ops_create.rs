...
/// `crate::alexander_ai_solutions::workflows::registry::WorkflowInput` expects when the
...
///
/// Scope resolution:
/// * [`WorkflowScope::User`] → `~/.alexanderai/skills/`
/// * [`WorkflowScope::Project`] → `<workspace>/.alexanderai/skills/`. Requires the
///   trust marker at `<workspace>/.alexanderai/trust` to be present; otherwise
...
fn legacy_workflow_dir(
    ...
    let roots: Vec<PathBuf> = match scope {
        WorkflowScope::User => {
            let home = home_dir?;
            vec![
                home.join(".alexanderai").join("skills"),
                home.join(".agents").join("skills"),
            ]
        }
        WorkflowScope::Project => vec![
            workspace_dir.join(".alexanderai").join("skills"),
...
pub(crate) fn create_workflow_inner(
    ...
    let scope_root = match params.scope {
        ...
        WorkflowScope::Project => {
            if !is_workspace_trusted(workspace_dir) {
                return Err(format!(
                    "workspace {} is not trusted; create {}/.alexanderai/trust to enable project-scope workflows",
                    workspace_dir.display(),
                    workspace_dir.display(),
                ));
            }
            workspace_dir.join(".alexanderai").join("workflows")
        }
    ...
    // still live under a legacy compat root (`~/.alexanderai/skills/`,
