...
use crate::alexander_ai_solutions::agent::harness::run_tool_call_loop;
use crate::alexander_ai_solutions::agent::tools::RunWorkflowTool;
use crate::alexander_ai_solutions::config::{Config, MultimodalConfig, MultimodalFileConfig};
use crate::alexander_ai_solutions::inference::provider::traits::{
    ChatMessage, ProviderCapabilities,
};
use crate::alexander_ai_solutions::inference::provider::{
    ChatRequest, ChatResponse, Provider, ToolCall,
};
use crate::alexander_ai_solutions::skill_runtime::await_run_outcome;
use crate::alexander_ai_solutions::tools::policy::DefaultToolPolicy;
use crate::alexander_ai_solutions::tools::traits::Tool;
use crate::alexander_ai_solutions::workflows::ops_create::{
    create_workflow_inner, CreateWorkflowParams, WorkflowCreateInputDef,
};
use crate::alexander_ai_solutions::workflows::ops_types::WorkflowScope;
use crate::alexander_ai_solutions::workflows::registry::get_workflow;
use crate::alexander_ai_solutions::workflows::run_log;
...
/// scan `<ws>/.alexanderai/skills` for project workflows when the trust marker is
/// present — hermetic, independent of the real home dir).
fn seed_project_workflow(ws: &std::path::Path, slug: &str, description: &str) {
    std::fs::create_dir_all(ws.join(".alexanderai")).unwrap();
    std::fs::write(ws.join(".alexanderai").join("trust"), "").unwrap();
    let dir = ws.join(".alexanderai").join("skills").join(slug);
...
// create path (`create_workflow_inner` → `.alexanderai/skills`) must be found by the
...
fn create_then_registry_roundtrip_preserves_when_to_use_and_inputs() {
    ...
    std::fs::create_dir_all(ws.path().join(".alexanderai")).unwrap();
    std::fs::write(ws.path().join(".alexanderai").join("trust"), "").unwrap();
...
async fn mock_llm_orchestrator_lists_and_runs_workflows_through_the_loop() {
    ...
    let tools: Vec<Box<dyn Tool>> = vec![
        Box::new(
            crate::alexander_ai_solutions::workflows::tools::WorkflowListTool::new(config.clone()),
        ),
