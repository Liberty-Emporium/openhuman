...
/// it to `<workspace>/.alexanderai/skills/<slug>/SKILL.md`.
///
/// Design rationale: openhuman's skill discovery scans
/// `<workspace>/.alexanderai/skills/` (plus `~/.alexanderai/skills/` and legacy
...
pub(crate) async fn install_workflow_from_url_with_home(
    ...
    // Install to user scope (`~/.alexanderai/skills/<slug>`), which `discover_workflows`
    // scans unconditionally. Project scope (`<ws>/.alexanderai/skills/`) is gated on
    // a `<ws>/.alexanderai/trust` marker and would render the install invisible to the
...
pub struct UninstallWorkflowParams {
    ...
    /// `~/.alexanderai/skills/<slug>/`. Retained as `name` for wire-format
...
/// Remove an installed user-scope SKILL.md skill from `~/.alexanderai/skills/`.
///
...
pub fn uninstall_workflow(
    ...
    // Workflows created post-rename live under `~/.alexanderai/workflows/`; older
    // ones under `~/.alexanderai/skills/` or the legacy `~/.agents/skills/` root.
