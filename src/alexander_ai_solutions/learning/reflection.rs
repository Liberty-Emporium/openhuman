...
use crate::alexander_ai_solutions::config::{Config, LearningConfig, ReflectionSource};
use crate::alexander_ai_solutions::learning::candidate::{
    self, CueFamily, EvidenceRef, FacetClass,
};
use crate::alexander_ai_solutions::memory::{Memory, MemoryCategory};
...
pub struct ReflectionHook {
    ...
    provider: Option<Arc<dyn crate::alexander_ai_solutions::inference::provider::Provider>>,
...
impl ReflectionHook {
    ...
    async fn run_reflection(&self, prompt: &str) -> anyhow::Result<String> {
        match self.config.reflection_source {
            ReflectionSource::Local => {
                ...
                let service =
                    crate::alexander_ai_solutions::inference::local::global(&self.full_config);
                service
    ...
    fn emit_candidates_from_reflection(output: &ReflectionOutput, episodic_id: i64) {
        ...
        for (n, pref) in output.user_preferences.iter().enumerate() {
            ...
            buf.push(
                crate::alexander_ai_solutions::learning::candidate::LearningCandidate {
...
impl PostTurnHook for ReflectionHook {
    ...
    async fn on_turn_complete(&self, ctx: &TurnContext) -> anyhow::Result<()> {
        ...
        if self.config.enabled {
            ...
            let episodic_id =
                crate::alexander_ai_solutions::learning::candidate::global().len() as i64;
        ...
        let episodic_id = crate::alexander_ai_solutions::learning::candidate::global().len() as i64;
