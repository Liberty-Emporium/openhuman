...
use crate::alexander_ai_solutions::integrations::{build_client, IntegrationClient};
use regex::Regex;
use serde_json::json;
...
pub async fn summarise_profile_with_llm(config: &Config, raw_md: &str) -> anyhow::Result<String> {
    use crate::alexander_ai_solutions::inference::provider::ops::{
...
fn build_memory_client() -> anyhow::Result<crate::alexander_ai_solutions::memory_store::MemoryClient>
{
    crate::alexander_ai_solutions::memory_store::MemoryClient::new_local()
...
async fn persist_linkedin_profile(
    memory: &crate::alexander_ai_solutions::memory_store::MemoryClient,
    url: &str,
    data: &serde_json::Value,
) -> anyhow::Result<()> {
    let content = format!(
