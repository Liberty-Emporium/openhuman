...
use crate::alexander_ai_solutions::runtime_python::process::PythonLaunchSpec;
...
async fn prepare_launch(config: &Config) -> Result<ServerLaunch> {
    ...
    let python_bin = if want_spacy {
        ...
        crate::alexander_ai_solutions::runtime_python::PythonBootstrap::new(
            config.runtime_python.clone(),
        )
        .resolve()
        .await?
        .python_bin
...
async fn spawn_inner(launch: &ServerLaunch) -> Result<ServerInner> {
    ...
    let resolved = crate::alexander_ai_solutions::runtime_python::ResolvedPython {
        bin_dir: launch
            .python_bin
            .parent()
            .map(PathBuf::from)
        ...
        source: crate::alexander_ai_solutions::runtime_python::PythonSource::Managed,
    };
    let mut spec = PythonLaunchSpec::new(launch.script_path.clone());
    for (key, value) in &launch.env {
        spec.env.insert(key.clone(), value.clone());
    }
    let mut child = crate::alexander_ai_solutions::runtime_python::process::spawn_stdio_process(
        &resolved, &spec,
    )
