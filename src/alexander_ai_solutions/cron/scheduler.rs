...
use crate::alexander_ai_solutions::cron::{
...
fn cron_alert_body(job: &CronJob, output: &str) -> String {
    ...
    crate::alexander_ai_solutions::util::truncate_with_ellipsis(&body, 512)
}
...
fn is_budget_exhausted_failure(
    ...
    let signal = last_agent_error.unwrap_or(last_output);
    crate::alexander_ai_solutions::inference::provider::is_budget_exhausted_message(signal)
}
...
async fn execute_and_persist_job(
    ...
    publish_global(DomainEvent::CronJobCompleted {
        job_id: job.id.clone(),
        success,
        output: crate::alexander_ai_solutions::util::truncate_with_ellipsis(&output, 512),
    });
    let failure_message = (!success)
        .then(|| crate::alexander_ai_solutions::util::truncate_with_ellipsis(&output, 256));
    ...
    if let Some(ref agent_id) = job.agent_id {
            ...
            crate::alexander_ai_solutions::agent::harness::definition::AgentDefinitionRegistry::global()
        {
            if let Some(def) = registry.get(agent_id) {
                ...
                use crate::alexander_ai_solutions::agent::harness::definition::ModelSpec;
                let fallback_model = effective
                    .default_model
                    .clone()
                    .unwrap_or_else(|| crate::alexander_ai_solutions::config::DEFAULT_MODEL.to_string());
                ...
                Ok(mut agent) => {
                    ...
                    let origin =
                        crate::alexander_ai_solutions::agent::turn_origin::AgentTurnOrigin::TrustedAutomation {
                            job_id: job.id.clone(),
                            source:
                                crate::alexander_ai_solutions::agent::turn_origin::TrustedAutomationSource::Cron,
                        };
                    let turn = crate::alexander_ai_solutions::agent::turn_origin::with_origin(
                        origin,
                    ...
                    if is_morning_briefing_job(job) {
                        ...
                        crate::alexander_ai_solutions::agent::harness::with_task_recency_window(
...
fn push_cron_alert(config: &Config, job: &CronJob, output: &str) {
    use crate::alexander_ai_solutions::notifications::store as notif_store;
    use crate::alexander_ai_solutions::notifications::types::{
