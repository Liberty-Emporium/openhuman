...
//! channels module's
//! [`crate::alexander_ai_solutions::channels::proactive::ProactiveMessageSubscriber`]
...
///
/// [`set_onboarding_completed`]: crate::alexander_ai_solutions::config::ops::ui::set_onboarding_completed
...
fn prune_legacy_welcome(
    config: &Config,
    existing: &[crate::alexander_ai_solutions::cron::CronJob],
) {
    ...
    fn seeds_morning_briefing_disabled_and_idempotent() {
        ...
        assert!(
            jobs.iter()
                .filter(|j| matches!(
                    j.job_type,
                    crate::alexander_ai_solutions::cron::JobType::Agent
                ))
                .all(|j| !j.enabled),
