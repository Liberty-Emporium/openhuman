...
use crate::alexander_ai_solutions::cron::{self, ActiveHours, DeliveryConfig};
...
fn test_job(command: &str) -> CronJob {
    CronJob {
        id: "test-job".into(),
        expression: "* * * * *".into(),
        schedule: crate::alexander_ai_solutions::cron::Schedule::Cron {
            expr: "* * * * *".into(),
            tz: None,
            active_hours: None,
        },
...
async fn push_cron_alert_deduplicates_repeated_morning_briefing_failures() {
    ...
    let items = crate::alexander_ai_solutions::notifications::store::list(
        &config,
        10,
        0,
        Some("cron"),
...
async fn run_job_command_blocks_readonly_mode() {
    let tmp = TempDir::new().unwrap();
    let mut config = test_config(&tmp).await;
    config.autonomy.level = crate::alexander_ai_solutions::security::AutonomyLevel::ReadOnly;
...
async fn persist_job_result_success_deletes_one_shot() {
    ...
    let job = cron::add_agent_job(
        &config,
        Some("one-shot".into()),
        crate::alexander_ai_solutions::cron::Schedule::At { at },
        "Hello",
...
async fn cron_alerts(config: &Config) -> usize {
    crate::alexander_ai_solutions::notifications::store::list(config, 10, 0, Some("cron"), None)
        .unwrap()
        .len()
}
