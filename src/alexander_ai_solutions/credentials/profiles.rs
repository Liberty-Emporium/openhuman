...
use crate::alexander_ai_solutions::util::retry_with_backoff;
...
///
/// For a typical path like `/home/alice/.alexanderai/users/uid-123` this
...
impl AuthProfilesStore {
    pub fn new(state_dir: &Path, encrypt_secrets: bool) -> Self {
        ...
        let use_keychain = policy
            == crate::alexander_ai_solutions::keyring_consent::PolicyDecision::Proceed
        ...
        match policy {
            ...
            crate::alexander_ai_solutions::keyring_consent::PolicyDecision::ConsentRequired => {
                log::warn!(
            ...
            crate::alexander_ai_solutions::keyring_consent::PolicyDecision::Declined => {
                log::warn!(
    ...
    fn keychain_store_secrets(&self, profile: &AuthProfile) -> anyhow::Result<()> {
        ...
        crate::alexander_ai_solutions::keyring::set(&self.user_id, &key, &payload).map_err(
            |e| {
                anyhow::anyhow!(
    ...
    fn keychain_load_secrets(&self, profile_id: &str) -> anyhow::Result<Option<KeychainSecrets>> {
        ...
        let payload = match crate::alexander_ai_solutions::keyring::get(&self.user_id, &key) {
            Ok(Some(p)) => p,
            Ok(None) => {
                log::debug!(
    ...
    fn keychain_delete_secrets(&self, profile_id: &str) {
        let key = self.keychain_key_for_profile(profile_id);
        if let Err(e) = crate::alexander_ai_solutions::keyring::delete(&self.user_id, &key) {
            log::warn!(
    ...
    fn acquire_lock(&self) -> Result<AuthProfileLockGuard> {
        ...
        loop {
            let open_result = crate::alexander_ai_solutions::util::retry_with_backoff(
