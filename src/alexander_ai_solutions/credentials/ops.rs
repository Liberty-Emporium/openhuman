...
use crate::alexander_ai_solutions::keyring::SecretStore;
...
use crate::alexander_ai_solutions::memory_conversations as conversations;
...
pub async fn start_login_gated_services(config: &Config) {
    ...
    if config.local_ai.runtime_enabled {
        let service = crate::alexander_ai_solutions::inference::local::global(config);
    ...
    crate::alexander_ai_solutions::voice::server::start_if_enabled(config).await;
    ...
    if !config.voice_server.auto_start {
        crate::alexander_ai_solutions::voice::dictation_listener::start_if_enabled(config).await;
    }

    // 3b. Always-on listening (Phase 2): continuous mic + VAD → STT → agent,
    //     no hotkey. Opt-in via config.voice_server.always_on_enabled.
    crate::alexander_ai_solutions::voice::always_on::start_if_enabled(config).await;

    // 4. Screen intelligence (capture + vision analysis)
    crate::alexander_ai_solutions::screen_intelligence::server::start_if_enabled(config).await;

    // 5. Autocomplete (text suggestions + Swift overlay helper)
    crate::alexander_ai_solutions::autocomplete::start_if_enabled(config).await;
...
pub async fn stop_login_gated_services(config: &Config) {
    ...
    // 2. Voice server
    if let Some(server) = crate::alexander_ai_solutions::voice::server::try_global_server() {
        server.stop().await;
    ...
    if let Some(server) =
        crate::alexander_ai_solutions::screen_intelligence::server::try_global_server()
    {
        server.stop().await;
    ...
    crate::alexander_ai_solutions::voice::dictation_listener::stop();
    ...
    crate::alexander_ai_solutions::voice::always_on::stop();
...
async fn store_session_inner(
    ...
    if !local_session {
        match decode_jwt_exp(trimmed_token) {
            Some(exp) => {
                metadata.insert(
                    crate::alexander_ai_solutions::credentials::session_support::SESSION_EXPIRES_AT_META
                        .to_string(),
                    exp.to_rfc3339(),
                );
                tracing::info!(
    ...
    let effective_config = if resolved_user_id.is_some() {
        match crate::alexander_ai_solutions::config::load_config_with_timeout().await {
            Ok(c) => c,
            Err(_) => config.clone(),
        }
    } else {
    ...
    if local_session {
        match crate::alexander_ai_solutions::config::ops::set_onboarding_completed(false).await {
    ...
    match crate::alexander_ai_solutions::memory::global::init(
...
pub async fn clear_session(config: &Config) -> Result<RpcOutcome<serde_json::Value>, String> {
    ...
    if let Ok(root_dir) = default_root_openhuman_dir() {
        if let Err(e) = crate::alexander_ai_solutions::config::clear_active_user(&root_dir) {
...
fn clear_provider_auth_error(provider: &str) {
    ...
    crate::alexander_ai_solutions::inference::provider::auth_error_registry::clear(slug);
}
...
///
/// Used by [`crate::alexander_ai_solutions::composio::client::create_composio_client`]
