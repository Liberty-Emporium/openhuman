...
use crate::alexander_ai_solutions::scheduler_gate;
        ...
        match crate::alexander_ai_solutions::config::rpc::load_config_with_timeout().await {
            Ok(config) => {
                let is_local_session = crate::api::jwt::get_session_token(&config)
                    ...
                    .flatten()
                    .is_some_and(|token| {
                        crate::alexander_ai_solutions::credentials::session_support::is_local_session_token(
                            &token,
                        )
                    });
                if is_local_session {
                ...
                if let Err(err) =
                    crate::alexander_ai_solutions::credentials::rpc::clear_session(&config).await
                {
                    tracing::warn!(
                        source = %source,
                        error = %err,
