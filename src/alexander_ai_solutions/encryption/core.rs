...
/// (for example `~/.alexanderai/users/{user_id}` in production or
/// `~/.alexanderai-staging/users/{user_id}` when `OPENHUMAN_APP_ENV=staging`);
...
pub fn get_data_dir() -> Result<PathBuf, String> {
    ...
    let data_dir = if let Some(user_id) =
        crate::alexander_ai_solutions::config::read_active_user_id(&root_dir)
    {
        let user_dir =
            crate::alexander_ai_solutions::config::user_openhuman_dir(&root_dir, &user_id);
...
/// (for example `~/.alexanderai/encryption.key` or `~/.alexanderai-staging/encryption.key`).
