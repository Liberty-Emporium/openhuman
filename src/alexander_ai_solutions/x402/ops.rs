...
async fn derive_wallet_signing_key() -> Result<SigningKey, X402Error> {
    use crate::alexander_ai_solutions::wallet::WalletChain;

    let secret = crate::alexander_ai_solutions::wallet::secret_material(WalletChain::Solana)
        .await
        .map_err(|e| X402Error::Wallet(format!("wallet secret: {e}")))?;

    let config = crate::alexander_ai_solutions::config::rpc::load_config_with_timeout()
        .await
...
async fn derive_evm_signer(
    ...
    let secret = crate::alexander_ai_solutions::wallet::secret_material(WalletChain::Evm)
        .await
        .map_err(|e| X402Error::Wallet(format!("wallet secret: {e}")))?;
...
async fn fetch_recent_blockhash_for_x402() -> Result<[u8; 32], X402Error> {
    ...
    let result: BlockhashResponse = crate::alexander_ai_solutions::wallet::rpc::rpc_call(
        WalletChain::Solana,
        "getLatestBlockhash",
