...
pub(super) fn migrate_legacy_embeddings_to_sidecar(
    ...
    let (provider, model, dims) =
        crate::alexander_ai_solutions::memory_store::effective_embedding_settings(
            &config.memory,
            config.workload_local_model("embeddings").as_deref(),
        );
    let sig = crate::alexander_ai_solutions::embeddings::format_embedding_signature(
        &provider, &model, dims,
    );
    log::info!(
    ...
    if has_uncovered {
        let backfill_job =
            crate::alexander_ai_solutions::memory_queue::types::NewJob::reembed_backfill(
                &crate::alexander_ai_solutions::memory_queue::types::ReembedBackfillPayload {
                    signature: sig.clone(),
                },
            )?;
        crate::alexander_ai_solutions::memory_queue::enqueue_tx(&tx, &backfill_job)?;
    }
