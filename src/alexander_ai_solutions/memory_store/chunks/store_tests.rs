...
use super::*;
use crate::alexander_ai_solutions::memory_store::chunks::types::chunk_id;
use chrono::TimeZone;
use rusqlite::params;
...
async fn clear_memory_delete_cascades_orphaned_source_tree_and_settles_queued_job() {
    use crate::alexander_ai_solutions::memory_queue::{store as queue_store, types as queue_types};
    ...
    let outcome = crate::alexander_ai_solutions::memory_queue::handlers::handle_job(&cfg, &claimed)
        .await
...
fn clear_memory_delete_keeps_tree_when_another_connection_still_owns_chunks() {
    use crate::alexander_ai_solutions::memory_store::trees::store as tree_store;
    use crate::alexander_ai_solutions::memory_store::trees::types::{
        Buffer, Tree, TreeKind, TreeStatus,
    };
...
async fn queued_jobs_for_deleted_chunk_settle_to_done() {
    ...
    for _ in 0..2 {
        ...
        let outcome = crate::alexander_ai_solutions::memory_queue::handlers::handle_job(&cfg, &job)
            .await
            .expect("handle_job ok");
        assert!(
            matches!(outcome, queue_types::JobOutcome::Done),
...
fn legacy_embeddings_migrate_to_sidecar_once() {
    ...
    let (p, m, dims) = crate::alexander_ai_solutions::memory_store::effective_embedding_settings(
        &cfg.memory,
        cfg.workload_local_model("embeddings").as_deref(),
    );
    let sig = crate::alexander_ai_solutions::embeddings::format_embedding_signature(&p, &m, dims);
...
fn clear_reembed_skipped_for_signature_removes_all_tombstones_for_sig() {
    ...
    .unwrap();
    crate::alexander_ai_solutions::memory_store::trees::store::mark_summary_reembed_skipped(
        &cfg,
        summary_id,
        &sig,
        "summary tombstone",
...
async fn delete_source_rpc_purges_document_source_fully() {
    use crate::alexander_ai_solutions::memory::read_rpc::{
        delete_source_rpc, list_chunks_rpc, recall_rpc,
    };
    use crate::alexander_ai_solutions::memory_store::trees::store as tree_store;
...
async fn delete_source_rpc_clears_versioned_ingest_gates() {
    use crate::alexander_ai_solutions::memory::read_rpc::delete_source_rpc;
...
async fn delete_source_rpc_cleans_legacy_partial_delete() {
    use crate::alexander_ai_solutions::memory::read_rpc::{delete_chunk_rpc, delete_source_rpc};
    use crate::alexander_ai_solutions::memory_store::trees::store as tree_store;
    use crate::alexander_ai_solutions::memory_store::trees::types::TreeKind;
...
fn delete_source_registered_in_schema_and_controllers() {
    use crate::alexander_ai_solutions::memory::schema::{
