...
use crate::alexander_ai_solutions::memory::util::redact::{self, redact as redact_value};
use crate::alexander_ai_solutions::memory_store::chunks::types::{
    Chunk, Metadata, SourceKind, SourceRef,
};
use crate::alexander_ai_solutions::memory_store::content::StagedChunk;
...
pub fn list_chunks(config: &Config, query: &ListChunksQuery) -> Result<Vec<Chunk>> {
    with_connection(config, |conn| {
        ...
        if let Some(ref allowed) = query.source_scope {
            let before = rows.len();
            rows.retain(|c| {
                crate::alexander_ai_solutions::memory::source_scope::chunk_source_allowed_in(
                    allowed,
                    &c.metadata.tags,
...
fn delete_chunks_by_source_filter(
    ...
    let deleted = with_connection(config, |conn| {
        ...
        for source_id in &orphaned_deleted_sources {
            if let Some(tree) =
                crate::alexander_ai_solutions::memory_store::trees::store::get_tree_by_scope_conn(
                    &tx,
                    crate::alexander_ai_solutions::memory_store::trees::types::TreeKind::Source,
                    source_id,
                )?
            {
                let cascade = crate::alexander_ai_solutions::memory_store::trees::store::delete_tree_cascade_tx(
                    &tx, &tree.id,
                )?;
