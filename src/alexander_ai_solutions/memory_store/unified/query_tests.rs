...
use crate::alexander_ai_solutions::memory_store::{NamespaceDocumentInput, UnifiedMemory};
...
async fn recall_namespace_memories_includes_namespace_kv() {
    ...
    assert!(hits.iter().any(|hit| matches!(
        hit.kind,
        crate::alexander_ai_solutions::memory_store::MemoryItemKind::Kv
    )));
}

#[tokio::test]
async fn query_returns_episodic_hits_when_available() {
    use crate::alexander_ai_solutions::memory_store::fts5::{self, EpisodicEntry};
    ...
    let episodic_hits: Vec<_> = hits
        .iter()
        .filter(|h| h.kind == crate::alexander_ai_solutions::memory_store::MemoryItemKind::Episodic)
        .collect();
    assert!(
...
async fn query_returns_event_hits_when_available() {
    use crate::alexander_ai_solutions::memory_store::events::{self, EventRecord, EventType};
    ...
    let event_hits: Vec<_> = hits
        .iter()
        .filter(|h| h.kind == crate::alexander_ai_solutions::memory_store::MemoryItemKind::Event)
        .collect();
    assert!(
        !event_hits.is_empty(),
...
async fn query_episodic_hits_have_correct_kind() {
    ...
    for hit in hits.iter().filter(|h| h.id.starts_with("episodic:")) {
        assert_eq!(
            hit.kind,
            crate::alexander_ai_solutions::memory_store::MemoryItemKind::Episodic,
...
async fn recall_supporting_relations_stay_scoped_per_document() {
    ...
    let objects = |hit: &crate::alexander_ai_solutions::memory_store::NamespaceMemoryHit| {
...
use crate::alexander_ai_solutions::embeddings::EmbeddingProvider;
