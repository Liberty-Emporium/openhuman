...
fn run_read(args: &[String]) -> Result<()> {
    ...
    rt.block_on(async {
        let params = crate::alexander_ai_solutions::text_input::ReadFieldParams {
            include_bounds: Some(opts.include_bounds),
        };
        let outcome = crate::alexander_ai_solutions::text_input::rpc::read_field(params)
            .await
...
fn run_insert(args: &[String]) -> Result<()> {
    ...
    rt.block_on(async {
        let params = crate::alexander_ai_solutions::text_input::InsertTextParams {
            text,
            validate_focus: None,
            expected_app: None,
            expected_role: None,
        };
        let outcome = crate::alexander_ai_solutions::text_input::rpc::insert_text(params)
            .await
            .map_err(|e| anyhow::anyhow!(e))?;
...
fn run_ghost(args: &[String]) -> Result<()> {
    ...
    rt.block_on(async {
        let params = crate::alexander_ai_solutions::text_input::ShowGhostTextParams {
            text,
            ttl_ms: Some(opts.ttl_ms),
            bounds: None,
        };
        let outcome = crate::alexander_ai_solutions::text_input::rpc::show_ghost(params)
            .await
            .map_err(|e| anyhow::anyhow!(e))?;
...
fn run_dismiss(args: &[String]) -> Result<()> {
    ...
    rt.block_on(async {
        let outcome = crate::alexander_ai_solutions::text_input::rpc::dismiss_ghost()
            .await
            .map_err(|e| anyhow::anyhow!(e))?;
