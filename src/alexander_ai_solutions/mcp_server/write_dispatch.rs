...
use crate::alexander_ai_solutions::mcp_audit::{self, NewMcpWriteRecord};
...
mod tests {
    ...
    async fn audit_write_rejection_records_failure_row() {
        ...
        for _ in 0..50 {
            rows = crate::alexander_ai_solutions::mcp_audit::list_writes(
                &config,
                &crate::alexander_ai_solutions::mcp_audit::McpWriteListQuery::default(),
            )
            .expect("list writes");
            if rows.len() == 1 {
                break;
