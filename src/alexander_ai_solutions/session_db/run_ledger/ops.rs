...
pub fn upsert_agent_run(config: &Config, upsert: AgentRunUpsert) -> Result<AgentRun> {
    ...
    crate::alexander_ai_solutions::session_db::store::with_connection(config, |conn| {
...
/// may still continue.
///
/// [`register_run_ledger_finalize_subscriber`]: crate::alexander_ai_solutions::agent_orchestration::run_ledger_finalize::register_run_ledger_finalize_subscriber
...
pub fn claim_agent_team_task(
    ...
    let outcome = crate::alexander_ai_solutions::session_db::store::with_connection(
        config,
        |conn| {
            init_run_ledger_schema(conn)?;
...
pub fn shutdown_agent_team_member(
    ...
    let result = crate::alexander_ai_solutions::session_db::store::with_connection(
        config,
        |conn| {
            init_run_ledger_schema(conn)?;
            ...
            let released: Vec<String> = {
                ...
                let ids =
                    stmt.query_map(params![team_id, member_id], |row| row.get::<_, String>(0))?;
                let mut out = Vec::new();
                for id in ids {
                    out.push(id?);
                }
