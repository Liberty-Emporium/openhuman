...
//! the per-model input price from [`crate::alexander_ai_solutions::agent::cost`].
//!
...
use crate::alexander_ai_solutions::tokenjuice::types::{CompressorKind, ContentKind};
...
impl Default for State {
    fn default() -> Self {
        Self {
            ...
            attribution_model: crate::alexander_ai_solutions::config::DEFAULT_MODEL.to_string(),
            snapshot_path: None,
        }
    }
}
...
/// snapshot. Called once at startup from [`crate::alexander_ai_solutions::tokenjuice::install_config`].
...
fn cost_saved_usd(model: &str, tokens_saved: u64) -> f64 {
    let pricing = crate::alexander_ai_solutions::agent::cost::lookup_pricing(model);
