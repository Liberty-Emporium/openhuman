...
//! runs inside the shared [`crate::alexander_ai_solutions::runtime_python_server`] as the
//! `kompress` backend — this module is just the thin Rust entry the
//! [`crate::alexander_ai_solutions::tokenjuice::compressors::ml_text`] compressor calls.
//!
...
use crate::alexander_ai_solutions::tokenjuice::types::CompressOptions;
...
pub async fn compress(text: &str, _opts: &CompressOptions) -> Result<Option<String>> {
    ...
    let resp =
        crate::alexander_ai_solutions::runtime_python_server::request_kompress(&config, text)
            .await?;
