...
use crate::alexander_ai_solutions::tokenjuice::detect::hint::{
    extension_to_kind, mime_to_kind, prior_to_kind, tool_prior, ToolPrior,
};
use crate::alexander_ai_solutions::tokenjuice::types::{ContentHint, ContentKind};
...
fn log_line_ratio(content: &str) -> f32 {
    use crate::alexander_ai_solutions::tokenjuice::compressors::signals::{severity, Severity};
    let mut total = 0usize;
    let mut hits = 0usize;
    for line in content.lines().take(2000) {
        if line.trim().is_empty() {
