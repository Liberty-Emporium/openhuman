...
mod tests {
    ...
    async fn lexical_mode_indexes_and_searches_without_embedding() {
        ...
        let out = crate::alexander_ai_solutions::codegraph::search_ref(
            &mut store,
            "r",
            "main",
            "reconcile backoff",
        ...
        assert!(matches!(
            out.coverage,
            crate::alexander_ai_solutions::codegraph::Coverage::Full
        ));
        assert_eq!(
    ...
    async fn cloud_embed_probe() {
        let config = crate::alexander_ai_solutions::config::Config::load_or_init()
            .await
            .expect("load config");
        let provider = crate::alexander_ai_solutions::embeddings::provider_from_config(&config)
    ...
    async fn index_e2e_cloud() {
        ...
        assert!(
            !matches!(
                out.coverage,
                crate::alexander_ai_solutions::codegraph::Coverage::None
            ),
