...
pub async fn mark_read(config: &Config, source_ids: Option<Vec<String>>) -> Result<u64, String> {
    let target_ids: Vec<String> = match source_ids {
        Some(ids) => ids,
        None => crate::alexander_ai_solutions::memory_sources::registry::list_sources()
            .await
...
pub async fn create_checkpoint(label: &str, config: &Config) -> Result<Checkpoint, String> {
    let sources = crate::alexander_ai_solutions::memory_sources::registry::list_sources()
        .await
        .map_err(|e| format!("list sources: {e}"))?;
...
mod tests {
    use super::*;
    use crate::alexander_ai_solutions::memory_diff::git_store::Ledger;
