...
`JailRegistry` is rooted at a base directory (e.g. `~/.alexanderai/jails/` or `<workspace>/jails/`). Each jail is a `<base>/<id>/` directory; metadata for all jails lives in `<base>/index.json`.
