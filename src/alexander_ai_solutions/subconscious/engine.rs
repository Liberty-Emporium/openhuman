...
use crate::alexander_ai_solutions::config::schema::SubconsciousMode;
use crate::alexander_ai_solutions::config::Config;
use crate::alexander_ai_solutions::credentials::{AuthService, APP_SESSION_PROVIDER};
use crate::alexander_ai_solutions::memory_diff::types::CrossSourceDiff;
use anyhow::Result;
...
) -> crate::alexander_ai_solutions::agent::turn_origin::TrustedAutomationSource {
    if has_external_content {
        crate::alexander_ai_solutions::agent::turn_origin::TrustedAutomationSource::SubconsciousTainted
    } else {
        crate::alexander_ai_solutions::agent::turn_origin::TrustedAutomationSource::Subconscious
    }
}
...
impl SubconsciousEngine {
    pub fn new(config: &crate::alexander_ai_solutions::config::Config) -> Self {
        Self::from_heartbeat_config(&config.heartbeat, config.workspace_dir.clone())
    }

    pub fn from_heartbeat_config(
        heartbeat: &crate::alexander_ai_solutions::config::HeartbeatConfig,
        workspace_dir: PathBuf,
    ) -> Self {
    ...
    async fn tick_inner(&self) -> Result<TickResult> {
        ...
        let diff: Option<CrossSourceDiff> = match &baseline {
            Some(checkpoint_id) => {
                match crate::alexander_ai_solutions::memory_diff::ops::diff_since_checkpoint(
                    checkpoint_id,
                    &config,
                    false,
                )
    ...
    async fn prepare_context(&self, world_diff: &str) -> String {
        ...
        match crate::alexander_ai_solutions::agent_orchestration::tools::run_context_scout_with_catalog(
            &question,
            None,
    ...
    async fn refresh_baseline(&self, config: &Config) {
        match crate::alexander_ai_solutions::memory_diff::ops::create_checkpoint(
    ...
    async fn run_agent(
        ...
        use crate::alexander_ai_solutions::agent::Agent;
        ...
        effective.autonomy.level = crate::alexander_ai_solutions::security::AutonomyLevel::Full;
        match self.mode {
        ...
        let origin =
            crate::alexander_ai_solutions::agent::turn_origin::AgentTurnOrigin::TrustedAutomation {
                job_id: format!("subconscious:tick:{}", now_secs() as u64),
                source,
            };
        let response = crate::alexander_ai_solutions::agent::turn_origin::with_origin(
            origin,
    ...
    for source in &diff.per_source {
        ...
        for change in source.changes.iter().take(MAX_ITEMS_PER_SOURCE) {
            let verb = match change.kind {
                crate::alexander_ai_solutions::memory_diff::types::ChangeKind::Added => "added",
                crate::alexander_ai_solutions::memory_diff::types::ChangeKind::Removed => "removed",
                crate::alexander_ai_solutions::memory_diff::types::ChangeKind::Modified => {
                    "modified"
                }
            };
