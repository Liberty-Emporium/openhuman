...
fn resolve_one(config: &crate::alexander_ai_solutions::config::Config, raw: &str) -> SourceChunk {
...
fn resolve_summary(
    ...
    let lookup: anyhow::Result<Option<(String, i64, String)>> =
        crate::alexander_ai_solutions::memory_store::chunks::store::with_connection(
            config,
            |conn| {
