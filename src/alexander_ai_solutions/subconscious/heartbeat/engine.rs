use crate::alexander_ai_solutions::config::{Config, HeartbeatConfig};
use crate::alexander_ai_solutions::subconscious::global::get_or_init_engine;
use anyhow::Result;
use std::path::Path;
...
impl HeartbeatEngine {
        ...
        loop {
            ...
            // overlapping continuations across ticks.
            {
                ...
                tokio::spawn(async move {
                    crate::alexander_ai_solutions::thread_goals::continuation::run_continuation_tick(&cfg).await;
                });
            }
    ...
    async fn run_event_planner_tick_for_config(&self, config: &Config) {
        ...
        let summary =
            crate::alexander_ai_solutions::subconscious::heartbeat::planner::evaluate_and_dispatch(
                config,
                chrono::Utc::now(),
            )
            .await;
