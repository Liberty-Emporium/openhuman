...
fn handle_settings_get(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        crate::alexander_ai_solutions::subconscious::heartbeat::rpc::settings_get()
            .await?
...
fn handle_settings_set(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        let patch: crate::alexander_ai_solutions::subconscious::heartbeat::rpc::HeartbeatSettingsPatch =
            serde_json::from_value(Value::Object(params))
                .map_err(|e| format!("invalid heartbeat settings_set params: {e}"))?;
        crate::alexander_ai_solutions::subconscious::heartbeat::rpc::settings_set(patch)
            .await?
            .into_cli_compatible_json()
    })
}
...
fn handle_tick_now(_params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        crate::alexander_ai_solutions::subconscious::heartbeat::rpc::tick_now()
            .await?
            .into_cli_compatible_json()
    })
}
