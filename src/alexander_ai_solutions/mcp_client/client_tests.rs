...
fn display_description_runs_full_sanitization_pipeline() {
    ...
    assert!(
        out.len() <= crate::alexander_ai_solutions::mcp_client::sanitize::MAX_DESCRIPTION_BYTES
    );
}
...
fn display_title_caps_at_max_title_bytes() {
    ...
    assert!(out.len() <= crate::alexander_ai_solutions::mcp_client::sanitize::MAX_TITLE_BYTES);
}
