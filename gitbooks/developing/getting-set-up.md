...
```
[openhuman] CEF cache at /Users/<you>/Library/Caches/com.alexanderai.app/cef is held by another OpenHuman instance (host <hostname>, pid 12345).
...
Dev and release builds still share `com.alexanderai.app` as the cache identifier. Isolating dev to a separate `com.alexanderai.app.dev` cache requires changes to the vendored `tauri-runtime-cef` (cache path is built inside the runtime from the bundle identifier, not exposed to the openhuman shell). Tracked as a follow-up to #864.
