...
* **Cached and fast.** The catalog is fetched on boot in the background (without blocking startup), cached locally at `~/.alexanderai/skill-registry/cache.json` with a ~1-hour TTL and served stale-while-revalidate. A single-flight gate prevents duplicate downloads of the large catalog.
