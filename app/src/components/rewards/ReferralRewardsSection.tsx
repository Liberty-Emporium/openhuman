...
const ReferralRewardsSection = () => {
  ...
  const handleShare = async () => {
    if (!referralCodeToCopy) return;
    const shareText = [
      'Join me on Alexander AI Solutions.',
      `Referral code: ${referralCodeToCopy}`,
      `Download Alexander AI Solutions: ${LATEST_APP_DOWNLOAD_URL}`,
    ].join('\n');

    try {
      if (navigator.share) {
        await navigator.share({ title: 'Alexander AI Solutions', text: shareText });
      } else {
