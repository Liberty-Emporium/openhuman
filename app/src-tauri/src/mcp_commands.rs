...
//! - `mcp_resolve_binary_path` — locate the `alexander-ai-solutions-core` binary on disk.
...
pub struct McpBinaryInfo {
    /// Absolute path to the `alexander-ai-solutions-core` binary.
    pub path: String,
...
/// `target/debug/alexander-ai-solutions-core[.exe]`. Returns the full path to the binary
/// when found, or `None` if the tree is exhausted.
fn find_debug_binary_walking_up(start: &std::path::Path) -> Option<PathBuf> {
    #[cfg(target_os = "windows")]
    let bin_name = "alexander-ai-solutions-core.exe";
    #[cfg(not(target_os = "windows"))]
    let bin_name = "alexander-ai-solutions-core";
...
/// Resolve the absolute path to the `alexander-ai-solutions-core` binary.
///
/// In dev builds (`cfg!(debug_assertions)`) we:
/// 1. Check `OPENHUMAN_CORE_BINARY_PATH` env var first.
/// 2. Walk up from `current_exe()` looking for `target/debug/alexander-ai-solutions-core`.
///
/// In release builds the binary is a sibling of the shell executable:
/// - macOS: `../MacOS/alexander-ai-solutions-core` relative to the host exe.
...
fn resolve_binary_path() -> Result<PathBuf, String> {
    ...
    #[cfg(target_os = "macos")]
    let candidate = {
        // macOS .app: Contents/MacOS/<host> → Contents/MacOS/alexander-ai-solutions-core
        exe.parent()
    ...
    if !candidate.exists() {
        return Err(format!(
            "alexander-ai-solutions-core binary not found at expected path: {}",
            candidate.display()
        ));
    }
...
/// Tauri command — resolve the `alexander-ai-solutions-core` binary path and OS name.
///
...
mod tests {
    ...
    /// `target/debug/alexander-ai-solutions-core` by walking up from the test executable.
    ///
    /// We only assert the path *contains* `alexander-ai-solutions-core` — the binary may or
    ...
    fn binary_path_result_contains_openhuman_core() {
        match resolve_binary_path() {
            Ok(p) => {
                ...
                assert!(
                    s.contains("alexander-ai-solutions-core"),
                    "resolved path should contain 'alexander-ai-solutions-core', got: {s}"
                );
            }
            Err(e) => {
                ...
                assert!(
                    e.contains("alexander-ai-solutions-core")
    ...
    fn find_debug_binary_returns_none_for_empty_dir() {
        ...
        // of /tmp (or equivalent) will contain target/debug/alexander-ai-solutions-core.
